...
import { commands } from './commands-player-inventory'

describe('commands-player-inventory', () => {
  ...
  it('contains expected command names', () => {
    ...
    expect(names).toContain('unequip')
    expect(names).toContain('attune')
    expect(names).toContain('unattune')
    expect(names).toContain('mastery')
    expect(names).toContain('useitem')
    expect(names).toContain('inventory')
  })

  it('all inventory commands are player category and not dmOnly', () => {
    for (const cmd of commands) {
