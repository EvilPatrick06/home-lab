...
_BMOSEUP_DIR = os.path.abspath(os.path.join(_PI_DIR, "..", ".."))  # repo root / BMO-setup

HEALTH_CHECK_SH = os.path.join(_PI_DIR, "health_check.sh")
E2E_TEST_SH = os.path.join(_PI_DIR, "e2e_test.sh")
SETUP_BMO_SH = os.path.join(_BMOSEUP_DIR, "BMO-setup", "setup-bmo.sh")
DEPLOY_SH = os.path.join(_BMOSEUP_DIR, "BMO-setup", "docker", "deploy.sh")
...
class TestScriptsDirectory:
    """Syntax validation for all scripts in BMO-setup/pi/scripts/."""
