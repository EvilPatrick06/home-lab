...
**Resolution (2026-07-03):** implemented on `auto/bmo-reorg-batch`. Files relocated with `git mv` (rename-detected) and all import sites updated; affected pytest green through the admission gate, no new failures.
...
**Resolution [2026-07-02, bmo-resolver]:** Fixed on `auto/bmo-resolver`. Escaped the literal braces in the two DESIGN_PROMPT endpoint examples (`{{state:...}}`, `{{scene:"movie"}}`) so `str.format()` no longer treats them as replacement fields; added `tests/agents/test_plan_agent_prompts.py` asserting DESIGN_PROMPT and REDESIGN_PROMPT format cleanly (regression fails CI if braces are ever un-escaped again). Audited every other `.format()`-ed agent template (learning/research/cleanup/monitoring/deploy/design/docs/security/review SYSTEM_PROMPTs, router CLASSIFICATION_PROMPT, plan EXPLORE_PROMPT) — all format cleanly; the four SYSTEM_PROMPTs containing literal JSON braces (calendar/smart-home/timer/music) are passed as plain strings, never formatted, so they are safe.
...
**Resolution [2026-07-02, bmo-resolver]:** Fixed on `auto/bmo-resolver`. Added the same `if not camera: return 503 "Camera service not available"` guard the sibling endpoints use at the top of `api_camera_describe`, so the request fails fast instead of spawning `_do_describe` against `camera = None` and leaking the raw NoneType AttributeError via `vision_result`. Test: `tests/test_camera_describe_guard.py` (POST with no camera -> 503). The optional classifier NoneType branch was skipped — with the guard, the background thread can no longer be entered without a camera object, so the classifier can't see that error class from this endpoint.
...
**Resolution [2026-07-02, bmo-resolver]:** Fixed on `auto/bmo-resolver` in `voice_pipeline._play_audio`: (1) the flat 120s timeout is replaced by `_playback_timeout_s()` — decoded clip duration via ffprobe + 10s grace, clamped to [15, 120], falling back to the old 120s when ffprobe can't read the file — so a stuck playback of a ~3s chime now aborts in ~15s, not 2 minutes; (2) `subprocess.TimeoutExpired` is caught inside `_play_audio` (subprocess.run kills the wedged ffplay) and playback is retried once; (3) a doubly-failed playback is logged as a distinct `[tts] Playback failed` ERROR and NOT raised into the TTS-provider chain, so it can no longer masquerade as "All TTS failed" synthesis failure. Tests: `tests/test_playback_timeout.py` (4 cases: sizing, both clamps, ffprobe-missing fallback). The `fuser -v /dev/snd/*` capture-on-recurrence remains a debugging note; the suspected stale-child device-holder is separately mitigated by the new `ExecStopPost` adb teardown (see the left-over-process entry).
...
- **Resolved by:** bmo-resolver  **Branch:** `auto/bmo-resolver` (integrator merges)
- **Fix:** added a dedicated `self._api_lock = threading.RLock()` and wrapped the whole `_with_service_retry` build-or-reuse-client + execute + retry sequence in it, so the WS `on_connect` reader and the background poll loop can no longer interleave on the one shared `httplib2` connection (no more `ResponseNotReady` / reentrant `BufferedReader` read). `service.py` (`__init__`, `_with_service_retry`).
- **Verified:** `py_compile` clean; `pytest -k calendar` 70 passed.
- **Activation:** awaits the gated deploy + `bmo.service` restart (see board `restart-bmo`).
...
### [2026-06-29] `bmo.service` `KillMode=process` leaked orphan children every restart — reverted to control-group reaping

- **Resolved by:** bmo-resolver  **Branch:** `auto/bmo-resolver` (integrator merges)
- **Fix:** removed `KillMode=process` from `bmo/pi/systemd/bmo.service` (so it defaults to `control-group`, reaping the whole cgroup — audio/worker `python3` subprocs + the `adb` helper — on stop) and added `TimeoutStopSec=20`.
- **Activation:** awaits the gated deploy + `systemctl daemon-reload` + `bmo.service` restart (see board `restart-bmo`).
...
### [2026-06-29] `_follow_up_loop` uncaught `PortAudioError` tore down conversation mode — gated + guarded

- **Resolved by:** bmo-resolver  **Branch:** `auto/bmo-resolver` (integrator merges)
- **Fix:** gated `_wait_for_speech` on `_input_device_available()` (mirrors the wake-word loop) so it returns `False` and degrades to wake-word mode instead of opening a stream that raises `PortAudioError: Error querying device -1`; also added a `_follow_up_loop_guarded` wrapper used by `start_conversation` that catches any residual exception and degrades cleanly. `voice_pipeline.py`.
- **Verified:** `py_compile` clean; added 2 regression tests (`test_wait_for_speech_returns_false_when_no_input_device`, `test_follow_up_loop_guarded_swallows_device_error`); `pytest -k 'InputDeviceGuard or follow_up or wait_for_speech'` 8 passed.
- **Activation:** awaits the gated deploy + `bmo.service` restart (see board `restart-bmo`).
...
### [2026-06-29] Google Calendar OAuth token revoked/expired (duplicate of the already-resolved reauth) — closed

- **Resolved by:** bmo-resolver (bookkeeping)  **Branch:** n/a (operational; token files gitignored/local)
- This entry was logged from the Jun 29 ~01:13 journal scan, i.e. BEFORE the user's ~02:11 reauth. It is the same outage already archived above as **[2026-06-29] Google Calendar refresh token revoked — calendar reauthorized**. Verified resolved: deploy `token.json` carries a fresh `refresh_token`, and `bmo.service` logs `Calendar: OK` through the day (the 17:01 `token expired — waiting for refresh` → 17:43 `Calendar: OK` confirms in-memory auto-refresh now works). Not re-posted to the board.
