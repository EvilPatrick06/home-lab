...
**Resolution [2026-07-17, dnd-resolver]:** Took the preferred fix — stopped committing the artifact: `git rm --cached src/renderer/public/pdf.worker.min.mjs` + gitignored it (postinstall regenerates it on every `npm ci`, so every dev/CI build still materializes the version matching the locked pdfjs-dist; knip already ignored it by path — its now-redundant knip ignore entry was also removed). A fresh `npm ci` on a clean checkout now leaves the tree clean, and API↔Worker version drift is impossible because the served worker is always the locked one. Verified: `git status` clean after `npm ci`; tsc web+node green; targeted vitest green. On `auto/dnd-resolver`, integrator merges.
...
**Resolution [2026-07-17, dnd-resolver]:** Implemented BOTH proposed mechanisms: (1) `generateSessionSummary()` now calls the new `MemoryManager.endSessionSitting()` after appending the summary, so a summary closes the sitting and the next session derives a fresh local-date id; (2) `getSessionLogId()` applies a 6h sitting-gap heuristic — if the last `appendSessionLog` was >6h ago the cached id is discarded, covering multi-day uptime where no summary was ever generated. `appendSessionLog` stamps `lastSessionLogAppendMs`. Added the fake-timer tests from the proposal: day-1 sitting → summary → 2 days later → id is the new date; gap-rollover without a summary; and no rollover mid-sitting across midnight (appends <6h apart). All 66 memory-manager tests green; tsc green. On `auto/dnd-resolver`, integrator merges.
...
**Archive note [2026-07-17, dnd-resolver]:** Entry was already resolved in place by the integrator on 2026-07-03 (bump declined; 1.30.1 pin kept; Dependabot ignore rule added; PR #62 closed) — moved here for log hygiene. The optional follow-up (revisit when react-native-css publishes a lightningcss>=1.31-compatible release, then drop the ignore) remains open as a watch item.
...
**Resolution [2026-07-17, dnd-resolver]:** Baseline is exit 0 again and the CI gate is now BLOCKING. (a) `mobile/**` added to `knip.json` `ignore` (separate npm project with its own lockfile + CI, unreachable from the desktop graph by design — 40 false rows gone). (b) Drop-one-entry audit of the embed/bridge chain: added `src/web/main.embed.tsx` as an entry (the `index.embed.html`/`vite.embed.config.ts` entry point) — the whole bridge chain (`bridge-api`, `bridge-transport`, `install-embed-api`, `src/shared/bridge/*`) proved LIVE (traced from that entry, not exempted); added runtime-registered `src/renderer/public/sw.js` as an entry. Two genuinely-dead exported types (`BridgeRpcMethod`, `BridgeEventName` — no consumer in desktop OR mobile) were DELETED; `BRIDGE_EVENT` (consumed by mobile via its synced `_shared` copy) kept with an `@public` knip tag + explanatory comment. (c) `dotenv` removed from devDependencies (no import/require anywhere; leftover from the removed signing path); lockfile regenerated. (d) The `dnd-app-ci.yml` `Dead code (knip)` step's `continue-on-error: true` is REMOVED — the ratchet can no longer silently rot. `npm run dead-code` exit 0 (only the known-keep dpdm hint remains); tsc web+node green. On `auto/dnd-resolver`, integrator merges.
...
**Resolution [2026-07-17, dnd-resolver]:** Hoisted the heavy dynamic imports (`./updater` pulls `electron-updater`, plus `electron`) out of the test bodies into a `beforeAll` in the `registerUpdateHandlers` describe block, so the module-graph transform/import cost lands in the setup budget instead of the first test's 15s per-test timeout — the same remedy class as the 2026-06-22 flake fixes. All 12 updater tests green (first test now just calls the pre-imported function); tsc green; biome formatted. On `auto/dnd-resolver`, integrator merges.
...
**Resolution [2026-07-15, dnd-resolver]:** Fixed (auto-approve, bug). Added a `localDateStamp()` (LOCAL date parts) helper in `memory-manager.ts` and a cached `getSessionLogId()` on `MemoryManager`: the session-history id is now derived ONCE per sitting from the LOCAL calendar date and cached (managers are per-campaign singletons), so a sitting no longer splits at 00:00 UTC and an evening session west of UTC files under today's local date. Both `ai-service.ts` sites (message-append and `generateSessionSummary`) now call `memMgr.getSessionLogId()`, so the end-of-session summary lands in the same log as its messages. New fake-timer unit tests assert the local-date value and stable-across-midnight caching. tsc(node) + biome + vitest green on `auto/dnd-resolver`; integrator merges.
...
**Resolution [2026-07-15, dnd-resolver]:** Fixed (auto-approve, bug). Added shared `src/renderer/src/utils/local-date.ts` `localDateStamp()` and applied it at all five sites: `GameChatPanel.tsx` + `CombatLogPanel.tsx` export filenames, `TimelineCard.tsx` milestone default (both sites — the persisted-data one), `import-export.ts` `dateStamp`, and `ErrorBoundary.tsx` bug-report filename. Also refactored the existing `chat-transcript-export.ts` inline local-date block onto the shared helper, so the transcript filename and its in-file header now agree. New unit test covers `localDateStamp` (local parts, not the UTC slice). tsc(web) + biome + forbidden-patterns + vitest green on `auto/dnd-resolver`; integrator merges.
...
**Resolution [2026-07-15, dnd-resolver]:** Fixed (auto-approve, bug). `DiceHistory.tsx`'s auto-scroll effect now depends on `filtered.length` (plus `view`) instead of `[view]`, so it fires when a new roll arrives — and only snaps to the bottom when the user is already near the bottom (`scrollHeight - scrollTop - clientHeight < 80`), so a user who scrolled up to read older rolls isn't yanked down (standard chat-log behavior). The now-accurate comment replaces the misleading one. tsc(web) + biome green on `auto/dnd-resolver`; integrator merges.
...
- **Resolution:** Added a traversal guard to the whole book id-keyed storage family. New exported `isSafeBookId` in `book-storage.ts` rejects non-string/empty ids and any id containing `/`, `\`, `..`, or `\0`; `getBookDataPath` (covering `BOOK_SAVE_DATA`/`BOOK_LOAD_DATA`/`removeBook`), `saveBookBytes`, and `importBook` all assert it before any path join, and the `BOOK_SAVE_BYTES`/`BOOK_SAVE_DATA`/`BOOK_LOAD_DATA`/`BOOK_IMPORT` IPC handlers throw `Invalid book id` at the boundary (mirroring the `CAMPAIGN_RESTORE_VERSION`/`CHARACTER_RESTORE_VERSION` guards). Strict `isValidUUID` was deliberately NOT used: core-book ids are slugs (`phb-2024`, `dmg-2024`, `mm-2025`) that must keep working through `getBookDataPath`. 5 new unit tests in `book-storage.test.ts` assert traversal ids are rejected with no write and a valid id lands inside `booksDir` + registers config. Verified: biome clean, `tsc --noEmit -p tsconfig.node.json` green, book-storage suite 31/31 green. No service restart needed — ships with the next app release.
...
- **Resolved by:** dnd-resolver (automated) — bookkeeping; the mechanism was implemented by dnd-bug-resolver
- **Date resolved:** 2026-07-02
- **Resolution:** Already implemented on master via dnd-bug-resolver's 2026-07-02 drift fix (see the resolved entry "mobile `_shared` sync copy has actually drifted"): `mobile/scripts/sync-shared.mjs` now has a `--check` mode and `dnd-app-ci.yml` runs it as the "Mobile shared-copy drift guard" step on every push. Moved out of the active log so it stops being re-posted for approval; no further work needed.
- **Branch:** n/a (log move only; implementation landed via auto/dnd-bug-resolver)
...
- **Resolved by:** dnd-bug-resolver (automated)
- **Date resolved:** 2026-07-02
- **Resolution:** Ran `node mobile/scripts/sync-shared.mjs` and committed the refreshed copy — `mobile/src/_shared/ipc-channels.ts` regains the 5 missing channels (`FILE_OPEN_REQUEST`, `FILE_CONSUME_PENDING`, `CAMPAIGN_VERSIONS`, `CAMPAIGN_RESTORE_VERSION`, `BOOK_SAVE_BYTES`); no other synced file had drifted. Added a `--check` mode to `sync-shared.mjs` (byte-compares every synced file against `src/shared`, reports missing/differing/stale files, exits 1 with a fix hint) and wired it into `dnd-app-ci.yml` as a "Mobile shared-copy drift guard" step beside the other drift guards — deliberately in the every-push desktop CI rather than the path-filtered mobile CI, because the drift originates from desktop-side `src/shared` changes that never touch `dnd-app/mobile/**`. Per the standing won't-do note, the `shared/types` re-export shims were NOT deduped or restructured — this only refreshes the committed copy and guards it. This also closes the [2026-06-28] SUGGESTIONS-LOG-DNDAPP drift-guard suggestion's mechanism.
- **Branch:** auto/dnd-bug-resolver
...
- **Resolved by:** dnd-bug-resolver (automated)
- **Date resolved:** 2026-07-02
- **Resolution:** `persistSnapshotWithEviction()` now (1) evicts through `removeSnapshot()`, clearing an evicted version's body from BOTH backends (IndexedDB best-effort + localStorage) instead of the raw `localStorage.removeItem`, and (2) only evicts versions whose body actually lives in localStorage (oldest-first among those) — when nothing evictable remains it stops and lets the save fail loud, so an IDB-outage fallback can no longer drain the version manifest or orphan IDB-resident bodies (IndexedDB-resident versions keep their manifest entry and body untouched). 3 regression tests added in `auto-save-eviction.test.ts`: both-backend eviction completes the save; a manifest of IDB-resident versions survives an unfreeable-quota outage intact with zero IDB deletes (the destructive failure mode from this entry); a genuinely oversized snapshot still drains localStorage-resident history and fails loud (pre-IDB behavior preserved). Existing 14 auto-save + 2 IDB tests pass unchanged. The optional startup orphan sweep was deliberately omitted: with eviction fixed, every deletion path (trim/delete/evict) goes through `removeSnapshot()`, so no code path leaves an IDB body unreferenced going forward, and pre-fix orphans require a rare mid-session IDB outage on a near-full localStorage (app still in testing).
- **Branch:** auto/dnd-bug-resolver
...
- **Resolved by:** dnd-bug-resolver (automated)
- **Date resolved:** 2026-07-02
- **Resolution:** Dropped the `createdAt: new Date().toISOString()` stamp from `scripts/build/build-chunk-index.mjs` and regenerated + committed `resources/chunk-index.json` — verified a fresh rebuild is now byte-identical to the committed file, and the parsed old/new indexes differ ONLY by the removed `createdAt` (all 5383 chunks equal). Implemented the entry's suggested drift gate: `npm run build:index -- --check` byte-compares a fresh in-memory rebuild against the committed file (mirroring `gen:ipc-surface --check` / `sync:doc-counts --check`, degrading to the existing skip-with-notice when source markdown is absent) and wired it into `dnd-app-ci.yml` — viable in CI because the sources are the tracked `bmo/pi/data/5e-references` tree (PHASE-24 24A). `ChunkIndex.createdAt` is now optional in `src/main/ai/types.ts`; the runtime userData rebuild (`chunk-builder.ts`) still stamps wall-clock time, which is correct there (a per-user, never-committed artifact).
- **Branch:** auto/dnd-bug-resolver
...
- **Resolved by:** dnd-bug-resolver (automated)
- **Date resolved:** 2026-07-02
- **Resolution:** Overtaken by events — fixed on master before this run; verified now. The Dependabot branch's PR #46 was merged 2026-06-29, and PR #50 (`auto/expo-sdk56`, merged 2026-06-29) repaired the resulting Expo SDK 56 breakage along exactly the lines the integrator review prescribed: regenerated/synced `dnd-app/mobile/package-lock.json`, migrated `app.config.ts` top-level `splash` → the `expo-splash-screen` config plugin, and added the mobile bundle-smoke CI gate (`expo export:embed`) so a tsc-green-but-unbuildable bump cannot land silently again. The MAJOR-version adoption (Expo ~56 / typescript ~6 / @babel/core ^8 group) was decided via those reviewed PR merges — not by this agent. dnd-app mobile CI (which runs `npm ci` against the mobile lockfile) has been green on master since (e.g. run 28405834059) and re-ran green for this branch. Nothing further actionable; closed as fixed-on-master.
- **Branch:** auto/dnd-bug-resolver
