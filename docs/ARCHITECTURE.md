...
| `/api/agent/:name/invoke` | POST | Directly invoke one of the 5 agents |
...
- `/api/auth/*`, `/api/account/*`, `/api/sync/*` (Discord-OAuth login + per-user cloud accounts/sync, which supersedes the single-folder rclone backup for signed-in users) → **public Access Bypass** at the edge (the browser VTT holds no CF creds), gated instead by a per-user **JWT** signed with the Flask `SECRET_KEY` and validated by `services/auth_guard.require_user`. The user id comes only from the verified token, so accounts are isolated. Per-user data lives on the Pi (`data/sync/<discord_id>/`) and mirrors to `gdrive:DND-VTT-Accounts/<discord_id>/` via rclone. Desktop login uses a loopback OAuth redirect (main-process); the same-origin browser VTT uses a URL-fragment token. See `bmo/docs/SERVICES.md`.
...
- [`../bmo/docs/AGENTS.md`](../bmo/docs/AGENTS.md) — 5 AI agents
