...
- **`dnd-app`** at **v2.6.4**. Multiplayer (Phase 29), Token System (Phase 13), DM View (Phase 14), and the complete AI Dungeon Master (P6 roadmap — solo play, 127 DM actions, 37 stat-change types, Discord VC narration) shipped. 856 test files. Ships as Windows NSIS installer + Linux AppImage. Open backlog + deferred work: [`dnd-app/docs/phases/PHASE-INDEX.md`](./dnd-app/docs/phases/PHASE-INDEX.md) (the backlog phase set for the whole repo).
- **`bmo`** runs 5 systemd services live on the Pi: `bmo`, `bmo-fan`, `bmo-kiosk`, `bmo-dm-bot`, `bmo-social-bot`. 66 pytest files. Game-discovery registry at `/api/games*` advertises via avahi for Windows-zero-config.
