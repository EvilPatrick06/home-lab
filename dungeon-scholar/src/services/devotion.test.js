...
describe('evaluateClaim (M13 monotone fence)', () => {
  ...
  it('resets the streak to 1 on a gap greater than one day', () => {
    const res = evaluateClaim({
      ...
      today: '2026-05-10',
      lastClaimedDate: '2026-05-07',
      lastClaimedAt: NOW - 3 * 86_400_000,
      loginStreak: 5,
    });
  ...
  it('self-heals a corrupt future fence (≥48h ahead) instead of locking', () => {
    ...
    const res = evaluateClaim({
      now: NOW,
      today: '2026-05-08',
      lastClaimedDate: '2026-05-06',
      lastClaimedAt: NOW + 72 * 3_600_000,
      loginStreak: 9,
    });
...
describe('devotionStatus (F5)', () => {
  it('returns continuing when the last claim was exactly yesterday (gap 1)', () => {
    expect(devotionStatus({ today: '2026-06-24', lastClaimedDate: '2026-06-23', streak: 3, longest: 5 })).toBe(
      'continuing',
    );
  });
  ...
  it('returns firstEver when a stored streak is 0 even with a prior date', () => {
    expect(devotionStatus({ today: '2026-06-24', lastClaimedDate: '2026-06-20', streak: 0, longest: 0 })).toBe(
      'firstEver',
    );
  });
  it('a lapsed first/1-day streak (longest < 2) is lapsedShort, NOT broken', () => {
    expect(devotionStatus({ today: '2026-06-24', lastClaimedDate: '2026-06-21', streak: 1, longest: 1 })).toBe(
      'lapsedShort',
    );
  });
  it('a genuinely lost streak (longest >= 2) lapsing is broken', () => {
    expect(devotionStatus({ today: '2026-06-24', lastClaimedDate: '2026-06-20', streak: 4, longest: 4 })).toBe(
      'broken',
    );
  });
});
