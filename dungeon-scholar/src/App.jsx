...
const DungeonExplore = React.lazy(
	() => import("./components/DungeonExplore.jsx"),
);
...
import { snapshotBaselines, TUTORIAL_STEPS } from "./game/tutorial";
...
const LibraryScreen = React.lazy(
	() => import("./features/library/LibraryScreen.jsx"),
);
...
const RunHistoryScreen = React.lazy(
	() => import("./features/progression/RunHistoryScreen.jsx"),
);
const ShopScreen = React.lazy(
	() => import("./features/progression/ShopScreen.jsx"),
);
const InventoryScreen = React.lazy(
	() => import("./features/progression/InventoryScreen.jsx"),
);
const BestiaryScreen = React.lazy(
	() => import("./features/progression/BestiaryScreen.jsx"),
);
const StableScreen = React.lazy(
	() => import("./features/progression/StableScreen.jsx"),
);
const SpellbookScreen = React.lazy(
	() => import("./features/progression/SpellbookScreen.jsx"),
);
const CalendarScreen = React.lazy(
	() => import("./features/progression/CalendarScreen.jsx"),
);
const AscensionScreen = React.lazy(
	() => import("./features/progression/AscensionScreen.jsx"),
);
const CraftingScreen = React.lazy(
	() => import("./features/progression/CraftingScreen.jsx"),
);
const QuestBoard = React.lazy(() => import("./features/quests/QuestBoard.jsx"));

import HomeScreen from "./features/home/HomeScreen.jsx";

const FlashcardsMode = React.lazy(
	() => import("./features/study/FlashcardsMode.jsx"),
);
...
const MistakeVault = React.lazy(
	() => import("./features/study/MistakeVault.jsx"),
);
const DomainStudyScreen = React.lazy(
	() => import("./features/study/DomainStudyScreen.jsx"),
);
...
function computeInitialScreen() {
	...
	if (
		exam &&
		typeof exam.deadlineMs === "number" &&
		exam.deadlineMs > Date.now()
	) {
		return "practiceExam";
	}
...
export default function DungeonScholarApp() {
	...
	const [screen, setScreen, pendingTomeId, clearPendingTome] =
		useHashRoute(computeInitialScreen);
	const { user } = useAuth();
	const [playerState, setPlayerState, sync] = usePlayerState(
		DEFAULT_STATE,
		user,
	);
	...
	const advanceTutorial = (currentId) => {
		...
		const willAdvance =
			!playerState.tutorialCompleted && curStep && curStep.id === currentId;
		...
		if (willComplete) {
			setTimeout(
				() =>
					showNotif("Tutorial Complete! Welcome, brave scholar.", "levelup"),
				400,
			);
		} else if (willAdvance) {
			setTimeout(
				() => showNotif(`+${curStep.xp || 0} XP — ${curStep.title}`, "xp"),
				100,
			);
		}
	};
	...
	const toggleTutorialPanel = () => {
		setPlayerState((prev) => ({
			...prev,
			tutorialPanelCollapsed: !prev.tutorialPanelCollapsed,
		}));
	};
	...
	const activeTome = useMemo(() => {
		...
		const entry = playerState.library.find(
			(t) => t.id === playerState.activeTomeId,
		);
		return entry || null;
	...
	const courseSet = activeTome
		? activeSealed
			? unsealedTomes[activeTome.id] || null
			: activeTome.data
		: null;
	...
	useEffect(() => {
		setAudioPersistErrorHandler(() =>
			showNotif("Audio settings cannot be saved on this device.", "info"),
		);
	}, []);
	...
	useEffect(() => {
		...
		for (const id of current) {
			...
			if (ach)
				showNotif(`Achievement Unlocked: ${ach.name}`, "achievement", () =>
	...
	useEffect(() => {
		...
		for (const id of current) {
			...
			if (title)
				showNotif(`Title Unlocked: ${title.name}`, "achievement", () =>
					setShowTitles(true),
				);
		}
	...
	useEffect(() => {
		...
		if (
			...
			playerState.weeklyQuests.weekStart !== weekStart
		) {
			setPlayerState((prev) => {
	...
	const handlePasteImport = (text) => {
		...
		try {
			...
			if (!isSealedTome(data) && (!data.metadata || !data.flashcards)) {
				showNotif(
					"Invalid tome format — needs metadata and flashcards",
					"error",
				);
				return false;
			}
	...
	const unlockSealedTome = async (tomeId, passphrase) => {
		...
		if (!entry || !isSealedTome(entry.data))
	...
	return (
		<div
			...
			style={{
				...
				background:
					"radial-gradient(ellipse at top, #1a0e08 0%, #0a0604 50%, #000000 100%)",
			}}
		>
			<div
          ...
          so the import side-effect actually fires. */}
			...
			{rlsExposed && (
				<RlsWarningBanner onDismiss={() => setRlsExposed(false)} />
			)}
          ...
          earlier window.confirm. */}
			...
			{notification && (
				...
				// on a vault vanquish.
				// biome-ignore lint/a11y/noStaticElementInteractions: notification is conditionally interactive (role=button when actionable, status otherwise; handlers no-op when not)
				<div
					...
					<div className="flex items-center gap-3">
						...
						<div>
							<h1
								...
								style={{
									background:
										"linear-gradient(to bottom, #fbbf24 0%, #d97706 50%, #92400e 100%)",
							...
							<p className="text-xs text-amber-700 tracking-[0.3em] italic">
								⚜ A SCHOLAR'S QUEST ⚜
							</p>
						</div>
					</div>
                ...
                from the same source-of-truth expressions the destination pages use. */}
						<div
							...
							style={{
								...
								boxShadow:
									"0 0 10px rgba(245, 158, 11, 0.15), inset 0 0 10px rgba(0,0,0,0.4)",
							}}
						...
						>
							<Coins
								...
								aria-hidden="true"
							/>
							<span className="text-amber-200 font-bold italic tabular-nums">
								{playerState.gold || 0}
							</span>
						</div>
						<button
							...
							title={
								claimableQuestCount > 0
									? `Quest Board (${claimableQuestCount} ready to claim)`
									: "Quest Board"
							}
							aria-label={
								claimableQuestCount > 0
						...
						{screen !== "home" && (
							<button
								...
								onClick={() => {
												...
												onConfirm: () => {
													...
													if (saved.tomeId) {
															...
															? Math.max(
																	...
																	saved.totalSeconds -
																		Math.max(
																			0,
																			Math.ceil(
																				(saved.deadlineMs - Date.now()) / 1000,
																			),
																		),
																)
															: 0;
														const answeredCount = Array.isArray(saved.answers)
															? saved.answers.filter(
																	(a) =>
														...
														const record = {
															...
															totalCount: Array.isArray(saved.sample)
																? saved.sample.length
																: 0,
														...
														setPlayerState((prev) => ({
															...
															library: prev.library.map((t) => {
																...
																return {
																	...
																	progress: {
																		...t.progress,
																		practiceExams: [...prior, record].slice(
																			-20,
																		),
																	},
																};
									...
									if (screen === "flashcards")
										clearSession(SESSION_KIND.FLASHCARDS);
									setScreen("home");
								}}
					...
					<ErrorBoundary onReset={() => setScreen("home")}>
						<div
							...
							style={{
								...
								boxShadow:
									"0 0 30px rgba(180, 83, 9, 0.15), inset 0 0 20px rgba(0,0,0,0.5)",
							}}
						>
							<div className="absolute top-1 left-1 text-amber-700 text-xs">
								⚜
							</div>
							<div className="absolute top-1 right-1 text-amber-700 text-xs">
								⚜
							</div>
							<div className="absolute bottom-1 left-1 text-amber-700 text-xs">
								⚜
							</div>
							<div className="absolute bottom-1 right-1 text-amber-700 text-xs">
								⚜
							</div>
								...
								<div className="flex items-center gap-4">
									...
									<div>
										...
										<div className="text-xs text-amber-700 tracking-wider">
											...
											{playerState.totalXp.toLocaleString()} Total XP ⚔
										</div>
									</div>
								</div>
								<div className="flex-1 min-w-[200px] max-w-md">
									<div className="flex justify-between text-xs mb-1">
										<span className="text-amber-700 tracking-widest">
											EXPERIENCE
										</span>
									...
									<div
										...
										style={{
											background:
												"linear-gradient(to bottom, #1c1917 0%, #0c0a09 100%)",
									...
									>
										<div
											...
											style={{
												width: `${xpPercent}%`,
												background:
													"linear-gradient(to bottom, #fde047 0%, #f59e0b 50%, #b45309 100%)",
                ...
                quest-board collision that triggered the QA report. */}
								<div className="flex gap-4 text-sm">
									...
									>
										...
										>
											{playerState.library.reduce(
												(sum, t) => sum + (t.progress?.runsCompleted || 0),
												0,
											)}
										</div>
										<div className="text-xs text-amber-700 tracking-wider">
									...
									>
										...
										>
											{playerState.library.reduce(
												(sum, t) => sum + (t.progress?.bossesDefeated || 0),
												0,
											)}
										</div>
										<div className="text-xs text-amber-700 tracking-wider">
            ...
            while locked (they don't read decrypted content). */}
										...
										const labelOf = (id) =>
											id === "light"
												...
												: id === "system"
													...
													: "Dark";
										const introTail =
											t === "light" && !playerState.lightModeIntroShown
											...
											() =>
												showNotif(
													...
													() => {
														...
														showNotif(
															`Theme reverted to ${labelOf(prevTheme)}`,
															"info",
															null,
															1800,
														);
							...
							{screen === "flashcards" && courseSet && (
								<FlashcardsMode
									...
									onResumeNotify={(info) =>
										showNotif(
											`Resumed Scroll ${info.index + 1} of ${info.total}`,
											"success",
											null,
											1500,
										)
						...
						{showImportCodeModal && (
							<ImportCodeModal
								...
								onSubmit={handleShareCodeImport}
							/>
						)}
						{shareTomeId && (
							<ShareTomeModal
						...
						{editMetadataTomeId && (
							<MetadataEditModal
								tome={playerState.library.find(
									(t) => t.id === editMetadataTomeId,
								)}
								onSave={(updates) => {
						...
						{notesTome && (
							<TomeNotes
								tome={
									playerState.library.find((t) => t.id === notesTome.id) ||
									notesTome
								}
						...
						{showWelcomeModal && (
							<WelcomeModal onStart={startTutorial} onSkip={skipTutorial} />
						)}
