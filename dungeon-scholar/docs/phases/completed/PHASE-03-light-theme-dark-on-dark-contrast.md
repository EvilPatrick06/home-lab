...
- **03A** `src/index.css` (:root + light override): added theme surface triplets `--surface-rose/red/danger/known/locked` (dark defaults == prior hardcoded values; light values are warm light tints). Convention documented in `docs/DESIGN-CONSTRAINTS.md`. Chose the **var-swap (preferred)** pattern uniformly; the non-inverting `--on-dark-fg` fallback was not needed (no surface must stay dark while carrying inverting text).
- **03B** `FlashcardsMode.jsx:348`: card background routed through `--panel-bg-sapphire` + `--panel-end` (OrnatePanel convention); face text `text-amber-50` now inverts dark-on-light in light theme. OcclusionCard renders inside the same (now theme-aware) surface.
- **03C** `LabMode.jsx`: both trial-card gradients (:104 list, :339 detail) -> `--panel-bg-rose`+`--panel-end`; inner prompt/option/input surfaces (3x `rgba(41,12,27,.6/.7)`) -> `rgba(var(--surface-rose,...),a)`. Rose titles + status chips unchanged.
- **03D** `MistakeVault.jsx:69`: card background -> `--surface-red`+`--surface-deep`; inner explanation panel already used `--surface-modal` (unchanged).
- **03E sweep**: routed remaining hardcoded-dark surfaces through theme vars — `ErrorBoundary.jsx` (:55 amber chunk panel -> `--surface-amber`; :90 red panel -> `--surface-danger`), `ConfirmModal.jsx` + `ResetConfirmModal.jsx` (danger branch -> `--surface-danger`), `ChatMode.jsx` clear-confirm modal -> `--surface-danger`, `SpellbookScreen.jsx` + `BestiaryScreen.jsx` (known/seen -> `--surface-known`, locked/unseen -> `--surface-locked`). `PromptModal.jsx` was already fully theme-aware (no change).
- **03F** `ThemePanel.jsx:64`: "both themes restyle every screen; pick whichever reads best" copy is now accurate; left as-is.
- **03G** `ChatMode.jsx`: Oracle bubble (:563) + user bubble (:545) body text changed from fixed `#fef3c7` to inverting `var(--color-amber-100)` so it darkens in light theme; the `isSearch` tome-search branch keeps `#fef3c7` (its background stays hardcoded-dark). `system_notice` text (:528) `#fde047` -> `var(--color-amber-300)`. "THE ORACLE" gold label + amber-600 sources left legible per plan; input `text-amber-50` already inverts correctly.
- **Verification:** Biome check clean on all touched files (formatting auto-fixed; remaining warnings pre-existing). A jsdom computed-contrast assertion was not added — jsdom does not resolve CSS-custom-property cascades or `oklch()`, so a luminance ratio test is not tractable there; relied on the build + diff review against `index.css` var values + next-deploy visual check in both themes (per the plan's "where practical" caveat).
- **Out of scope (logged):** flashcard SRS rating buttons (`rgba(127,29,29)/(146,64,14)` post-flip) and `text-amber-700` decorative micro-labels behave identically to every other var-backed panel app-wide (amber-700 corner glyphs etc.) and were not QA-flagged; the app-wide gold-button `text-amber-950` pattern is a separate cross-cutting concern. Logged to ISSUES for a dedicated follow-up rather than expanding this contrast pass.
