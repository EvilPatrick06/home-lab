...
- **07A (F1)** Centralized the post-import toast in `addTomeToLibrary` (`usePlayerActions.js`): it now fires exactly ONE activation-accurate toast for both branches (sealed + plain) — `Now studying: "<title>"` (success) when it auto-activated (no prior active tome), or the existing `Tome added: "<title>" — switch from the Library when ready.` (info) when a tome was already active — and accepts an optional `opts.detail` suffix. Reduced every import handler in `App.jsx` to error-only, removing the contradictory per-handler success toasts (`handlePasteImport` "Tome inscribed", `handleDeckImport` "Imported N cards into a new tome", `handleOcclusionCreate`, `handleShareCodeImport` "Tome received", the file-drop "Tome inscribed"); the PHASE-04 detail (dropped-quiz count, CSV card count + skipped rows) is now passed through `opts.detail` so it survives in the single toast. The starter-deck add now also gets the consistent centralized toast. Activation rule (`shouldActivate = !prev.activeTomeId`) unchanged; deferred (`setTimeout`, 17C) so it can't double-fire. Tests: `usePlayerActions.test.jsx` +2 (fake-timer assertions: no-active-tome → single "Now studying" success; active-tome → single "switch from the Library" info; neither shows the other).
- **07B (F2)** `ExamMode.jsx`: the <5-riddle gate copy "Regenerate the tome with the updated prompt to populate the deck." → "Add more riddles to this tome — or import a larger deck — to run a practice exam." (creation-agnostic; threshold unchanged). Shipped the generic wording rather than a provenance branch (plan's safe minimum).
- **07C (F3)** `DomainStudyScreen.jsx` (empty domain-weights codex notice) and `FlashcardsMode.jsx` (domain-filter empty state): dropped the AI-only "Regenerate … with the updated prompt …" sentences for creation-agnostic copy. `grep -rn "updated prompt" src` now returns nothing (all three instances cleared). Empty-state conditions unchanged.
- **Verification:** `npx vitest run` green — usePlayerActions.test.jsx (26). `grep "updated prompt"` = none. No test referenced the old toast strings. `npm run lint` exits 0. Full `npm run test` + build gated by CI on push.
