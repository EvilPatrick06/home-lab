...
function setup(opts: { manifest?: unknown }) {
...
describe('prewarmRemoteSounds + resolveSoundUrl', () => {
  it('prefers the Pi URL for a manifest-listed clip after prewarm', async () => {
    setup({ manifest: { version: '1', files: { 'ambient/tavern.mp3': { size: 9 } } } })
  ...
  it('keeps clips NOT in the manifest bundled', async () => {
    setup({ manifest: { version: '1', files: { 'ambient/tavern.mp3': { size: 9 } } } })
    await prewarmRemoteSounds()
  ...
  it('falls back to bundled when the manifest fetch fails (unreachable Pi)', async () => {
    setup({
