...
export default function ExtendedTab({
  ...
  return (
    <div className="space-y-3">
      ...
      {selected && (
        <div className="bg-surface-2/50 border border-border rounded-lg p-3 space-y-3">
          ...
          {selected.requirements && selected.requirements.length > 0 && (
            <div className="text-xs text-gray-500">
              <span className="font-semibold text-muted">{t('game.extendedTab.requirements')}</span>{' '}
          ...
          {selected.costs && (
            <div className="space-y-1">
              <label className="text-xs text-muted font-semibold">{t('game.extendedTab.lifestyle')}</label>
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-0.5 text-xs rounded cursor-pointer ${
                        lifestyleTier === tier ? 'bg-purple-600 text-white' : 'bg-gray-700 text-muted hover:bg-gray-600'
                      }`}
                    >
            ...
            <div className="space-y-1">
              ...
              {checkResults.length === selected.checks.length && !resultText && (
                <button
                  ...
                  className="mt-1 px-3 py-1 text-xs bg-amber-600 hover:bg-accent-strong text-white rounded cursor-pointer"
                >
          ...
          {selected.id === 'research' && selected.dcGuidelines && (
            <div className="space-y-1">
              <label className="text-xs text-muted font-semibold">{t('game.extendedTab.dcGuidelines')}</label>
              <div className="flex flex-wrap gap-1">
          ...
          {resultText && (
            <div className="bg-amber-600/10 border border-amber-500/30 rounded-lg p-2">
              <span className="text-xs text-accent font-semibold">{t('game.extendedTab.result')}</span>
