...
export default function CommandReferenceModal({ isDM, onClose }: CommandReferenceModalProps): JSX.Element {
    ...
    return (
      <div className="mb-4">
        ...
        <div className="space-y-2">
          {cmds.map((cmd) => (
            <div key={cmd.name} className="bg-surface-2/50 rounded-lg p-2.5">
              <div className="flex items-center gap-2 mb-1">
                <span className="font-mono text-sm font-semibold text-accent">/{cmd.name}</span>
              ...
              {(cmd.examples?.length ?? 0) > 0 && (
                <div className="mt-1 flex flex-wrap gap-1">
                  {cmd.examples?.map((ex, i) => (
                    <code key={i} className="text-xs bg-surface/80 px-1.5 py-0.5 rounded text-muted">
                      {ex}
                    </code>
                  ))}
                </div>
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center">
      ...
      <div className="relative bg-surface/95 backdrop-blur-sm border border-border/50 rounded-xl p-5 max-w-lg w-full mx-4 shadow-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-accent">{t('game.commandReferenceModal.title')}</h3>
          <button
            onClick={onClose}
        ...
        <input
          ...
          className="w-full px-3 py-1.5 mb-3 rounded-lg bg-surface-2 border border-border text-fg placeholder-gray-600 focus:outline-none focus:border-amber-500 text-sm"
        />
