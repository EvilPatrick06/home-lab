...
export function SubclassSelector5e({ slot, classId }: { slot: BuildSlot; classId: string }): JSX.Element {
  ...
  // Phase 18d — distinguish "loading" from "loaded but empty" so the Skeleton
  // shows during the fetch and the empty message only after a confirmed empty load.
        ...
        <div>
          ...
          {expanded && (
            <div className="mt-2 max-h-48 overflow-y-auto space-y-1">
              {loading && <Skeleton lines={3} className="py-1" />}
              {!loading &&
                subclasses.map((sc) => (
                  <button
                    key={sc.id}
