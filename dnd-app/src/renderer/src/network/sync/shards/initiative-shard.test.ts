...
describe('initiative-shard (Phase 31f)', () => {
  beforeEach(() => {
    useGameStore.setState({ initiative: null, round: 0, turnMode: 'free' })
  })
  ...
  it('read() returns the live { initiative, round, turnMode } bundle', () => {
    const initiative = makeInitiative([makeEntry('a', 18, true)], 0, 1)
    useGameStore.setState({ initiative, round: 1, turnMode: 'initiative' })
    expect(initiativeShard.read()).toEqual({ initiative, round: 1, turnMode: 'initiative' })
  })
  ...
  it('round-trips combat start (null → active initiative + mode flip)', () => {
    const prev: InitiativeShardValue = { initiative: null, round: 0, turnMode: 'free' }
    const next: InitiativeShardValue = {
  ...
  it('round-trips a turn advance against the real store via applyDelta', () => {
    ...
    useGameStore.setState({ initiative: initial, round: 1, turnMode: 'initiative' })
  ...
  it('round-trips ending combat (active initiative → null, round reset)', () => {
    ...
    const next: InitiativeShardValue = { initiative: null, round: 0, turnMode: 'initiative' }
    const delta = initiativeShard.diff(initiativeShard.read(), next)
    expect(delta).not.toBeNull()
    if (!delta) return
  ...
  it('onChange fires when initiative changes and reports the full bundle', () => {
    ...
    expect(cb).toHaveBeenLastCalledWith({ initiative, round: 1, turnMode: 'initiative' })
  ...
  it('onChange fires when only turnMode or round changes', () => {
    const cb = vi.fn()
    const off = initiativeShard.onChange(cb)
