...
import { resolveBackgroundGear } from '../starting-gear'
...
export async function buildCharacter5e(get: GetState): Promise<Character5e> {
  ...
  // `state.bgEquipment` holds BOTH of the background's options (A and B). Resolve the
  // chosen one via the shared helper (gold "<N> GP" pseudo-items → currency, real
  // gear → inventory) so the built sheet matches the builder's gear-tab display
  // exactly. See stores/builder/starting-gear.ts.
  const bgOptions = state.bgEquipment.length > 0 ? state.bgEquipment : (bgData?.equipment ?? [])
  const { items: bgEquipment, gold: bgGold } = resolveBackgroundGear(bgOptions, bgEquipmentChoice)
  ...
  const { weaponProfs, armorProfs, toolProfs, langProfs } = computeBuilderProficiencies5e({
    classData,
    bgData,
    storeBgEquipment: state.bgEquipment,
