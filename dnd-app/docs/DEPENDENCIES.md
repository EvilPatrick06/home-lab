...
- **vite 8 + @vitejs/plugin-react 6 + electron-vite 6.0.0-beta.1** (adopted 2026-05-30, commit `7d48309`). electron-vite 6 is the first to allow vite 8 (peer `^6||^7||^8`); plugin-react had to go to 6 (peer `vite ^8`). vite 8 defaults to **Rolldown** (`rolldown@1.0.2`) instead of Rollup/esbuild.
  ...
  - **Residual (cosmetic):** the `manualChunks` → `rolldownOptions.output.codeSplitting` migration is done (`electron.vite.config.ts:69-93`); the only open item is a rolldown-compatible successor for `rollup-plugin-visualizer` (`ANALYZE=1` path) — tracked in `dnd-app/docs/phases/PHASE-13-dnd-platform-debt.md` (rolldown-compatible `rollup-plugin-visualizer` successor).
