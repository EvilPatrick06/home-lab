...
Main Menu and every destination off it: **Host/Make Game**, **Join Game**, **Characters** (view list, new character, edit, delete, duplicate, import from D&D Beyond), **Library**, **Bastion**, **Calendar**, **Settings**, **About** (incl. "Check for Updates"), and the **Not Found** / error states. Click the global settings button and any persistent UI (back buttons, breadcrumbs, toasts).
...
2. **Standalone Calendar page** (off the main menu): view/navigate the configured calendar.
...
Every step: **System**, **Details**, **Rules / House Rules** (add/remove), **Session Zero** (tone, Lines, Veils, X-Card toggle, PvP toggle, content limits), **Permissions / player overrides**, **Map config**, **Calendar**, **Seed packs** (browse + apply), **Adventure** import/selector, **AI provider setup** (**local only** — Ollama / llama.cpp; do not configure cloud providers, no creds), **AI image setup** (**local Stable Diffusion only**; skip the OpenAI/Gemini cloud image models — no creds — but still test the fallback dropdown and "test connection" against the local SD URL), **Audio** (upload ambient/effect/music, preview, rename, remove), **Character** step, and the **Review** step. Create the campaign and confirm it persists/opens. **Build at least one fully populated campaign** (real characters, a map, NPCs, journal/handout content) rather than an empty one, and run a session in it — content-heavy state surfaces bugs that a bare campaign won't.
