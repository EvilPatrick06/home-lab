...
- **16b Steps 3-4 (pin interaction + rich form) — DONE 2026-05-29.** Step 4: `modals/dm-tools/PinCreateModal.tsx` (label + 6-icon picker + 8-color palette + visibility toggle + optional linked-journal ID) replaces the old `window.prompt`; `GameLayout` `pinDraftCell` state opens it from EmptyCellContextMenu and saves via `updateMap(pins:[...])`. Step 3: `renderPins` markers are interactive (pointer cursor + `pointertap`) → `MapCanvas` `onPinClick` prop → `GameLayout` opens the linked journal (`sharedJournal` modal) if `linkedJournalId` set, else toasts the label. (NPC/location deep-links + a DOM hover-tooltip beyond the rendered label are noted follow-ups.) **16b COMPLETE.**
- **16b Steps 1-2 (map pins: layer + render) — DONE 2026-05-29.** Step 1: `pinsContainer` Container added to `map-pixi-setup.ts` between tokens and fog (+ test assertions for the layer key + label; 10 specs). Step 2: `components/game/map/pin-layer.ts` `renderPins` draws each pin (colored teardrop + icon glyph + label, labels hidden below 0.5 zoom) into `pinsContainer`; a `MapCanvas` effect (`pinsContainerRef`) renders `map.pins` filtered by `visibleToPlayers` (non-DM) + `currentFloor`. Satisfies "DM-added pin renders; `visibleToPlayers:false` invisible to players." **Remaining (app-verification-heavy):** Step 3 (hover tooltip + click-to-open linked journal/NPC/location — needs interactive sprites + callback plumbing to GameLayout) and Step 4 (upgrade the label-only `EmptyCellContextMenu` prompt into a rich icon/color/visibility/link-picker form). Logged for app-capable follow-up.
- **16f Step 3 (map-switch fade) — DONE 2026-05-29.** `MapCanvas` flips a full-bleed black overlay (always `transition-opacity duration-300`) on `map.id` change — ~600ms dip masking the texture swap; skipped on first mount + when `prefers-reduced-motion: reduce`. **16f COMPLETE** (preload + transition).
- **16f Step 1 (scene preload) — DONE 2026-05-29.** `services/map/preload-adjacent.ts` `collectPreloadImagePaths` (pure, 4 specs: dedupe, cap at 3, skip self/missing/unknown) + a host-only `MapCanvas` effect that `Assets.load`s portal-linked maps' textures so switching has no load stutter. **Remaining:** Step 3 (fade-to-black map-switch transition respecting `prefers-reduced-motion`) — canvas animation, app-verification-heavy.
