...
### Web-build QA addendum (2026-07-02 v2.7.1, from WEB-QA-report-2026-07-02-v2.7.1.md)

Phase 63 was authored by phase-maker from the v2.7.1 WEB-build QA pass — an unattended static + deployed-artifact + live-HTTP-header run (no browser connected; all interactive surfaces carried to the next attended pass). The v2.7.0→v2.7.1 dnd-app delta is web-inert (Electron main-process only), so the run's new findings are serving-infra: **no new Critical/High**; most severe **Medium ×2** — one new (asset cache headers, below), one carried (**PHASE-60**, re-verified still live in deployed v2.7.1 — prioritize, not re-authored). The run also **closed two prior concerns by verification** (the PHASE-61 retention sweep is live and behaving as designed — deployed assets down 1,214 → 756 files within the 24h window — and the SW version-skew concern is cleared: per-version caches, shell-only precache) and recorded a small i18n spot-check as a carry-forward inside **PHASE-62** (same-value es key count 163 → 168; confirm the 5 additions are deliberate). Both new findings live in the same two Pi files and share a root cause (site-wide header policy predates the VTT mount), so they were authored as one phase. Independent of 60/61/62; may be reordered.

- **63** web serving headers — immutable caching for hashed assets + route-scoped VTT CSP (**Medium perf + Low security**, bmo/pi only — no dnd-app rebuild/redeploy): content-hashed `/DungeonTableOnline/assets/**` chunks serve `Cache-Control: no-cache` (`webapp_api.py` `send_from_directory` without `max_age`; `_cache_policy`'s cache branch is `/static/`-only), so every cold boot re-issues ~60 conditional GETs through the tunnel and the Cloudflare edge can't cache — 63A sets `public, max-age=31536000, immutable` on the hashed subtree **only** (stable-named `data/`/`sw.js`/`manifest`/`icons` keep revalidating; the SPA fallback stays `no-cache` by construction via `_cache_policy`'s HTML assign). And VTT HTML inherits the kiosk/IDE CSP (`unsafe-eval`/`unsafe-inline`/IDE-CDN/YouTube allowances the Vite bundle doesn't need — the deployed shell references zero external script/style origins) — 63B sets a VTT-scoped CSP at the `_serve_index()` chokepoint (clean `setdefault` override of the site-wide policy), keeping the runtime-required allowances (pdf.js blob worker, ws/wss + data: connect, CF-insights beacon) and gating the drops on a browser-connected CSP-violation sweep.
