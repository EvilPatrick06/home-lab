...
- **Character Sheet frozen / unresponsive (CRITICAL — non-throwing synchronous infinite loop).** Every saved character's sheet renders, then clicking ANY button freezes the whole app, with **NO console output** (confirmed by the user in 2.4.2 via the now-working F12). That is the decisive clue: **no console = not a React #185 loop** (logs "Maximum update depth"), **not a stack overflow** (logs "Maximum call stack"), **not any thrown error** (logs). It is a **non-throwing synchronous infinite loop** (a `while`/`for` that never terminates, never yields, never allocates enough to throw). It reproduces only with the user's content-rich character — a minimal character full-mounted in `CharacterSheet5ePage.test.tsx` clicks Edit/rests/history without freezing — so the loop is **content-dependent** (some items/feats/spells/refs shape) and on a path the click enables (edit-mode re-render or a modal), not the always-on render path. The v4 migration strips library-resolved content without the content library, so headless tests can't exercise the triggering data. *Action (in flight): (1) loop-hunt workflow scanning effect-resolver / effective-character / spell-data / feat-prereqs / migration / editable sections / rest-resources / calculators for unbounded loops+recursion; (2) ask the user for a DevTools Sources→Pause call stack (or a Performance flame chart) captured DURING the freeze — that names the looping function despite the empty console. Then fix the specific loop.*
