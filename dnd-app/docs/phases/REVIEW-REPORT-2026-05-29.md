...
- **Character Sheet frozen / unresponsive (CRITICAL — needs GUI repro; from 2.4.0 QA).** Every saved (and brand-new) character's sheet renders but ALL controls are dead — Edit / Short Rest / Long Rest / Level Up / Print / section-collapse / Back — with no error boundary; it's the post-save destination, so the core "view my character" flow is unusable. Ruled out headlessly via code review: no `t`-dep-setState loop in the page or its 15 sections (only `FeatPicker`, which is guarded); no setState `#185` loop (those throw → the sheet's ErrorBoundary, which the QA did NOT see); no non-gated overlay (`Modal` returns null when closed, `PrintSheet` is `{showPrint && …}` gated, no full-screen `inset-0` layer); stable `character` ref (the v4 migration runs once on load, not per-render). The `useT` memoization fix landed this session may resolve it if it was a subtle render-loop. *Action: GUI-retest the sheet after the useT fix; if still frozen, capture the devtools console — jsdom can't reproduce a CSS/pointer-events or Chromium-148-runtime cause.*
