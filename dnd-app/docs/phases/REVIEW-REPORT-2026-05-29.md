...
- **🟡 Discord↔VTT relay atomicity — NOT verified (flagged for a focused trace).** The relay path (`bmo/pi/agents/vtt_sync.py` ↔ `bmo/pi/bots/discord_dm_bot.py`) exists; whether it has dedup/transaction guarantees was **not** traced this pass. Do not treat "not atomic" as confirmed — it needs a code read of the relay before asserting.
