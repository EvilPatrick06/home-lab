...
- **Character Sheet frozen / unresponsive (CRITICAL — Chromium-runtime cause; needs devtools console).** Every saved (and brand-new) character's sheet renders but clicking ANY button freezes the whole app — Edit / Short Rest / Long Rest / Level Up / Print / section-collapse / Back — with NO error boundary shown. Still reproduced after the `useT` memoization fix (so that wasn't it). **Now ruled out empirically, not just by review:** a real full-mount happy-dom test (`CharacterSheet5ePage.test.tsx`) renders the actual sheet + sections with a migrated character and clicks Edit / rests / history / edit-toggle — it does NOT loop or throw. A pure JS/React render-loop WOULD reproduce headlessly AND trip the sheet ErrorBoundary / global boundary (neither is seen). Store→store loops ruled out (sheet stores have no write-back subscriptions). Global `DiceOverlay` is `pointer-events-none` (not an overlay-intercept). Conclusion: a real-Chromium-only cause (layout / observer / GPU / content-dependent path) that happy-dom can't simulate — and the v4 migration drops library-resolved content (armor/spells/conditions) without the content library, so the content-heavy render paths can't be exercised headlessly either. **F12 (and Ctrl+Shift+I) now opens devtools in PRODUCTION as of this release.** *Action: install this build, open the Character Sheet, press F12 → Console, click a button to trigger the freeze, and capture the red error(s) / whether CPU pegs. That single readout names the culprit; headless avenues are exhausted on this (display-less) Pi.*
