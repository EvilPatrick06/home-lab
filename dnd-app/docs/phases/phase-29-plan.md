...
> **29e/29f DONE — 2026-05-29 (resumed "do it all").** 29e: gameplay permission gates routed through hasPermission via a new local-permission helper (InGamePage use_dm_tools; PlayerList moderation keys; host per-peer state view keyed on view_hidden_tokens+view_dm_only_stats; host-handlers inbound moderation/journal/trade gated). Transport-role checks (host relay, AI scene prep) intentionally left. 29f: GameLayout view-as-role dropdown + exit banner; hasPermission viewAs mask. All 4 gates green.
