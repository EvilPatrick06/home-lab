...
- **06A (2026-06-11):** `shared/constants.ts` `SCENE_PREP_PROMPT`; `ipc-channels.ts` `AI_CANCEL_SCENE`. `conversation-manager.ts` — `removeTrailingUserMessage(content)` + `clearScenePrepExchange(prompt)` (only touches a pure prep exchange). `ai-service.ts` — `prepareScene` request uses `SCENE_PREP_PROMPT`; new exported `cancelScenePrep(campaignId)` (cancelChat the stream, `scenePrepStatus.delete`, trim prep exchange via `conversations.get`, save if changed; idempotent). `ai-handlers.ts` — `AI_CANCEL_SCENE` handler delegating to `cancelScenePrep(sanitizeCampaignId(id))`. `preload/index.ts` + `index.d.ts` — `ai.cancelScene`. Tests: conversation-manager (remove/clear matrix), ai-service (cancel unknown→success; prepare→preparing→cancel→idle), ai-handlers (register + valid-UUID delegate + non-UUID error; fixed mock `getPath` to absolute `/tmp` for Linux path-resolve stability). tsc node clean; 84 tests green across 3 files.
- **06B (2026-06-11):** `use-ai-dm-store.ts` — new `sceneStreamId: string | null` state (init null, SEPARATE from `activeStreamId`); `prepareScene` captures `result.streamId`; `checkSceneStatus` sets it; new `cancelScenePrep(campaignId)` action (resets `sceneStatus`/`sceneError`/`sceneStreamId` first, then `window.api.ai.cancelScene`); `reset()` clears it. `ScenePrepPage.tsx handleCancel` calls `cancelScenePrep` (drops the renderer-only `cancelStream` no-op + redundant setState). Store test: stub gains `cancelScene` + `prepareScene→{streamId:'scene-1'}`; new "scene-prep cancel wiring (06B)" describe (prepareScene stores id, cancelScenePrep invokes IPC + resets, reset clears id). tsc web clean; 45 store tests green.
- **06C (2026-06-11):** `ai-service.ts prepareScene` — replaced `conv.clear()` (wiped messages AND summaries) with `conv.removeTrailingUserMessage(SCENE_PREP_PROMPT)` on an error-status retry, so only the dangling prep prompt is dropped; a populated post-failure conversation survives and short-circuits to 'ready'. `ai-service.test.ts` — added a hoisted, per-test-configurable provider `streamChat` mock (a "timed out" error fails fast via streamWithRetry's no-retry-on-timeout path → deterministic 'error' status); retry matrix tests (fresh failed→trims→regenerates 'preparing'; populated→preserves 4 msgs→'ready'). `conv.clear()` no longer appears in prepareScene. tsc node clean; 45 tests green.
- **06D (2026-06-11):** `ai-service.ts startChat` binds `const contextBlock = providerContext + context` and passes it to `getMessagesForApi`; `handleStreamCompletion` gains a `contextBlock: string` param (before `fileReadDepth`, defaulted `deps` last) threaded at both call sites (startChat onDone + recursive restream onDone); `restreamConversation` replays `getMessagesForApi(contextBlock)` (was `('')`). FILE_READ/WEB_SEARCH continuations now keep the original game-state/character/rules block and the combat-mode gate. New `ai-service-restream-context.test.ts` (buildContext→sentinel; asserts both the original and the FILE_READ-continuation `getMessagesForApi` calls carry the sentinel). `getMessagesForApi('')` gone. Web-search-approval + file-read-cancel suites re-run green (param threaded). tsc node clean; 7 tests green across 3 files.
- **06E (2026-06-11):** `app-constants.ts` — renamed `SCENE_POLL_TIMEOUT_MS` → `SCENE_POLL_SLOW_NOTICE_MS` (now a one-time-notice threshold, not a poll kill switch). `use-game-effects.ts` — the in-game poll callback is wrapped in try/catch (a transient IPC failure no longer kills it or leaks an unhandled rejection); added an `'idle'` branch (clears interval + sets `isTyping:false, sceneStatus:'idle'` — prep cancelled elsewhere); the abandon-cap is replaced by a soft "still setting the scene" chat notice that does NOT clear the interval (poll continues until ready/error/idle/unmount, so `isTyping` is never stranded and a late-finishing scene still posts). `use-game-effects.test.ts` — fake-timer poll tests (slow-notice posts once + polling continues to `ready`; `idle` stops + clears isTyping). No `SCENE_POLL_TIMEOUT_MS` references remain. tsc web clean; 6 tests green.
- **06F (2026-06-11):** i18n `pages.lobbyPage.prepCampaignNotFound` + `prepBackToMenu` (en+es) + regenerated key union. `ScenePrepPage.tsx` — subscribes `useCampaignStore.loading`; a mount effect calls `loadCampaigns()` (self-heals a deep link, mirrors LobbyPage/InGamePage); the `!campaign` branch now shows a spinner while loading vs an accurate "Campaign not found" + back-to-menu Button (navigates `/`) when genuinely absent (was the wrong "Scene prep failed" dead end). New `ScenePrepPage.test.tsx` (happy-dom RTL: not-found+back, loading-spinner, known-campaign Cancel→cancelScene IPC). locale-parity + generated-keys gates green. tsc web clean; 6 tests green across 3 files.
