...
- 48A — DONE (2026-06-23) (`components/builder/5e/builder-dirty.ts` + `.test.ts`, `CharacterBuilder5e.tsx`) — root cause: `handleBack` only showed the existing leave-confirmation when `editingCharacterId` was set; a NEW build was `resetBuilder()`+navigated away silently. Added `builderHasUnsavedContent(characterName, buildSlots)` and gated Back on `editingCharacterId || builderHasUnsavedContent(...)`, reusing the existing leave-without-saving Modal + i18n. A pristine builder still exits unprompted. (The builder already autosaves a resumable draft, so this adds the missing confirmation on top.) Test: pristine -> false, name entered -> true, any slot selected -> true.
- 48B — DONE (2026-06-23) (`components/builder/5e/SpellSummary5e.tsx`, `SpellPicker5e.tsx` + `.test.tsx`) — `SpellRow` wrapped in `React.memo` with a stable `onToggle(id)` contract (was a per-row closure that defeated memo); `SpellPicker5e` passes the stable store action directly and `useMemo`s the `filtered` + `spellLevels` computations. A checkbox toggle in the long level-10 Wizard list now re-renders only the changed row, not all ~200, removing the recompute/re-render storm behind the freeze. Summary-row call sites pass a no-op arrow, assignable to the new contract (unchanged). Test: SpellPicker lists spells + a checkbox click calls toggleSpell with the spell id.
- 48C — DONE (2026-06-23) (`services/keyboard-shortcuts.test.ts`, new `keyboard-shortcuts-data.test.ts`) — VERIFIED the conflict/swap machinery already works: `hasConflict` flags rebinding to an in-use key (mechanism test on the mocked set), and a data test against the real `keyboard-shortcuts.json` pins `toggle-journal`='j' / `open-dice`='t' as distinct in-use keys so the QA T-rebind WOULD fire the swap modal (the QA likely hit a pre-fix build or the harness didn't confirm the modal). **F5 was a plan misread:** the two `c` defaults are `c` (focus-chat) and `Shift+C` (center-on-me) — distinct combos, NOT a duplicate; added a data test asserting no two defaults share an identical key+modifier combo. No JSON change needed.
- 48D — DONE (2026-06-23) (`pages/bastion/CreateBastionModal.tsx` + `.test.tsx`, i18n en/es) — when there are zero characters the modal renders an empty-state hint + Create-a-character CTA (navigates to `/characters/5e/create`, closes the modal) instead of a dead-end disabled Create over an empty owner dropdown; the owner requirement (DMG-correct) is unchanged when characters exist. Test: zero characters -> CTA + no dropdown; with characters -> dropdown + no CTA.
