...
- **20H (2026-06-16):** docs + env truth. `bmo/docs/SERVICES.md`: the four `/api/discord/dm/*` routes documented as proxies to the bot's loopback control server (5006, `DM_BOT_CONTROL_PORT`); 503 now truthfully = bot process down; no systemd change (control server inside the existing `bmo-dm-bot` unit) + restart-both note. `bmo/pi/bots/README.md`: env table (`DISCORD_DM_VOICE_CHANNEL[_ID]`, `DM_BOT_CONTROL_PORT`), narrate result vocabulary (`queued`/`duplicate`/`no_voice`/`dropped_queue_full`, always-200 rationale), session-end reasons. `docs/ARCHITECTURE.md`: VTT↔BMO section updated with the proxy/control-plane + event_id + result-field + status-fields contract (sync plane deferred to PHASE-22). Cheap check: `grep DM_BOT_CONTROL_PORT .env.template SERVICES.md` present. **⚠ USER ACTION (post-pull):** restart **both** `bmo` and `bmo-dm-bot` so the proxy + control server are live.
- **20G (2026-06-16):** in-app Discord session start/stop/status UI (F5). New `DiscordSessionSection.tsx` (`campaignId` + `narrationEnabled` props): polls `bmoDmStatus` on mount + every 20s, maps `{ok:false}`/`running:false`→botDown, `active:false`→idle, else active; status dot + voice-mic indicator + player list; Start (`bmoStartDm`, structured `channel_not_found`/`join_failed`/`session_active` errors) / Stop (`bmoStopDm`, dismissible recap block); "auto-ended" line when `last_session_end.reason === 'auto_leave_empty'`; "narration on but no session" hint. Rendered in `DMTabPanel` next to the Speak-narration toggle. i18n `game.discordSession.*` keys added to en.json + es.json + `i18n:gen-keys` regenerated (6116 keys). Colocated test (idle/start-wiring/start-error/active+recap/botDown). Cheap check: tsc web clean; `vitest DiscordSessionSection.test.tsx` → 5 passed.
- **20F (2026-06-16):** single toggle-gated narration sender + eventId + honest results + 4xx fix (F2/F4/F6). `bmo-bridge.ts`: 4xx now resets `consecutiveBmoFailures` and returns (counter only increments on network/5xx — F6); `sendNarration` adds `event_id: randomUUID()` (node:crypto) (F4); module-level `narrationEnabled` gate + `setNarrationEnabled`/`isNarrationEnabled` (default false). `ai-service.ts`: the lone auto `sendNarration` is wrapped in `isNarrationEnabled()` and a dropped/non-ok result is `broadcastNarrationStatus`'d to all windows (validated via new `BmoNarrationStatusSchema`). New channels `BMO_SET_NARRATION_ENABLED` (renderer→main) + `BMO_NARRATION_STATUS` (main→renderer); `ai-handlers` handler validates + calls `setNarrationEnabled`. Preload: `bmoSetNarrationEnabled` + `onBmoNarrationStatus` (wrapped listener + exact unsubscribe, PHASE-05 pattern); `index.d.ts` `bmoDmStatus` retyped to the truthful BridgeResponse union + the two new entries. `use-narration-tts-store` syncs main on set + at init (window-guarded). `use-game-effects`: deleted both renderer auto-send call sites + the `narrateThroughBmo` callback + the store-read + unused imports (`speakNarrationThroughBmo`/`useNarrationTtsStore`/`useCallback`); added a `onBmoNarrationStatus` effect that surfaces failures as a deduped DM alert (60s/changed-reason; 404 → "No active Discord session"). Tests: bmo-bridge (4xx-no-unreachable, 3-network-fail→unreachable, narrate event_id) + store (sync on set + init) + bmo-narration unchanged. Cheap check: tsc web+node clean; `vitest bmo-bridge/use-narration-tts-store/bmo-narration` → 27 passed.
- **20E (2026-06-16):** `/initiative` actually tracks (F12). `DMSession` gained `initiative_collecting` (reset cleared). New module helpers `_upsert_initiative(session,name,total)` (replace-or-append + sort desc) and `_format_initiative_order(order)` ("1. Name — 17" / "No rolls yet"). `_roll_cmd` records a d20 roll into the order when `active and initiative_collecting`, before building the embed, and shows the current standings in an embed field. `/initiative` gained an `action` choice (`start` default opens a fresh round + sets collecting; `show` renders the order; `end` stops collecting). `/control/status` already serializes `initiative_order`. Tests: upsert sort + per-player replace + format rendering. Cheap check: `pytest tests/test_dm_bot_control.py tests/test_dm_bot_voice.py` → 42 passed; py_compile clean.
- **20D (2026-06-16):** voice-health loop + auto-leave parity (F10/F11). New `_voice_health_tick` (active + dropped → force-disconnect the stale client, null it, `start_voice_listen`; returns connected-ness) + `_voice_health_loop` (20s, backs off to 60s after 3 failed rejoins; started in `setup_hook`, cancelled in `close()`). Reworked `_auto_leave_if_empty` to parity with stop: `_stopping` guard, bounded `wait_for(_generate_recap,10)` (fallback ""), `_campaign_memory.end_session`, clear `_campaign_name`/`_session_id`, set `_last_session_end={"reason":"auto_leave_empty",…}` (surfaced via `/control/status`), then leave + reset. Tests: tick rejoins on drop (force-disconnect + start_voice_listen) + no-op without a session; auto-leave ends session with recap/memory-close/trace; `_stopping` skips auto-leave. Cheap check: `pytest tests/test_dm_bot_control.py` → 24 passed.
- **20C (2026-06-16):** control plane — aiohttp server in the bot process + app.py proxy (F1/F4/F7/F8). New `bots/dm_bot_control.py`: `build_control_app(bot)` (testable) + `start_control_server(bot)` (127.0.0.1:`DM_BOT_CONTROL_PORT`=5006). Routes: `/control/start` (409 session_active / 404 channel_not_found / 502 join_failed+reason / else full slash parity incl. cleared initiative + `text_channel_id`=VC id + campaign memory + queued greeting), `/control/stop` (`_stopping` idempotency → already_stopping; bounded recap `wait_for(_generate_recap,10)`; **direct** bounded farewell synth+play since `reset()` drains the queue; `_last_session_end`), `/control/narrate` (bounded insertion-ordered event_id dedup cap 500 → `duplicate`; else `queue_narration`; always HTTP 200 so the VTT never retries a drop), `/control/status` (voice_connected/queue_len/initiative_order/last_narration_status/last_session_end). `app.py`: the four `/api/discord/dm/*` routes are now `_proxy_to_dm_control(...)` (local `import requests`, `(2,12)` timeouts, relays status+JSON, ConnectionError/Timeout→503 "DM bot not running") — `get_dm_bot`/`run_coroutine_threadsafe` gone; v1 aliases + `@limiter.limit(RATE_LIMIT_NARRATE)` kept; empty-text narrate 400 short-circuits before the hop. Bot `setup_hook` starts the control server; `close()` cleans it up; slash `dm_stop` now shares `_stopping` + bounded direct farewell. Tests: control app (start parity/errors, narrate duplicate+no_voice, stop idempotent, status shape) via aiohttp TestClient; app.py proxy (forward + GET + 503-on-ConnectionError + narrate-400-skips-proxy). Cheap check: `pytest tests/test_dm_bot_control.py tests/test_app_endpoints.py` → 60 passed; `ast.parse(app.py)` clean.
- **20B (2026-06-16):** honest `_speak` + narration queue (F3/F4). `DMSession` gained `narration_queue: asyncio.Queue(maxsize=20)` + `last_narration_status`; `reset()` drains the queue. Split `_speak`: `queue_narration(text,npc,emotion)` (sync — `'no_voice'`/`'dropped_queue_full'`/`'queued'`, never a silent drop), `_narration_worker` (single FIFO consumer that *waits out* the TTS cooldown instead of skipping, sets `last_narration_status`, never dies), `_synthesize_and_play` (`'spoken'`/`'tts_error'`/`'play_error'`/`'no_voice'`; keeps the `text[:500]` truncation for PHASE-21), `_play_audio` now returns bool. `_speak` kept as a thin wrapper → `queue_narration` (greeting/farewell/initiative/player-input call sites unchanged). `DMBot.__init__` got `_narration_task`/`_control_runner`/`_voice_health_task`/`_stopping`/`_last_session_end`; `setup_hook` starts the worker (+ forward refs to 20C control server / 20D health loop) and a `close()` override cancels them. Tests: queue statuses (no_voice/queued/dropped_queue_full), FIFO worker order, cooldown-waits-not-drops. Cheap check: `pytest tests/test_dm_bot_control.py` → 12 passed.
- **20A (2026-06-16):** bot hygiene (F8/F9). `_log` (discord_dm_bot.py:63) now accepts `exc_info` and appends a formatted traceback (+ `import traceback`) so `on_app_command_error`'s `_log(..., exc_info=error)` no longer self-crashes. `DUNGEON_CHANNEL_NAME` is now `os.environ.get("DISCORD_DM_VOICE_CHANNEL", …)` + new `DUNGEON_CHANNEL_ID` (`DISCORD_DM_VOICE_CHANNEL_ID`); `find_dungeon_channel` honors the numeric ID (when it resolves to a VoiceChannel) before the name scan. `join_voice` returns `(vc, None)` / `(None, "Type: msg")`; slash `dm_start` surfaces the reason, `start_voice_listen` logs it. New module-level `_candidate_guilds(bot)` (scopes to `_guild_id` when set, else all guilds); `start_voice_listen`'s guild loop uses it. `.env.template`: added `DISCORD_DM_VOICE_CHANNEL`/`_ID` + `DM_BOT_CONTROL_PORT`. New `tests/test_dm_bot_control.py` (exc_info no-raise, ID override + name fallback, join reason on failure + vc on success, `_candidate_guilds`); `test_dm_bot_voice.py` join mock updated to the `(vc, reason)` tuple. Cheap check: `pytest tests/test_dm_bot_voice.py tests/test_dm_bot_control.py` → 22 passed.
