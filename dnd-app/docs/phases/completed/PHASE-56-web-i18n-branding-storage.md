...
- 56A — DONE (`src/renderer/src/i18n/index.ts` + `i18n/document-locale.test.ts`) — exported `applyDocumentLocale(locale)` (sets `document.documentElement.lang` + `dir` via a `dirFor` map; no-op outside a DOM), registered `i18n.on('languageChanged', applyDocumentLocale)` at module load (active even when `initI18n` early-returns), and set the initial lang/dir after `i18n.init`. `<html lang>`/`dir` now track the active locale on every change path including the App's post-settings switch. Test (happy-dom env): direct `applyDocumentLocale` + a `changeLanguage` round-trip update `document.documentElement.lang`.
- 56B — DONE (`components/settings/AudioSection.tsx`, `pages/campaign-detail/MapManager.tsx`) — added `accent-amber-500` to the `master-volume` + `ambient-volume` Audio sliders and the `grid-opacity` slider in MapManager; swept `DMAudioPanel.tsx` + `GridControlPanel.tsx` and confirmed they already carry the themed accent. Every range input now uses the amber accent.
- 56C — DONE (owner-decision: chose the non-destructive cross-reference, NOT a public-name rename) (`pages/AboutPage.tsx` + en/es) — added a web-only line under the version: "Dungeon Table Online is the web edition of D&D Virtual Tabletop." (i18n key `pages.aboutPage.webEditionNote`, en+es, gated by `isWebBuild()` so desktop is unchanged). Generated-keys regenerated. Rationale: keeps the established public name while cross-referencing the in-app brand, the lowest-risk of the plan's two options.
- 56D — DONE (scoped to the QA-flagged set; broader set logged) (`constants/settings-keys.ts`, `stores/use-lobby-store.ts`, new `utils/storage-migrations.ts` + `utils/bootstrap-storage.ts`, `main.tsx`) — namespaced `library-recent` → `dnd-vtt-library-recent`, `lobby-chat-*` → `dnd-vtt-lobby-chat-*` (centralized on `dynamicKeys.lobbyChat`, dropped the hardcoded `CHAT_HISTORY_KEY_PREFIX`), and `lobby-dice-colors` → `dnd-vtt-lobby-dice-colors` (new `SETTINGS_KEYS.LOBBY_DICE_COLORS`). Added a one-time read-side migration (old→new copy + delete, idempotent, never clobbers an existing new key, best-effort/never-throws) run as the FIRST import in `main.tsx` (before any store evaluates its localStorage-reading initializer). Tests: 6 migration cases. The remaining un-prefixed keys (`library-favorites`, `dice-tray-position`, `narration-tts-enabled`, `encounter-presets`, `notification-config`, `macro-storage-*`, `builder-draft-*`, `autosave:*`) + a `DiceTray.tsx` key inconsistency are logged to `docs/logs/SUGGESTIONS-LOG-DNDAPP.md` for a follow-up sweep (scattered hardcoded sites + an `encounter-presets` content-type-id name clash make a blanket rename risky in a low-priority polish phase).
- 56E — DEFERRED to live WEB-QA (verification-gated, no running build) — the Español main-menu + character-card walk cannot be exercised by the automated executor (no deployed build) and the plan forbids guesswork; no code change. Logged to SUGGESTIONS for the next WEB-QA pass to drive on `https://bmo.mybmoai.work/DungeonTableOnline/`.
