...
- **22A (2026-06-16):** `vtt_sync` contract + config truth (F2/F3/F6/F7). `agents/vtt_sync.py`: `VTT_SYNC_URL` no longer falls back to `vtt.local` — empty = disabled (`sync_enabled()`, `_log_disabled_once`, `_post_to_vtt`/`request_vtt_state`/`check_vtt_health` no-op without spawning retry threads); new `VTT_SYNC_TOKEN` (falls back to `BMO_API_KEY`) → `_auth_headers()` bearer on every POST/GET; `last_push` status + `validate_sync_config()`; payload keys aligned to the VTT zod contract (`discord_message {text,author}`, `discord_roll {formula,total,rollerName,rolls?}`, join/leave `{playerName}`); deleted `register_sync_routes`/`_parse_json` (kept `VTTState`/`vtt_state`/`format_initiative_embed`); dropped the unused `json` import + rewrote the stale docstring. `dnd_dm.py`: `_parse_gamestate` now runs BEFORE `push_discord_message` so raw ```gamestate``` never ships. Deleted `scripts/apply_patch.py` (+ README line). `.env.template`: `VTT_SYNC_URL=""` + `VTT_SYNC_TOKEN`. Tests: bearer on/off, URL-unset no-op, 4 payload shapes, `last_push`, `validate_sync_config` (existing retry/eventId stay green). Cheap check: `pytest tests/agents/test_vtt_sync.py` → 11 passed.
- **22B (2026-06-16):** VTT→Pi state through the control plane + `vtt_state` consumption (F2/F7). `dm_bot_control.py`: new `POST /control/sync/{initiative,state}` (write `vtt_state`; initiative schedules `post_initiative_embed` only when session active + text channel); `/control/status` gains a read-only `vtt_sync` block (`validate_sync_config` + `last_push` + has_initiative/has_game_state/state_age_s, no probe); `/control/start` fires `asyncio.to_thread(request_vtt_state)`. `app.py`: two Flask proxies → `/control/sync/*` (503 on bot-down). `discord_dm_bot.py`: `post_initiative_embed` (edit ONE tracker message under a lock + ≥1s spacing; `_initiative_message` cleared on every session-end path); `_vtt_state_context()` injects a fresh (<10min) `[VTT STATE] map=…; party: …` line into `_handle_player_input`; `setup_hook` logs `validate_sync_config()`. Tests: sync routes update state, embed gating, send-then-edit, status block non-blocking, context freshness; app-proxy forward + 503. Cheap check: `pytest test_dm_bot_control test_app_endpoints test_vtt_sync` → 94 passed.
- **22C (2026-06-16):** bot event push wiring (F5). `discord_dm_bot.py`: imported the four push helpers + a `_try_push` wrapper (swallows errors so sync can never break the bot). Call sites: `on_message` (player text), `_handle_player_input` (DM reply `clean_text[:2000]`), `_roll_cmd` (`push_discord_roll(..., rolls=rolls)` in the active-session block), `on_voice_state_update` (join/leave), and both session-end paths (`'BMO'` informational message, no new event type). Tests: spies across message/reply/roll/join/leave; no-push when inactive; raising-helper swallowed. Cheap check: `pytest test_dm_bot_control test_dm_bot_voice` → 73 passed.
- **22D (2026-06-16):** dnd-app channel split + schema + preload + key/bind (F4/F6). `ipc-channels.ts`: new `BMO_SYNC_INITIATIVE_EVENT` (main→renderer); `bmo-bridge.ts` forwards initiative on it (was double-using the invoke channel). `ipc-schemas.ts`: 7th union member `bmo_unreachable`; `notifyBmoUnreachable` rewritten to emit it (no "paused" lie). `bmo-bridge.ts`: `applySyncBindFromSettings` (env wins → keyed-opt-in `0.0.0.0` → loopback; returns the host; restarts the receiver on change). `settings-storage.ts`: `bmoApiKey` (encrypted at rest like turn creds) + `bmoSyncLanEnabled`. `index.ts` + `storage-handlers.ts`: wired `applyBmoApiKeyFromSettings` + `applySyncBindFromSettings` at startup + save (the previously-dead settings key path). Preload + d.ts: `bmoSyncInitiative`/`bmoSyncGameState` invokes + `onBmoSyncEvent`/`onBmoSyncInitiative` (PHASE-05 wrapped-listener + unsubscribe). Tests: new-channel forward, `bmo_unreachable` schema-valid, bind precedence, Pi-shaped payloads parse. Cheap check: tsc node+web clean; `vitest bmo-bridge ipc-schemas bmo-sync-handlers` → 63 passed.
- **22E (2026-06-16):** renderer sync consumption + opt-in push (F1/F4). New `use-discord-sync-store.ts` (bounded-100 newest-first activity FIFO + persisted `syncToDiscordEnabled` default false) + `use-discord-sync.ts` (DM-only; inbound events → activity feed + truthful join/leave/unreachable alerts (60s dedup) — DISPLAY-ONLY, never mutates game state; `state_request` answered regardless of toggle; outbound opt-in `useGameStore.subscribe` + prev-diff + debounce 1500/3000ms push of initiative/condensed state). Mounted in `GameLayout.tsx` next to `useGameEffects`. `DiscordSessionSection.tsx` gained the sync toggle + collapsible activity list. i18n `game.discordSync.*` + `notify.discordSync.*` en+es; keys regenerated. Tests: store FIFO/persistence + hook inbound mapping/alert-dedup/unsubscribe/state_request/non-DM-inert/debounced-push. Cheap check: tsc web clean; `vitest` store+hook+section → green.
- **22F (2026-06-16):** "Push to Discord" TEXT into the AI pipeline (F8). `discord-service.ts` `cleanTextForDiscord`: now also strips `[NPC:]`/`[EMOTION:]`/`[SPEAKER:]`/`[RULING…][/RULING]`. `ai-service.ts` finalize: a fire-and-forget `sendNarrationToDiscord(displayText, campaign.name)` (via `loadCampaignById`) beside the voice sender — gated by the Discord integration's own `config.enabled` (default OFF, no-ops otherwise); rewrote the stale "NO Discord TEXT" comment. Tests: tag-strip additions. Cheap check: tsc node clean; `vitest discord-service.test.ts` → 20 passed.
- **22G (2026-06-16):** docs + protocol truth. `docs/ARCHITECTURE.md` (full bidirectional sync plane + `bmo_unreachable` + eventId/bearer + loopback-default/keyed-LAN bind + deletions), `bmo/docs/SERVICES.md` (two sync routes + `vtt_sync` status block + `VTT_SYNC_URL` unset = disabled), `bmo/pi/bots/README.md` (env table `VTT_SYNC_URL`/`VTT_SYNC_TOKEN` + push-event vocabulary + initiative-embed behavior), `dnd-app/README.md` (receiver bearer/bind settings + new renderer channels). `grep register_sync_routes|apply_patch` → only this plan's history. **⚠ USER ACTION (post-pull):** restart **both** `bmo` and `bmo-dm-bot`.
