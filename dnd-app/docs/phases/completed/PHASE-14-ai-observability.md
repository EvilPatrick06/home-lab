...
- **14E (2026-06-16):** Context-inspector panel. New `AI_GET_CONTEXT_INSPECTOR` channel + handler (`ai-handlers.ts`, `sanitizeCampaignId` from day one) composing `{ breakdown: getLastTokenBreakdown(campaignId), contextTruncated, lastTokenEstimate, contextWindow: getActiveContextWindow(), conversationBudget: getEffectiveBudgets().conversationHistory, chunkIds }` — 01C exports present so no fallback needed. `ai-service.ts` — new read-only `getLastAssistantContextChunkIds(campaignId)` (`conversations.get`, never instantiates — CQS). preload `getContextInspector` + `index.d.ts` typing. New `ContextInspectorPanel.tsx` (in `components/game/bottom/`) absorbs DMTabPanel's token block: reuses `game.dmTabPanel.budget*`/`totalContext` row keys verbatim + adds model-window (amber when total>window), last-request estimate (0→em-dash), truthful truncation line (`role="status"`), and a `<details>` chunk-id provenance list; preview fallback (`previewTokenBudget`) with a note when there's no live build; refresh on mount + on the falling edge of `isTyping`. `DMTabPanel.tsx` — deleted the token state/`refreshTokenBudget`/both effects/`wasTypingRef` + the button+detail JSX (dropped unused `useCallback`/`useEffect`/`useRef` imports), lazy-mounts `ContextInspectorPanel` (passes `campaignId` + filtered `characterIds`); kept `AiContextPanel`. New `game.contextInspector.*` i18n (en+es). Tests: `ContextInspectorPanel.test.tsx` 5 cases (live snapshot rows + window/estimate, no preview note; `breakdown:null`→preview note + `previewTokenBudget(campaignId, ids)`; truncation warning in `role=status`; chunk-id list w/ count; empty→chunksNone); `DMTabPanel.test.tsx` mocks updated (`getContextInspector` stub + `players:[]` fixture) and AI-DM tab still renders the model label; `ai-handlers.test.ts` inspector composition (real token-budget numbers + mocked getters/chunkIds) + path-traversal `{success:false}`. tsc web+node clean; 29 tests green; keys regenerated.
- **14D (2026-06-16):** File-read `'done'` emission + "Reading file…" indicator. Post-04 clearing gate verified (9 `fileReadStatus: null` writers ≥ 5). `ai-service.ts` FILE_READ branch — after `readRequestedFile` resolves + the abort check + the synthetic-message appends, emits a second `AI_STREAM_FILE_READ` with `status:'done'` (same captured `win`/`streamId`/`path`) BEFORE `restreamConversation()`, so the label reverts when the read finishes instead of persisting through the restream (an emit, NOT store clearing — clearing stays PHASE-04's). `ChatPanel.tsx` — reads `s.fileReadStatus`; the typing-indicator label is now file-read (basename via `path.split(/[\\/]/).pop()`, full path in `title`) → cold-model-load → generic. i18n `game.chatPanel.aiReadingFile` (en+es). Tests: `ai-service-file-read-cancel.test.ts` asserts a completed read emits `reading` then `done` (ordered, same path/streamId); new `ChatPanel.test.tsx` render (happy-dom, `useAiReadiness` mocked) shows "reading intro.md" while reading + the generic label after flipping to `done`. tsc node+web clean; 6 tests green; keys regenerated.
- **14C (2026-06-16):** Truncation alert wiring — `wasContextTruncated`/`getLastTokenEstimate` finally get production callers. `ai-handlers.ts` — the `AI_CHAT_STREAM` `onDone` lambda appends `contextTruncated: aiService.wasContextTruncated(parsed.data.campaignId)` + `tokenEstimate: aiService.getLastTokenEstimate(parsed.data.campaignId)` to the `AI_STREAM_DONE` payload (additive; `startChat`/`handleStreamCompletion` signatures untouched). `index.d.ts` `AiStreamDoneData` gains the two optional fields. `use-ai-dm-store.ts` — `lastContextTruncated`/`lastTokenEstimate` state (init `false`/`null`); `handleDone` fires a RISING-EDGE `pushDmAlert('warning', …)` (truncated && !prev) so a permanently-over-budget campaign alerts once, and records the fields; cleared in `reset()` + both `initFromCampaign` branches. `AiDmStatusBar.tsx` — amber `✂` "Context trimmed last turn" chip (with explanatory `title`) in the same `role="status"` region; `ChatPanel` passes `contextTruncated={s.lastContextTruncated}`. i18n `notify.aiDmStore.contextTruncated` + `game.aiStatusBar.contextTrimmed`/`contextTrimmedTitle` (en+es). Tests: store rising-edge fires once + clears + reset clears (DmAlertTray mocked); `ai-handlers.test.ts` drives `AI_CHAT_STREAM` (captured onDone fired post-startChat to dodge the `streamId` TDZ; fake window with `isDestroyed`) asserting payload `{contextTruncated:true,tokenEstimate:4242}` + getters called with the campaignId; `AiDmStatusBar.test.tsx` chip renders/doesn't. tsc node+web clean; 84 tests green; keys regenerated.
- **14B (2026-06-16):** Connection state in the AI store + badge in `AiDmStatusBar`. `use-ai-dm-store.ts` — new `connectionStatus: 'connected'|'degraded'|'disconnected'|null` state (init `null`, NOT cleared on reset/initFromCampaign — it's the provider link, not the campaign); `setupListeners` registers `onConnectionStatus` (no streamId gate — global health), pushes its unsub into the post-05 `unsubscribes` array, and seeds via `getConnectionStatus().then(...)`. `AiDmStatusBar.tsx` — new optional `connection` prop renders an amber `▲` degraded chip / red `■` disconnected chip (shape+border+color+label) inside a `role="status"` region; connected/null render nothing extra. `ChatPanel.tsx` reads `s.connectionStatus` and passes `connection`. i18n `game.aiStatusBar.connDegraded/connDisconnected` (en+es). Tests: store (event sets status, cleanup unsubscribes, seed sets `connected`); `AiDmStatusBar.test.tsx` (degraded in role=status, disconnected, none when connected). tsc web clean; 62 tests green; keys regenerated.
- **14A (2026-06-16):** Connection-status invoke handler + push-on-transition. Prereq gates verified live: `getConnectionStatus`/`getConsecutiveFailures` at `ai-service.ts:216/222`, `AI_CONNECTION_STATUS` channel at `ipc-channels.ts:121`, removed-handler comment + `_AiConnectionStatus` alias present. New `AI_CONNECTION_STATUS_CHANGED` channel (`ipc-channels.ts`). `ai-service.ts` — `lastEmittedConnectionStatus` + `notifyConnectionStatusChanged()` (best-effort `BrowserWindow.getAllWindows()` broadcast, transition-gated) called after BOTH `consecutiveFailures = 0` and `consecutiveFailures++` in `streamWithRetry`. `ai-handlers.ts` — replaced the Phase-17d removal comment with the `AI_CONNECTION_STATUS` invoke handler (`{ status, consecutiveFailures }`); deleted the now-genuinely-used `_AiConnectionStatus` keep-alive alias (kept `_StreamResult`). `preload/index.ts` — `getConnectionStatus` invoke + `onConnectionStatus` named-listener (post-05 unsub) + added to `removeAllAiListeners`; `index.d.ts` typings. Tests: `ai-service.test.ts` drives `streamWithRetry` (normalize via success, then 3 non-timeout failures → `degraded`(cf 1)→`disconnected`(cf 3), then success→`connected`, 2nd success→no event) with `BrowserWindow.getAllWindows` overridden to a fake `webContents.send` spy; `ai-handlers.test.ts` asserts the invoke handler returns the mocked `{status,consecutiveFailures}`. tsc node+web clean; 66 tests green.
