...
- **12E (2026-06-11):** `spell-effect-actions.ts` `executeEndSpell` — end-spell chat line `🛑 ${name} (${caster}) ends.` → `🛑 Spell ends: ${name} (cast by ${caster}).` (explicit event + caster attribution; stays hardcoded English by executor convention). Test asserts the new content + `end-spell` id prefix (Entangle/Druid fixture). Old `(${caster}) ends.` pattern gone. tsc web clean; 11 tests green.
- **12D (2026-06-11):** i18n-ized the AI-DM store's hardcoded English. New `notify.aiDmStore` keys (en+es): `responseTimedOut` (en byte-identical to the old literal), `scenePrepFailed`, `dmOverrideRejected`, `dmOverrideRejectedWithNote` (`{{note}}` preserved), `dmSender`; regenerated `generated-keys.ts`. `use-ai-dm-store.ts` — the safety-timeout `lastError`, `rejectPendingActions` sender+content, `checkSceneStatus` fallback, and all three `senderName` literals (two `'System'`→`game.chatPanel.systemSender`, one `'DM'`→`dmSender`) now route through `i18n.t` (computed at event time → picks up the active locale). Store test asserts the timeout via the key. No string literals remain; `senderName: '` count = 0. tsc web clean; generated-keys + key-check + locale-parity + store tests green (50).
- **12C (2026-06-11):** `ChatPanel.tsx` full-view send button — literal `Send` → `{t('game.chatPanel.send')}` (matching the collapsed variant; `t` already in scope). `chatPanel.send` now appears twice; no `Send` literal remains. tsc web clean; ChatPanel test green.
- **12B (2026-06-11):** Normalized the es.json AI-DM entity name to one canonical form per length: `Dungeon Master con IA`→`Dungeon Master de IA`, `DM con IA`→`DM de IA`, `DM IA`→`DM de IA` (24 keys incl. PHASE-10's `pages.aiDmCard.notReadyWarning`). Applied as raw-text substring replacements so only the three entity-naming variants change — feature-usage "con IA" phrases (map analysis, recap) are untouched. The F8 enumeration script now prints nothing; canonical-form count 40 (≥35); `{{error}}` preserved in `notify.aiDmStore.aiDmError`; JSON parses; locale-parity green.
- **12A (2026-06-11):** Pure-value locale fixes. `game.playerBottomBar.centerOnMeTitle` — dropped the `(Phase 16a)`/`(Fase 16a)` internal tag (en+es). `game.resizeMapModal.note` (en) — "new bounds boundaries" → "new bounds". `pages.settingsPage.noCampaignsToBackup` (en) — "to backup" → "to back up". Status-term consistency: `game.chatPanel.aiResponding`/`aiReady`/`aiPaused`/`aiChecking` now name the entity canonically — en "AI DM responding/ready/paused" + "Checking AI DM status..."; es "El DM de IA está respondiendo" / "DM de IA listo" / "DM de IA en pausa" / "Comprobando estado del DM de IA..." (PHASE-10's aiPaused/aiChecking included; `aiProviderUnavailable`/`aiNoModel` refer to provider/model, left as-is). No keys added. Acceptance greps clean (no Phase 16a/Fase 16a/bounds boundaries/to backup); both JSON parse; locale-parity green.
