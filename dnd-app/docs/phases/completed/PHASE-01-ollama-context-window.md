...
- **01E (2026-06-11):** Opt-in KV-cache tuning. `ollama-context.ts` `setOllamaKvCacheType`/`getOllamaKvCacheType` (off by default). `ollama-manager.ts startOllama` injects `OLLAMA_FLASH_ATTENTION=1`+`OLLAMA_KV_CACHE_TYPE=<v>` into the spawn env ONLY when a type is set (default spawn env byte-identical to before). Plumbed `ollamaKvCacheType?: 'q8_0'|'q4_0'` through `AiConfig`/`AiConfigSchema`(`z.enum`)/ai-service(currentConfig+configure+getConfig+init, `setOllamaKvCacheType` calls)/preload/`AiDmConfig`/`configureAiFromCampaign`. New `docs/OLLAMA-TUNING.md` (operator guidance: num_ctx/keep_alive, VRAM table, in-app vs systemd enabling, Gemma caveat, verification, sources). ollama-context.test covers setter/getter; ollama-manager.test asserts the contextSize≥4096 re-export invariant. (startOllama spawn-env asserted via the setter test + tsc — a live spawn test hits the ESM same-module-spy limit on detectOllama.)
- **01D (2026-06-11):** Prefix-cache ordering. `context-builder.ts` now buckets each section and emits them static-first → volatile-last (campaign → character/party/encounter → rulebook → SRD+monsters → creatures → memory → game-state LAST); available-monsters moved to the SRD bucket; all computation/side-effects/breakdown updates unchanged, only emission order. `ai-service.ts` prepends the static `[PROVIDER CONTEXT]` blurb (was trailing the volatile snapshot → re-prefilled every turn). Ordering-contract comment added. `context-builder.test` asserts campaign<character<creatures<gameState order.
- **01C (2026-06-11):** `token-budgets.json` reconciled — `systemPrompt` 1500→12500 (measured static ≈12,336), `total` 25000→21500 (= sum of the 7 consumed dynamic sections). `token-budget.ts` += `CLOUD_CONTEXT_WINDOW`(100k)/`OUTPUT_RESERVE`(2000), `setActiveContextWindow`/`getActiveContextWindow`, `getStaticSystemPromptTokens` (memoized `estimateTokens(assembleSystemPrompt('general'))`), `EffectiveBudgets` + `getEffectiveBudgets()` (raw when window holds the sum — cloud byte-identical; else proportional scale with per-section floors; else pin-at-floors + one-time WARN), `_resetBudgetWarnings`. `context-builder.ts` 7 budget reads → `getEffectiveBudgets()` captured once. `conversation-manager.ts` history loop + `_contextTruncated` use the effective budget + a new window-overflow clause (`totalTokens > window − OUTPUT_RESERVE`). `ai-service.ts startChat` sets the window (`resolveNumCtx` for Ollama, `CLOUD_CONTEXT_WINDOW` for cloud) before context build. Tests extended (token-budget: consistency/scaling/floors/memo/cloud-passthrough; conversation-manager: window-overflow flag + mock window). Note: at the 16384 default the 12.3k static prompt leaves <floors of room, so budgets correctly pin at floors and the flag reports truncated honestly — the static-prompt size is itself addressed by PHASE-11 (GameMode collapse) / PHASE-26 (summarization). tsc node clean; 42 tests green (+ context-builder green).
- **01B (2026-06-11):** Created leaf `src/main/ai/ollama-context.ts` — `OLLAMA_KEEP_ALIVE`/`DEFAULT_NUM_CTX`(16384)/`MIN_NUM_CTX`(4096), `CURATED_MODELS`+`CuratedModel` (moved from ollama-manager, contextSize values bumped to 16384 for the ≥32k-native models, gemma2 8192, phi3 4096), `curatedContextSize` (exact-id then family-prefix), `setConfiguredContextLength`/`getConfiguredContextLength`, `fetchModelMaxContext` (`/api/show`, 5s timeout, cached), `resolveNumCtx` (config>curated>default, clamped `[MIN, modelMax]`, cached for byte-stability), `clearNumCtxCache`, KV-cache-type setter/getter. `ollama-manager.ts` re-exports `CURATED_MODELS`/`CuratedModel` from the leaf (ai-handlers import unchanged). `ollama-client.ts` both functions now send `options:{num_ctx: await resolveNumCtx(...)}`. Plumbed `contextLength?` through `AiConfig` (types.ts), `AiConfigSchema` (ipc-schemas.ts, `z.number().int().min(2048).max(131072).optional()`), `ai-service.ts` currentConfig/configure/getConfig/initFromSavedConfig (+ `setConfiguredContextLength` calls), preload `AiConfigData`, renderer `AiDmConfig`, `configureAiFromCampaign`. New `ollama-context.test.ts` (precedence/clamp/cache/family-match, 26 cases). tsc web+node clean; 86 tests green across the 3 files.
- **01A (2026-06-11):** Migrated `ollama-client.ts` to native `/api/chat` NDJSON. `ollamaStreamChat` (`ollama-client.ts:93-96`) + `ollamaChatOnce` (`:243-247`) now POST `/api/chat` with `keep_alive: '60m'`; SSE/`data:`/`[DONE]`/jsonBuffer parser replaced with NDJSON line parsing (`message.content` appended, `message.thinking` ignored, mid-stream `{error}` → `onError`, trailing-line handling); `OllamaChatResponse` interface reshaped to native (`:20-28`); added `OllamaLastStats` + `getLastOllamaStats()` (`:31-48`) set from the `done:true` chunk in both functions. Tests rewritten for NDJSON (`ollama-client.test.ts`: `/api/chat` URL, `keep_alive` in body, split-chunk reassembly, malformed-line skip, mid-stream error, thinking ignored, stats capture) — 29 pass; tsc node clean.
