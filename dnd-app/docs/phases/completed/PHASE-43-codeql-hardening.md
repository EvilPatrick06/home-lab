...
Executed 2026-06-18. **Live baseline drifted hard from the 2026-06-10 snapshot:** 582 open code-scanning alerts (167 noise-candidate / 415 production; was 552) + **48 open Dependabot alerts** (was "2 torch, no patch" — now aiohttp/python-multipart/yt-dlp/cryptography/starlette/nltk/torch + JS vite/ws/js-yaml/esbuild). Full triage + owner-action list recorded in `docs/SECURITY-LOG.md` (gitignored). Gate: full bmo pytest **1177 passed, 6 skipped**; dnd-app tsc clean on the changed importer surface; all 6 new/edited workflow YAMLs parse.

- **43A — refreshed live data.** 582 CodeQL (per-rule: 158 stack-trace, 127 path-injection, 79 log-injection, 24 file-access-to-http, 23 request-forgery, …) + 48 Dependabot. Cross-ref: PHASE-16 moved app.py (2695 lines now); the cited line numbers were re-derived by symbol per sub-phase.
- **43B — advanced setup.** NEW `.github/workflows/codeql.yml` (matrix python/javascript-typescript/actions, build-mode none, least-priv permissions) + `.github/codeql/codeql-config.yml` (paths-ignore). **OWNER ACTION:** flip Settings → Code scanning → CodeQL → "Switch to advanced" (the committed workflow is inert + the 167 noise alerts stay until the toggle; running both = duplicate scans).
- **43C — noise verified.** `_archive/**` (no live import; only a test fn name matched), `dnd-app/scripts/**` (comments/README only), test files → excluded. **PROMOTED `bmo/pi/dev/**`** OUT of the exclusion: it's production-reachable (`agent.py:18` + `routes/ide.py` `from dev.dev_tools`), so its alerts stay in scope (config comment records this).
- **43D — pickle ACE deleted.** `voice_pipeline.py` + `camera_service.py` legacy `.pkl` `pickle.load` branches + imports/PATH consts removed; `grep -rn pickle bmo/pi --include=*.py` empty. (A sibling agent transiently reverted the camera one — caught + re-applied + re-verified.)
- **43F — importer sanitization (10 prod).** `io/strip-html.ts` `stripHtmlToFixedPoint` + applied at the 10 flagged sites in import-dnd-beyond/{features,inventory,spells}.ts + import-foundry.ts; adversarial nested-tag test. (The 6 alerts in `scripts/audit|build` are excluded by 43B, not touched.)
- **43G — bmo Flask hardening.** `bmo_logging.py`: `fail(log, exc)` (generic client msg + traceback to journald) + `_s(v)` (CRLF strip, CWE-117) + CRLF-escaping human/JSON formatters (defense-in-depth). `fail()` applied across the stack-trace concentration (ide.py 46, tv_api 19, ide_app 14, app.py 6, calendar/music/chat/rclone); `_s()` across the log-injection sites (voice_pipeline 20, mcp_client 16, app.py 11, scene_service 8, +12 files). `cloud_providers.py` left to PHASE-15. pytest stayed 1177-green.
- **43H — workflow perms (5 alerts).** `permissions: contents: read` added to bmo-pi-pytest / dnd-app-ci / dnd-app-validate-5e / security-audit (release.yml + deploy.yml + the PHASE-42 workflows already scoped). Dependabot torch-only F8 is stale → see SECURITY-LOG.
- **43E + Dependabot — OWNER ACTIONS (could not auto-execute).** Bulk-dismissing code-scanning alerts under the owner's identity was correctly blocked by the harness (security-state writes need explicit owner authorization). 47 fetch/SSRF alerts are CLASSIFIED accepted-risk (local-user-configured endpoint URLs — Ollama/Gemini/Discord/cloud-sync/BMO/registry/library/LAN/sound-cache; not remote/peer/AI) with the exact alert numbers + dismissal rationale in SECURITY-LOG for the owner to dismiss. Dependabot: patchable bumps (aiohttp/cryptography/starlette/python-multipart/yt-dlp via pip-compile re-resolve on the Pi venv; JS vite/ws/js-yaml/esbuild via `npm audit fix`) + no-patch dismissals (nltk/torch `tolerable_risk`) are itemized in SECURITY-LOG (the bmo Python re-resolve is a live-host step, deferred).

**Net this phase:** the durable code-security fixes (pickle ACE, importer sanitization, ~171 stack-trace/log-injection sites, 5 workflow perms) are committed and will auto-close on the next scan; the alert-COUNT reduction + the SSRF/Dependabot dispositions are gated on owner actions (advanced-setup toggle, alert dismissals, dependency re-resolve) documented in `docs/SECURITY-LOG.md`.
