...
- **23A (2026-06-16):** flat extraction schema + prompt + mapper. New `src/main/ai/structured-extraction.ts`: `EXTRACTION_CHANGE_TYPES` (12, exported for PHASE-27), `ExtractedChangeSchema` (flat: type/target/value/name/reason, all required), `EXTRACTION_JSON_SCHEMA = z.toJSONSchema(...)`, `buildExtractionPrompts` (schema echo + per-type semantics + referent lists + empty-result rule), `parseExtractionResponse` (JSON.parse → brace-slice fallback → null; NEVER repairJson), `mapExtractedToStatChanges` (party→characterName / creature→`creature_*`+targetLabel / `''`→acting / unique-prefix / unknown+creature-incompatible→dropped-with-issue; re-validated via `validateStatChanges`), `runStructuredExtraction` orchestrator (provider+model passed in — no cycle). Imports limited to zod/ai-schemas/(type)game-state-validation/(type)llm-provider/types. Tests: new `structured-extraction.test.ts` (schema shape, prompt, parser clean/fenced/garbage, mapper cases, orchestrator skip/error/success). Cheap check: tsc node clean; vitest green.
- **23B (2026-06-16):** Ollama `structuredOnce` capability. `llm-provider.ts`: optional `structuredOnce?` on the interface (cloud providers compile untouched). `ollama-client.ts`: `ollamaStructuredOnce` (POST `/api/chat`, `stream:false`, `format`=schema, `keep_alive`=OLLAMA_KEEP_ALIVE, `options:{num_ctx:resolveNumCtx, temperature:0}`, `OLLAMA_PREFILL_TIMEOUT_MS`, `ollamaHttpError`); registered on `ollamaProvider`. Tests: request body asserts (stream/format/temperature/num_ctx/keep_alive), 404 pull-hint, registered-on-provider. Cheap check: tsc node clean; `vitest ollama-client.test.ts` green.
- **23C (2026-06-16):** game-state snapshot + validation + dedupe. New `src/main/ai/game-state-validation.ts`: `GameStateSnapshot`, `buildGameStateSnapshot` (resolves names via `loadCharacterById`, tolerates load failures), `validateAgainstGameState` (referents: unknown character/creature via exact-then-unique-prefix; bounds: damage/heal [1,1000], temp_hp [0,1000], gold |≤100000|, xp [0,100000], exhaustion [1,6] int, spell-slot level [1,9]+count [1,4] int, set_ability_score [1,30], hit_dice |≤20|; unlisted types pass through), `dedupeStatChanges` (key-sorted JSON, reason omitted, case-insensitive referents). Tests: every bounds rule accept+reject, referent matching, pass-through, dedupe echo, failing-load snapshot. Cheap check: tsc node clean; vitest green.
- **23D (2026-06-16):** config plumbing + opt-in UI (default off). `types.ts`: `StructuredExtractionMode` + `AiConfig.structuredExtraction?`; `ipc-schemas.ts` `AiConfigSchema += z.enum(['off','fallback','always']).optional()`; `ai-service.ts` currentConfig literal + configure/getConfig/loadConfigFromDisk round-trip + persisted JSON + exported `getStructuredExtractionMode()`; preload `AiConfigData`; `campaign.ts` `AiDmConfig`; `ai-dm-routing.ts` passes it; `AiDmCard.tsx` Ollama-gated `<select>` (Off / On parse failure / Every response), onChange now MERGES (preserves the field), seeded in openConfigure/openEnable, saved into aiDm + configure payload; i18n `pages.aiDmCard.structuredExtraction*` en+es. Tests: `AiDmCard.test.tsx` (hidden for cloud, shown+default-off for Ollama, mode lands in saveCampaign + configure). Cheap check: tsc web+node clean; vitest green.
- **23E (2026-06-16):** two-call wiring in `handleStreamCompletion`. Replaced the `parseStatChanges`/`parseDmActions` calls with the detailed parsers (computed once); when `mode≠off`, detects tag failure (`rawJsonError` / issues-with-zero-valid / orphan opener via new `hasOrphanStatChangesTag`/`hasOrphanDmActionsTag`) and, on `always` or fallback-failure, builds the snapshot, runs `runStructuredExtraction(getActiveProvider(), currentConfig.model, displayText, snapshot)`, merges `[...tag, ...dedupe(tag, extracted)]`, validates against game state, logs rejections. Whole insertion in its own try/catch → degrades to tag results, never trips the outer catch. `off` = zero new awaits. Tests (`ai-service.test.ts`): off→no call, always→one call+merge, fallback+well-formed→no call, fallback+rawJsonError→call, fallback+orphan→call, extraction-null→tag results unchanged, validation-rejection→dropped (mocks for structured-extraction + game-state-validation + detailed parsers). Cheap check: `vitest ai-service.test.ts` → 55 passed.
- **23F (2026-06-16):** repairJson retirement instrumentation. `ai-schemas.ts`: `repairJsonDetailed(): {repaired, modified}` (+ module counters + `getRepairJsonStats()`); `repairJson` is now a thin wrapper (existing suite untouched). `stat-mutations.ts`/`dm-actions.ts` call sites switched to `repairJsonDetailed` + `logToFile('INFO', …modified a [STAT_CHANGES]|[DM_ACTIONS] block)`. Retirement criteria documented in `ai-schemas.ts` comment + a new `AI_ACTION_CONTRACT.md` section (+ flat-schema paragraph). Logged the prompt-slimming follow-up to `docs/SUGGESTIONS-LOG-DNDAPP.md` (rule 12). Tests: `ai-schemas.test.ts` (modified-flag + stats counter), orphan-helper units in `stat-mutations.test.ts`/`dm-actions.test.ts`. Cheap check: `vitest ai-schemas/stat-mutations/dm-actions` → 211 passed.
