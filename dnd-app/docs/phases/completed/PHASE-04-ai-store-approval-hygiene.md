...
- **04A (2026-06-11):** Added `WEB_SEARCH_APPROVAL_TIMEOUT_MS = 30_000` to `src/shared/constants.ts`; `ai-service.ts` imports it (deleted local literal — zero behavior change); re-exported via `app-constants.ts:26`. Store: `webSearchStatus` type gains `receivedAt: number` (stamped in `handleWebSearch`); new `clearWebSearchStatus()` action; `webSearchStatus`/`fileReadStatus`/`streamStatus` now cleared in `cancelStream` (UNCONDITIONAL set — was skipped when `activeStreamId` null), the inactivity safety-timeout body, `handleDone`, `handleError`, and `reset()` (8 `webSearchStatus: null` writers total). `use-ai-dm-store.test.ts`: web-search test asserts `objectContaining` + numeric `receivedAt`; new "stream-status lifecycle clearing (F1)" describe (cancel w/ + w/o active stream, safety-timeout, done, error, reset, clearWebSearchStatus). tsc web+node clean; 21 store tests green.
- **04B (2026-06-11):** Store: `pendingActions: PendingActionSet | null` → `pendingActionSets: PendingActionSet[]` FIFO; `setPendingActions` → `enqueuePendingActions` (appends; `pushDmAlert('info', rulingQueued)` when queue already non-empty); `approvePendingActions` operates on the head, captures the executor `ExecutionResult` and posts a system chat line + DM alert per `failed` entry (F4); `rejectPendingActions` keeps the `[DM Override]` line on the head; new silent `dismissPendingActions()` (F5). `game-action-executor.ts` enqueues via `enqueuePendingActions`. `RulingApprovalModal.tsx` reads `pendingActionSets[0]`, Dismiss → `dismissPendingActions` (Escape/backdrop too), queue-count note when >1. `GameLayout.tsx:1230` gate `effectiveIsDM && pendingActions` → `isDM && pendingActionSets.length > 0` (F6 half). New i18n keys `notify.aiDmStore.approvedActionFailed`/`rulingQueued`, `game.rulingApprovalModal.queueCount` (en+es). Tests: store FIFO/approve-head/dismiss-silent/override-logs/failure-surfacing (mocked executor), RulingApprovalModal RTL (happy-dom), executor source-grep. tsc web clean; 39 tests green across 3 files.
- **04C (2026-06-11):** Store factory gains `clearApprovalState()` (clearTimeout every `pendingMutations[].timeoutId`, returns empty `pendingActionSets`/`pendingMutations`). Spread into `reset()` and BOTH `initFromCampaign` branches (+ `webSearchStatus: null, fileReadStatus: null`) — the AI-disabled early-return now clears stale campaign-A approval UI too. `use-ai-dm-store.test.ts`: fake-timer `vi.getTimerCount()` hygiene for reset + initFromCampaign (enabled + disabled), asserting both queues empty and the auto-reject timer disposed. tsc web clean; 29 store tests green.
- **04D (2026-06-11):** Store: new transient `webSearchDecided` + `markWebSearchDecided()`; `handleWebSearch` resets the flag on `pending_approval` and fires `pushDmAlert('info', webSearchAutoRejected)` on a `pending_approval→rejected` transition the DM never decided. `WebSearchApprovalPrompt.tsx` rewritten: `role="dialog"`/`aria-modal`/`aria-labelledby` (useId), window `keydown` Escape→reject + Tab focus trap, initial focus on Reject (first focusable), focus restore on unmount, live `Countdown` to `receivedAt + WEB_SEARCH_APPROVAL_TIMEOUT_MS`, and `decide()` surfaces a `success:false` IPC result via `staleRequest` alert + `clearWebSearchStatus()` (markWebSearchDecided fires before the IPC). `GameLayout.tsx:879` gate `effectiveIsDM` → `isDM` (F6 complete). New i18n keys `game.webSearchApproval.countdown`/`staleRequest`, `notify.aiDmStore.webSearchAutoRejected` (en+es). Tests: WebSearchApprovalPrompt RTL (dialog role, countdown, Escape→reject, stale-clear, empty), store decided-flag transitions. tsc web clean; 37 tests green across 2 files.
- **04E (2026-06-11):** `MutationApprovalPanel.tsx`: `changeLabel` gains the 12 missing types (npc_attitude, reduce/add_exhaustion, set_equipped, set_proficiency, set_skill_proficiency, set_save_proficiency, creature_set_resistance/vulnerability/immunity, creature_expend/restore_spell_slot) — verified field names against `ai-schemas.ts`; `changeColor` rewritten as explicit `HARMFUL`/`BENEFICIAL` Sets checked before any prefix rule (fixes creature_heal/restore/remove_condition rendering red, F9); reason now a full-width `break-words` line with a `title` tooltip (was `truncate max-w-[120px]`, F10); Reject All button beside Approve All (F11); panel root `role="status"` + `aria-label`, an `announce()` effect on queue growth, and `announce()` in the store's auto-reject timer body (F12). Store: new one-shot `rejectAllMutations()` (clears timers then queue). New i18n keys (en+es) under `game.mutationApprovalPanel.` (20 keys). New `MutationApprovalPanel.test.tsx` (no-raw-type-leak, color, full reason+title, role=status, Reject All empties store) + store `rejectAllMutations` timer-count test. tsc web clean; 38 tests green across 2 files.
- **04F (2026-06-11):** `DmAlertTray.tsx`: badge button gains `aria-expanded`/`aria-haspopup`/`aria-controls`; expanded panel gains `id`; root `ref` + a while-open `useEffect` that closes on Escape (window keydown) and outside `pointerdown` (ignoring clicks inside the root subtree), both listeners torn down on collapse (F13). New `DmAlertTray.test.tsx` RTL (toggle+aria-expanded, Escape closes, outside-pointerdown closes / inside doesn't). tsc web clean; 4 tests green.
