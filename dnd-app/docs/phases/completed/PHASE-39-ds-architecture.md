...
Executed 2026-06-17. `src/App.jsx` shrank from **11,027 → ~1,580 lines** (only `DungeonScholarApp` + `computeInitialScreen` remain). PHASE-17/18/19 had already landed (in `completed/`), so all their fixes moved verbatim. Method: byte-exact verbatim moves, build-driven import verification (`npm run build` resolves every reference), full vitest as the parity net.

- **39A — game data + helpers → `src/game/`.** NEW `titles.js`, `quests.js`, `items.js`, `bestiary.js`, `difficulty.js`, `achievements.js`, `tome.js`, `defaultState.js` (37 module-scope symbols) + `ErrorBoundary` → `src/components/ErrorBoundary.jsx`. Only cross-module dep: `quests.js` imports `petLevelFromXp` (services/pets) + `BESTIARY_ENTRIES` (`getCounterValue`). `index.ts`→`getSkillDefinitions` n/a (that's dnd-app). Tests: `tome.test.js`, `quests.test.js`, `titles.test.js` (18 cases). App.jsx 11027→9760.
- **39B — UI primitives + tutorial.** 13 files: `components/ui/{OrnatePanel,CollapsibleGroup,ModeCard,badges,FilteredModeBanner,RecordTile,ConfirmModal,ResetConfirmModal,AchievementsModal,TitlesModal}.jsx` (named) + `features/tutorial/{WelcomeModal,TutorialPanel}.jsx` (default). App.jsx 9760→9148.
- **39C — library feature → `src/features/library/`.** `LibraryScreen.jsx` (default) + `ShareTomeModal.jsx` (default + `downloadTomeJson`/`SHARE_LARGE_THRESHOLD` named) + `ImportCodeModal.jsx` + `MetadataEditModal.jsx` + `PasteTomeModal.jsx`. App.jsx 9148→8239.
- **39D — progression + quests.** `features/progression/` 9 screens (Shop/Inventory/Crafting/Bestiary/Stable/Spellbook(+SpellbookContent named, imported by InventoryScreen)/Calendar/Ascension/RunHistory) + `features/quests/QuestBoard.jsx` (holds QuestCard/QuestSection/StoryStepCard/StoryChainView private). App.jsx 8239→5794.
- **39E — study + home.** `features/study/{FlashcardsMode,QuizMode,LabMode,ChatMode,MistakeVault,DomainStudyScreen}.jsx` + `features/home/{HomeScreen,AudioPanel,ThemePanel}.jsx` (HomeScreen renders AudioPanel/ThemePanel). App.jsx 5794→2537 (only `DungeonScholarApp`).
- **39F — `usePlayerActions` hook.** NEW `features/player/usePlayerActions.js` (returns 53 mutators + cross-library/quest-status memos) + `usePlayerActions.test.jsx` (6 cases). Signature `{ playerState, setPlayerState, showNotif, user }`. **`switchActiveTome` left in App** (calls `setScreen` — App-local; build-verified the only coupling). App.jsx 2544→1533.
- **39G — hash router (F4).** NEW `router/useHashRoute.js` (`SCREENS` ×20, pure `parseHash`/`formatHash`, the hook) + `useHashRoute.test.jsx` (12 cases). App.jsx: `computeInitialScreen` lifted to module scope; `const [screen,setScreen] = useState(...)` → `useHashRoute(computeInitialScreen)` (`App.jsx` ~108); deep-link effect activates the tome inline (no forced `home`, so `#/tome/<id>/<screen>` keeps its screen); courseSet-guard effect bounces the 6 gated screens to home when no tome. Exam precedence preserved (a live Trial overrides the hash). All 36 `setScreen` sites untouched.
- **39H — route code-split + bundle + docs.** 17 screen imports → `React.lazy`; HomeScreen stays static; two route-local `Suspense` wrappers collapsed into ONE around the screen switch (`App.jsx` ~1215). Main `index-*.js` **551.50 kB → 375.53 kB** (≤380 target), **no chunk-size warning**; each screen its own chunk (Library 14 kB, Quiz 15.8 kB, DomainStudy 28.7 kB, …); first paint = rolldown-runtime + vendor-react (182 kB) + vendor-icons (21 kB) + index (375 kB), katex defers. README "Project structure" + hash-URL note updated; `docs/PHASE-24-POLISH.md` "App.jsx is large" closed.

**Gate:** `npm run build` exit 0, no chunk warning; `npx vitest run` 460 passing (442 baseline + 6 player + 12 router; +18 game in 39A counted within suite). No lint gate for ds. The deploy workflow (`.github/workflows/deploy.yml`) runs `npm run test` + `npm run build` on push = the authoritative gate.
