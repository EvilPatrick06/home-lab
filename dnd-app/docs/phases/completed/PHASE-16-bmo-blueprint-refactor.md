...
- **16I (2026-06-16):** app.py final wiring + import prune + docs truth. Consolidated the bottom register block under one `# ── Blueprint registration` header (module-scope `register_system`/`register_music`/`register_calendar`/`register_tv`/`register_chat`/`register_realtime` each with a one-line route-prefix comment; `register_ide`+game_relay/library/rclone/sounds stay in `__main__`) + a comment explaining the import-time-vs-__main__ split. Dead-import prune: dropped top-level `import re` (0 uses) + `import shutil` (only a function-local `import shutil` remains at the kdeconnect check), `_save_setting` (unused), `RATE_LIMIT_CHAT`/`RATE_LIMIT_DND_LOAD`/`RATE_LIMIT_IDE_JOBS` (moved with their routes), and the orphaned chat-history aliases `MAX_CHAT_MESSAGE_LEN`/`ALLOWED_CHAT_SPEAKERS`/`_is_voice_speaker`/`RECENT_CHAT_FILE`/`DND_LOG_DIR`/`_save_recent_message`/`_save_dnd_message`/`_save_pending_assistant_stub`/`_finalize_pending_assistant` (kept only `_normalize_chat_speaker`/`_save_chat_message`/`_load_recent_chat` — still called by the init_services voice hook + startup restore). Verified leftover moved-private-name refs are all legit (settings alias, init_services TV seam). state.py docstring: closing paragraph → "Used by the routes/ blueprint modules (…)". app.py module docstring rewritten to list what stays vs what moved to routes/. `grep -c "@socketio.on" app.py`→0; lone `global` is the init_services service-singleton decl; `wc -l app.py`→2650 (≤2650). py_compile clean across app.py + extensions + all routes/ + the three services.
- **16H (2026-06-16):** `routes/realtime_ws.py` (core real-time SocketIO surface). Moved verbatim from app.py's WebSocket Events block (was 5108–5415, here 2619–2925): `_bmo_websocket_authorized` + `_finish_chat_response` as module functions; handlers `connect`/`chat_message`/`plan_approve`/`plan_reject`/`client_timezone`/`scratchpad_read|write|clear`/`disconnect` defined inside `register_realtime(socketio_obj)` via `@socketio.on(...)` (mirrors `register_ide`'s shape — stamps module-level `socketio`). Service refs (`agent`/`voice`/`timers`/`weather`/`music`/`calendar`/`oled_face`/`alert_service`/`_sync_expression`/`_normalize_timezone`/`_request_client_timezone`/`_pi_timezone`/`BMO_API_KEY`/`_bmo_client_is_trusted_localhost`/`app`) late-bound via `_app()`; persistence + speaker-normalization via `services.chat_history`; `on_disconnect` defers IDE cleanup to `routes.ide.cleanup_client_session` (now imported inside the handler). app.py: WS block (2619–2925) deleted, `cleanup_client_session` dropped from the `routes.ide` import (only `register_ide` used), `register_realtime(socketio)` added to the module-scope register block (attaches at import → test client can route real events). New `tests/test_realtime_ws.py` (real SocketIO + real test client: chat_message→chat_response round-trip with text assertion + agent.chat called, user-turn persistence, plan_approve feeds "yes"/speaker=system without persisting a user turn). app.py: 2974→2669 lines. py_compile clean; 39 tests green (realtime_ws round-trip + app_endpoints WS smoke unchanged).
- **16G (2026-06-16):** `routes/chat_api.py` (chat + DnD HTTP). `chat_bp` (no prefix). Moved verbatim: `_strip_markdown`, `/api/chat` (+`@limiter.limit(RATE_LIMIT_CHAT)`), `_DND_ALLOWED_DATA_ROOTS`+`_safe_dnd_path`, `/api/dnd/load` (+`RATE_LIMIT_DND_LOAD`)/`sessions`/`sessions/<date>`/`restore`/`gamestate`/`players`, `/api/chat/history`, `/api/chat/clear`, `/api/chat/compact`. `agent`/`voice`/`socketio`/`_request_client_timezone` via `_app()`; persistence + speaker-normalization + `MAX_CHAT_MESSAGE_LEN`/`DND_LOG_DIR`/`RECENT_CHAT_FILE` via `services.chat_history`; limiter from `extensions`. app.py: 3 blocks deleted (chat+dnd, history/clear, compact), `register_chat(app)` module-scope (the now-orphaned `MAX_CHAT_MESSAGE_LEN`/`RECENT_CHAT_FILE`/`DND_LOG_DIR` aliases left for the 16I prune). New `tests/test_chat_api.py` (chat 400/413, spoofed-`voice:` speaker→`text` persisted, dnd-load 403 jail + 400 empty, history list). py_compile clean; 47 tests green (chat_api + app_endpoints TestChatAPI unchanged + chat_history).
- **16F (2026-06-16):** `routes/tv_api.py` (full TV subsystem, ~563-line block). `AppState.tv_is_on`/`tv_auto_skip` added (state.py docstring updated — TV handles live in tv_api, only the toggles on AppState). `tv_bp` (url_prefix `/api/tv`): constants (TV_IP/TV_KEYS/TV_APPS/cert+worker+python paths — each gains +1 `dirname` so they resolve under bmo/pi/), module-state handles (`_tv_proc`/`_tv_remote`/`_tv_pairing_remote`/`_tv_loop`/thread handles), `_ensure_tv_worker`/`_tv_cmd`/`init_tv_remote`/`_parse_media_description`/`_get_tv_media_title`/`_auto_skip_loop`, all 14 `/api/tv/*` + 2 auto-skip routes — moved verbatim; `_tv_is_on`→`STATE.tv_is_on`, `_tv_auto_skip`→`STATE.tv_auto_skip` (globals dropped). Public seam `tv_cmd`/`tv_connected`/`TV_APPS`/`init_tv_remote`/`tv_is_on`/`set_tv_is_on`; app.py's init_services scene closures rewritten against it (function-local import). app.py: TV block (2430-2992) deleted, `register_tv(app)` module-scope; no `global _tv*` left. New `tests/test_tv_api.py` (auto-skip get/toggle, launch unknown→400, key-no-cert→503, apps list, path-constant relocation lock). py_compile clean; 42 tests green.
- **16E (2026-06-16):** `routes/calendar_api.py` (/api/calendar/* + OAuth flow). `calendar_bp` (url_prefix `/api/calendar`, `_api` suffix avoids shadowing stdlib `calendar`); 9 routes relative + all OAuth helpers (`_ensure_calendar_*`, token read/merge/write, client config, auth HTML) moved verbatim; `calendar` resolved via `_calendar()`. `__file__` RELOCATION FIX: `_calendar_config_dir` gains a 2nd `dirname` and `_calendar_legacy_config_dir` a 3rd, so under `routes/` they still resolve to `bmo/pi/config` (+ legacy `bmo/config`). app.py: calendar block (1161-1599) deleted, `register_calendar(app)` module-scope. `tests/test_calendar_auth_paths.py` imports from `routes.calendar_api`, `@patch` targets → `routes.calendar_api.{shutil,os}`, + a new assertion locking the config-dir relocation. py_compile clean; 81 tests green (auth-paths + calendar_service + app_endpoints).
- **16D (2026-06-16):** `routes/music_api.py` (/api/music/* — 26 routes). `music_bp` (url_prefix `/api/music`); every `@app.route("/api/music/X")`→`@music_bp.route("/X")` with bodies verbatim, `music` resolved per-handler via `_music()=_app().music` (preserving the None→AttributeError→500 behavior); play/play-queue use `unmute_sink`/`get_system_audio_state` from `services.system_audio`. app.py: music block (1166-1382) deleted, `register_music(app)` module-scope, and the now-unused `_get_system_audio_state`/`_audio_unmute_sink` aliases dropped (only `_set_system_volume` remains, for init_services). New `tests/test_music_api.py` (fresh-Flask mount: search 400, play full-state shape + auto-unmute, muted-sink warning, no_unmute skip, queue/add 400). py_compile clean; 85 tests green (music_api + music_service + music_restore + app_endpoints).
- **16C (2026-06-16):** `services/system_audio.py` + `routes/system_api.py` (~850-line shrink: app.py 5278→4431). New `services/system_audio.py` (`get_system_volume`/`get_system_audio_state`/`set_system_volume`/`unmute_sink`, verbatim wpctl wrappers). `AppState.tts_output` field; `_tts_output` global dropped. New `routes/system_api.py` (`system_bp`, no prefix — absolute paths) with `register_system(app)`: health (`/health`+`/api/v1/health`, `/api/health/full`), wifi (helpers + 5 routes), service-restart, status/summary, volume GET/POST + audio/unmute + audio devices/status/output/inputs/input/bluetooth + tts output/audio + settings/config — all moved verbatim, service singletons resolved via `_app()`, volume via `system_audio`, dotted settings via `settings_store`. app.py: deleted the 3 blocks (864-1395, 2894-3150, 4498-4555), added system_audio compat aliases (`_set_system_volume`/`_get_system_audio_state`/`_audio_unmute_sink` — still used by init_services + music routes until 16D) + module-scope `register_system(app)` (registers at import so tests mount it). Tests: `test_system_audio.py` (parse/clamp/unmute), `test_system_api.py` (health, full-schema, volume shape, tts validation+STATE, settings guard). py_compile clean; 46 tests green — `test_app_endpoints.py` health/status/full unchanged (late-binding honors `app.health_checker = mock`). SERVICES.md row.
- **16B (2026-06-16):** `services/chat_history.py` (persistence + speaker normalization). Moved verbatim (underscores dropped from public names): `MAX_CHAT_MESSAGE_LEN`/`ALLOWED_CHAT_SPEAKERS`/`is_voice_speaker`/`normalize_chat_speaker` (from `app.py:146-178`) + `RECENT_CHAT_FILE`/`MAX_RECENT`/`DND_LOG_DIR` + `load_recent_chat`/`save_recent_message`/`save_dnd_message`/`save_chat_message`/`save_pending_assistant_stub`/`finalize_pending_assistant` (from the chat-persistence block). `save_chat_message` reads the agent via a `_agent_resolver` seam; `app.py` installs `chat_history.set_agent_resolver(lambda: agent)` after the service-globals block. `app.py` keeps diff-free compat aliases (`_save_chat_message` etc., `MAX_CHAT_MESSAGE_LEN`, `RECENT_CHAT_FILE`, `DND_LOG_DIR`, …) for the not-yet-extracted /api/chat + /api/dnd routes + init_services voice hook + startup-resume. `tests/test_chat_speaker.py:11` now imports from `services.chat_history`. New `tests/test_chat_history.py` (rolling-buffer cap, stub→finalize strips `incomplete`, finalize-miss False, no-agent vs DM-agent DnD-log write). SERVICES.md row. py_compile clean; 50 tests green (chat_history + speaker + app_endpoints — /api/chat unchanged).
- **16A (2026-06-16):** `extensions.py` (deferred-init limiter) + `services/settings_store.py`. New `extensions.py` holds `_rate_limit_key`/`_is_localhost_request`/`limiter = Limiter(...)` (NO `init_app`) + the 5 `RATE_LIMIT_*` constants, moved verbatim from `app.py:181-227`. `app.py` now `from extensions import (limiter, RATE_LIMIT_*)` + `limiter.init_app(app)`; the 7 `@limiter.limit` sites unchanged. New `services/settings_store.py` exposes `load_setting`/`save_setting` with a monkeypatchable module-level `SETTINGS_PATH` (two dirnames up from `services/` → `bmo/pi/data/settings.json`); `app.py` imports them as `_load_setting`/`_save_setting` aliases (9 internal call sites diff-free) and the old defs (`:3308-3344`) are deleted. `agent.py:1565` swapped `from app import _save_setting` → `from services.settings_store import save_setting as _save_setting` (+ separate `from app import socketio`). New `tests/test_settings_store.py` (roundtrip, default on missing key/file, nested non-clobber, path-suffix lock); SERVICES.md Infrastructure row. py_compile clean; 16 + 36 (settings/auth + app_endpoints) tests green — `import app` + limiter intact.
