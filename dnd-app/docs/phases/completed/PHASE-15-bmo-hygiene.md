...
- **15G (2026-06-16):** Requirements lockfile sync guard (F7). New `tests/test_requirements_sync.py` (stdlib only): PEP-503-normalized parser, parametrized over the three `.in`→`.txt` pairs asserting `declared − UNSAFE ⊆ pinned` (UNSAFE = pip/setuptools/wheel/distribute, which pip-compile omits) with a failure message naming the missing packages + the recompile command; `test_main_lockfile_is_compiled` checks the `autogenerated by pip-compile` header. Passes today (4 tests); fails if a name is added to any `.in` without recompiling.
- **15F (2026-06-16):** Security-header regression lock (F6 — decision: NOT flask-talisman). New `tests/test_security_headers.py` (reuses the deferred-import app fixtures from `test_app_endpoints`): nosniff / X-Frame-Options=SAMEORIGIN / Referrer-Policy / Permissions-Policy (camera+microphone) on `/health`; CSP on the 404 HTML probe contains `default-src 'self'` + `script-src` + `'unsafe-eval'` (the Alpine.js requirement); `Cache-Control: no-cache` on HTML; `/api/games` CORS `*`. Docstring records the talisman rejection (upstream archived 2026-04-18, fork dormant, `force_https` breaks LAN-HTTP) and the PHASE-16 hand-off. ZERO `app.py` edits. 7 tests green.
- **15E (2026-06-16):** Deleted the stale `// ── IDE Tab (under construction — new IDE on port 5001) ──` comment from `bmo.js:405` (the IDE shipped long ago: `routes/ide.py`, `ide.html`, `ide_app/`, kiosk tab redirects to `/ide`) + the doubled blank line (4453→4451 lines). `grep "under construction" bmo.js` → empty.
- **15D (2026-06-16):** curl secret hygiene + `--fail` across all three `os.system("curl …")` sites (F3). `cloud_providers.py` — module-level `import shlex`; new `_curl_cfg_quote` + `_run_curl_config(option_lines, label)` writing every option (incl. secrets) to a `0600` `tempfile` config passed via `-K`, prepending `fail`/`silent`/`show-error`, decoding the `os.system` wait status with `os.waitstatus_to_exitcode` and raising `RuntimeError` on nonzero (exit 22 = HTTP ≥ 400), removing the cfg in `finally`. Converted `fish_audio_tts` (deleted the dead `headers` dict; Bearer→config header), `gemini_chat_stream` (key-bearing URL→config `url`), `groq_stt` (Bearer header + multipart `form =` lines→config). Only ONE real `os.system` remains (inside the helper, `# nosec B605`); the two surviving `Bearer {GROQ_API_KEY}` are the in-process `requests` paths (never a cmdline). New `tests/test_cloud_providers_curl.py`: a fake `os.system` snapshots the cmd + `-K` config; asserts no secret/`Authorization` on the cmdline for fish/gemini-stream/groq-stt (secret present only in the config), `fail` in the config, exit-22 → `RuntimeError("exit 22")`, cfg removed after the call, and quoting escapes. py_compile clean; 6 tests green. Behavior change (intentional): these HTTP errors now raise instead of returning garbage bytes — activates on the user's next BMO restart (NOT restarted here).
- **15C (2026-06-16):** Removed the inert `tools=["read_file","list_directory"]` from `create_dnd_dm_agent` (`dnd_dm.py:664`, now a comment) — `run()` has no tool loop, so the grant was misleading; `AgentConfig.tools` default-factory makes `config.tools == []`. `grep read_file|list_directory dnd_dm.py` → none. Test `test_dm_agent_declares_no_tools` asserts `config.tools == []`.
- **15B (2026-06-16):** DM long-rest persists what it narrates (F2). `_resolve_long_rest` (`dnd_dm.py:586`) now case-insensitively matches a KNOWN character (no phantom records), and for a match sets `hp=hp_max`, `conditions=[]`, pops the stale `spell_slots` remaining-map (no per-level maxima tracked → sheet baseline of full slots is better than a depleted map), bumps `hit_dice_remaining` by `max(1, hit_dice_max//2)` capped at max, and calls `_save_gamestate()` once ("Applied to the saved game state."); an unknown name stays read-only with a ```gamestate``` fallback instruction. `_resolve_short_rest` (`:568`) stays read-only (hit-dice spend is a player choice) but appends an explicit ```gamestate``` persistence instruction. New `tests/agents/test_dnd_dm_rest.py` (fixture snapshots/pops `agents*`+`agent`, stubs the heavy `agent` core + `agents.vtt_sync`, imports real dnd_dm against a tmp gamestate): mutate+save round-trips, case-insensitive match, unknown read-only, short-rest read-only+instruction. py_compile clean; 5 tests green.
- **15A (2026-06-16):** Agent-registry per-agent import isolation + loud logging (F1). `agents/_registry.py` rewritten data-driven: a `_AGENT_SPECS` tuple of 23 `(module, factory)` pairs (original order preserved) + an `importlib.import_module` loop with a per-agent `except Exception` that prints `[registry] FAILED to create agent … — continuing without it` (catches ImportError/AttributeError/ctor errors alike); docstring de-hardcoded ("every non-core specialized agent listed in `_AGENT_SPECS`"). `agent.py:869` — the silent `try/except ImportError: pass  # Remaining agents not yet implemented` became `try import … except ImportError: print('[agent] FAILED to import agents._registry …') else: create+register` (boots on 5 core agents if the module itself breaks, but never silently). New `tests/agents/test_registry.py` — a fixture snapshots/pops the whole `agents*` sys.modules subtree (sibling files mock the package) so the REAL module imports, then restores it; asserts 23 unique specs, a missing module is isolated+logged, a raising factory is isolated+logged. `py_compile` clean; registry + base_agent + app_endpoints = 81 passed (the 78 baseline unaffected).
