...
- **11H (2026-06-11):** `ai-vision.ts` — deleted `captureMapScreenshot` + the `BrowserWindow` import; replaced `encodeForVision` with `buildMapStateDescription(tokenData)` (the text body, no `imageBase64`); `analyzeMapState` drops the capture call and the misleading "(A screenshot … has been captured for reference.)" sentence (it was never sent — the provider `chatOnce` interface is text-only). Header comment documents the deliberate text-only choice. Removed the caller-less `AI_CAPTURE_MAP` surface: handler + `captureMapScreenshot` import (`ai-handlers.ts`), channel (`ipc-channels.ts`), preload `captureMap` (`index.ts` + `index.d.ts`). `AI_ANALYZE_MAP` (the live `AiMapAnalysisModal` feature) untouched. `grep -rn captureMap|AI_CAPTURE_MAP|capture-map src` → zero hits. tsc web+node clean; 35 tests green across 2 files.
- **11G (2026-06-11):** New `library-name-resolver.ts` — cached id→name lookup over the bundled `spells/spells.json`/`feats/index.json`/`equipment/magic-items.json` (mirrors srd-provider's `getDataDir()` loader; `resolveEntryName(category, id)` → name or null; `_resetNameCacheForTests`). `character-context.ts` `formatCharacter5e` — new `refDisplayName(category, ref)` helper (overrides.name → library name → title-cased slug → raw id → 'Unknown'; weapons/armor skip the library since their refs carry full overrides). Added `else if` v4-ref fallbacks (inline v3 output byte-unchanged): Prepared/Known Spells from `knownSpellRefs` + `state.preparedSpellIds`; Weapons from `weaponRefs` (detail from overrides + `(equipped)` from `state.weaponEquipped`); Equipped/Carried Armor from `armorRefs` + `state.armorEquipped`; Feats from `featRefs`; a new `Magic Items: …(attuned)` line from `magicItemRefs` + `state.magicItemAttuned` (before the existing attunement-count line). The conditions read (PHASE-02) is untouched. Tests: `library-name-resolver.test.ts` (resolves acid-splash/adamantine-armor/a feat, null miss, cache reset); `character-context.test.ts` v4 fixture suite (spells/weapons/armor/feats/magic-items resolved, homebrew-slug fallback, v3-inline regression). tsc node clean; 44 tests green across 2 files.
- **11F (2026-06-11):** Wired `actingCharacterId` end-to-end. `ipc-schemas.ts` — `AiChatRequestSchema` gains `actingCharacterId: z.string().optional()` (was stripped by zod, so `ai-service.ts`'s `request.actingCharacterId` was always undefined). `ai-dm-routing.ts` — new exported `resolveActingCharacterId(senderName, lobbyPlayers, campaignPlayers)` (lobby name-match → active campaign name-match → single-member roster fallback → undefined); `routePlayerMessageToAiDm` computes it and passes it as `sendMessage`'s 7th arg in BOTH the main path and the context-free catch fallback. `use-game-network.ts` time-request path resolves + passes it too. The per-actor full/abbreviated context split (already coded + tested in `context-builder.test.ts`) now actually activates. Tests: routing 7th-arg assertion + `resolveActingCharacterId` unit cases (lobby/campaign/solo-single/unknown-in-2+); `ipc-schemas.test.ts` round-trips the field + still strips unknown keys. tsc web+node clean; 50 tests green across 3 files.
- **11E (2026-06-11):** `voice-narration.ts` — exported `NPC_ARCHETYPES` (8) + `EMOTION_VOCABULARY` (8) `as const`; `VOICE_NARRATION_PROMPT` rebuilt via `${…join(', ')}` interpolation so it's physically unable to drift from the lists (byte-identical output — the assembler byte-equality test stays green). New `voice-narration.test.ts`: prompt contains every term; both arrays pinned with `toEqual`; every term matches the `parseVoiceTags` charset `/^[a-z_]+$/`; prompt still starts with the `HDR_VOICE` marker. No `[SPEAKER:]` added (PHASE-21 21C) and no Pi file touched (PHASE-21 21D owns the alias table). tsc node clean; 12 tests green across 2 files.
- **11D (2026-06-11):** `TravelPaceModal.tsx` `PACES` — each `effects` array now one 2024-accurate line (Fast/Normal/Slow per the PHB). `en.json`/`es.json` `game.travelPaceModal.paces`: rewrote `fast.effect0` to the combined 2024 line and DELETED `fast.effect1`; rewrote `normal.effect0`/`slow.effect0`; DELETED `slow.effect1` (en+es symmetric so locale-parity stays green). Regenerated `generated-keys.ts`. No `effect1` travel-pace keys remain. tsc web clean; locale-parity + generated-keys + TravelPaceModal tests green.
- **11C (2026-06-11):** `narrative-rules.ts:82` + `social-rules.ts:19` check-request format → `"Please make a [Ability] ([Skill]) check"` plain text (dropped the `**bold**` the three "never bold" rules contradicted). `exploration-rules.ts:10-12` travel pace → 2024 PHB effects (distances unchanged): Fast = "Disadvantage on Wisdom (Perception or Survival) checks and Dexterity (Stealth) checks", Normal = "Disadvantage on Dexterity (Stealth) checks", Slow = "Advantage on Wisdom (Perception or Survival) checks" (removed the 2014 "-5 passive Perception"/"Cannot use Stealth"/"Can use Stealth"). Greps clean: no `**[Ability]` in `src/main/ai`, no `Cannot use Stealth`/`passive Perception` in exploration-rules. tsc node clean; assembler byte-equality test stays green (recomputes from the same constants).
- **11B (2026-06-11):** Collapsed the dead-mode machinery. `prompt-assembler.ts` — deleted `export type GameMode` + the per-mode switch; `assembleSystemPrompt()` is now zero-arg and returns one static seven-section join (NARRATIVE→CHARACTER→COMBAT→EXPLORATION→SOCIAL→DM_ACTIONS→VOICE), byte-identical to the old `'general'` output, with a header comment explaining it stays a cache-stable prefix (only `COMBAT_TACTICS_PROMPT`, appended by conversation-manager when the context has `Initiative:`, is combat-conditional). `conversation-manager.ts` dropped the `GameMode` import + `gameMode` local, calls `assembleSystemPrompt()` (kept `hasCombat` for the tactics append). `token-budget.ts` `getStaticSystemPromptTokens()` → zero-arg call. `exploration-rules.ts`/`social-rules.ts` header comments → "Always included in every prompt assembly." `prompt-assembler.test.ts` rewritten to a single describe (all 7 markers, exact section-join equality, leads narrative, ends voice, DM-actions before voice, deterministic). `grep -rn GameMode src` → no hits. tsc node clean; 48 tests green across 2 files.
- **11A (2026-06-11):** `dm-actions-schema.ts` — deleted the `**Ability Scores & Features:**` block (the trio doc'd as `[DM_ACTIONS]` got silently dropped by `validateDmAction`); replaced with a one-line pointer that they are `[STAT_CHANGES]` (no trio names, so the acceptance grep is clean). `character-rules.ts` — inserted a `### Character Sheet Mutations` heading before the `set_ability_score` bullet so the 7 character-targeted changes (trio + `set_equipped`/`set_proficiency`/`set_skill_proficiency`/`set_save_proficiency`) sit under an honest heading (was under `### Creature Mutations`). `dm-actions.ts` — added `light_source`/`extinguish_source` to the `DmAction` union (a `// Lighting` group after the darkness-zone variants), matching `ai-schemas.ts` `LightSourceSchema`/`ExtinguishSourceSchema` exactly. `ai-schemas.test.ts` — new "DM action schema ↔ DmAction union contract" describe extracts the union's `action: '…'` literals from `dm-actions.ts` source and asserts both directions vs `DM_ACTION_SCHEMAS` (+ a lighting-actions presence test). `AI_ACTION_CONTRACT.md` — the union is now sync point #4 (#1↔#2↔#4 enforced by tests). tsc node clean; 103 tests green across 2 files.
