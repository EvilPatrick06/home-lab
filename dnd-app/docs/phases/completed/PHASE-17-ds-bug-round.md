...
- **17H (2026-06-16):** residual fork-hostile literal in `supabase-setup.md` (H2 remainder). Step 5 (Site URL + Redirect URLs) now leads with `https://<your-username>.github.io/<your-repo>/` placeholders and keeps the owner URL as an `(e.g. …)` worked example, matching Step 3's pattern. Verified `grep evilpatrick06 docs/supabase-setup.md` — every remaining hit sits next to placeholder/"(e.g. …)" text (preamble lines 11/12/18, Step 3 line 67, Step 5 line 85). Doc-only; cheap check is the grep.
- **17G (2026-06-16):** implemented Foresight Scroll + Tinker's Oil; fixed the mid-battle spell crash (F9 + F10). **F10:** `castSpell`'s `auto_correct` + `reveal_answer` cases now read `battle.currentQuestion` (battle objects have no `questions[]`/`questionIdx` — both spells threw `TypeError` the instant they were cast). **Foresight Scroll:** `POTION_EFFECTS.foresight_scroll` → `{ kind: 'foresight' }`; exported `takeForesightPreview(chargesRef, q)` (consumes one banked charge, returns `q.domain` or `'Uncharted'`); added `foresightChargesRef` (reset per delve); `usePotion` `case 'foresight'` banks a charge with an "Eyes Beyond" notice; battle creation (boss + mob) and `onBattleAnswer` follow-ups attach `previewDomain: takeForesightPreview(...)`; `BattleModal` renders a `🔮 Foresight: <domain>` badge above the question. **Tinker's Oil → mana:** `POTION_EFFECTS.tinkers_oil` → `{ kind: 'mana', amount: 2 }`; `usePotion` `case 'mana'` restores up to `maxMana` (NOT consumed at full mana); App.jsx shop copy updated to "Restore 2 mana within a delve." / `effect: 'restore_mana'` (the old "50/50 or Hint power-ups" were removed with the wave-based delve; mana is the live in-run analog). **No more consuming no-op:** deleted `case 'noop'`; `default` now shows "nothing happens" and returns WITHOUT consuming the item. Exported `POTION_EFFECTS`. Tests: no `noop` kind + every kind in the implemented set + the re-spec, plus 4 `takeForesightPreview` cases. Cheap check: `vitest src/components/DungeonExplore.test.js` → 34 passed; `npx vite build` clean.
- **17F (2026-06-16):** surface localStorage write failures (M10). `persistence.js`: added `isQuotaExceededError` (DOMException name/code-22/1014 matrix) and made `saveToLocalStorage` return `{ ok }` / `{ ok: false, quota }` instead of swallowing silently (existing callers ignore the return — no breakage). `usePlayerState.js`: sticky `localSaveFailed` state + `noteSaveResult` helper applied at all three save sites (applyRemoteState, flushLocal, debounced setState); exposed on the `sync` object. `App.jsx`: a one-shot toast effect on `sync.localSaveFailed` (sticky flag ⇒ fires once/session) + a mount effect registering `setAudioPersistErrorHandler`. `sound.js`: module-level `setAudioPersistErrorHandler` + a fire-once `persistErrorFired` guard in `saveSettings`'s catch (no React import). Tests: `isQuotaExceededError` matrix + `saveToLocalStorage` ok/quota/generic via `vi.spyOn(localStorage,'setItem')` (Storage.prototype spy didn't intercept this env's localStorage). Cheap check: `vitest persistence.test.js` → 31 passed; usePlayerState tests unchanged/green; `npx vite build` clean.
- **17E (2026-06-16):** mistake-vault ID guard (M3) + daily-reward monotone fence (M13). **M3:** `recordAnswer`'s vault branch now `if (!item.id) return t;` before the dedup find — id-less malformed items are never vaulted (they aliased each other under `m.id === undefined`, and the vault UI / de-vault flow key on `m.id` so they could never be redeemed anyway). **M13:** added `CLOCK_SKEW_LIMIT_MS` (48h) + pure `evaluateClaim({now,today,lastClaimedDate,lastClaimedAt,loginStreak})` to `services/devotion.js` (same-day guard → backward-clock-within-48h refusal → `computeNextClaim` streak/cycle). Rewrote `claimDailyReward` to delegate to it, persist `lastClaimedAt: Date.now()` alongside `lastClaimedDate`, and removed the inline gap/streak math; added `lastClaimedAt: null` to `DEFAULT_STATE` (legacy saves skip the fence via the `typeof === 'number'` check). `CalendarScreen` preview now derives `cycleDayIdx` from `computeNextClaim` (kept `gap` only for the status message) so preview ↔ claim never diverge. Imported `computeNextClaim`/`evaluateClaim` into App.jsx. 7 new `evaluateClaim` tests (same-day, next-day +1, gap>1 reset, rollback-within-48h refusal, ≥48h future self-heal, legacy no-fence, preview parity). Cheap check: `vitest src/services/devotion.test.js` → 23 passed; `npx vite build` clean.
- **17D (2026-06-16):** `updateTomeProgress` functional form + read-modify-write sweep (M4). Extended `updateTomeProgress(updates)` (App.jsx) to accept either an object patch OR `(prevProgress) => patch` computed inside the `setPlayerState` updater against live progress; doc comment states both forms + the purity requirement. Converted every F3-table read-modify-write site to the functional form: card `cardsReviewed` (FlashcardsMode rate), `quizAnswered` (handleAnswer + handleSkip — dropped the now-unused `newQuizCount` const, quiz_warrior still uses `totalQuizAcrossLib`), lab `writeLabProgress` + both completion writes (submitStep + continueAfterGrade — hoisted `const completedAt = Date.now()` above each call since `Date.now()` is impure in an updater), ChatMode `setMessages`/send/search-result/assistant-reply/fallback (append to `prev.chatHistory`, keeping `newMessages`/`newOracleCount` for the request payload + oracle_friend check), `ExamMode.jsx` practiceExams append (deleted the `prior` const), `DungeonExplore.jsx` finishRun counters (`runsCompleted`/`bossesDefeated`) + `runHistory` append. Only `clearChat`'s `{ chatHistory: [] }` absolute write stays object-form (excepted). Verified: `grep updateTomeProgress(` shows zero remaining patch derived from the render-time `tomeProgress` prop. Cheap check: `npx vite build` clean + `vitest src/components/DungeonExplore.test.js` → 27 passed.
- **17C (2026-06-16):** setState-updater purity sweep (H5). Added three central derive-from-transition effects in `DungeonScholarApp` (after `unlockSpecialTitle`): `seenAchievementsRef`→toast on new `playerState.achievements` entries, `seenTitlesRef`→toast on new `unlockedTitles` (SPECIAL_TITLES only), `prevLevelRef`→toast on `playerState.level` increase; each mount-initializes its "seen" set so loaded saves don't spam. Made `updateProgress`/`checkAchievement`/`unlockSpecialTitle` pure (deleted their in-updater `setTimeout(showNotif)`). **Extended beyond the plan's literal step-list** because the central achievements effect now covers EVERY `achievements` push: also removed the now-redundant in-updater achievement toasts in `recordAnswer` (volume/accuracy), `removeFromVault` (vault_warrior), `trackModeUse` (all_modes/first_card/first_oracle), `addTomeToLibrary` (first_tome/tome_collector/tome_archivist) — all 14 ids verified present in `ACHIEVEMENTS` so the effect names them correctly — and the special-title toasts in the tutorial path (`initiated`) + story-chain reward (`adeptVeteran`/`masterSlayer`/`mythicSage`/`pathwalker`, all in SPECIAL_TITLES). **Deviation (logged):** dropped the `(+${ACHIEVEMENT_GOLD} gold)` suffix the plan specified for the central message — only `checkAchievement` grants gold; the 14 other grant paths award none, so a fixed suffix would be a false UI claim (the truthfulness the 09/10 phases enforced); gold still updates the visible counter. Hoisted the synchronous setters out of QuizMode `handleAnswer` (streak math + `checkAchievement` calls now in the handler) and `overrideVerdict` (`recordAnswer`/`awardXP`/`setStreak` before a pure `setAnswered`). Relocated the simple informational deferred toasts out of their updaters too (tutorial complete/+XP via render-state transition, pet level-up, tome-added info). **Left inside updaters (deliberate, documented):** the 4 quest-XP deferred toasts (`claimQuest`/`claimWeeklyQuest`/`claimStoryStep` at 2535/2609/2713/2715) — relocating needs either duplicated quest-eligibility guards (drift hazard) or reading quest state from the render closure for the eligibility decision (reintroduces the M4/F3 stale-closure bug 17D fixes); a deferred toast that fires once in production is strictly more correct, matching the plan's own "recordAnswer is safe" precedent. Net: zero synchronous setters and zero unlock-toast `showNotif` inside any updater; unlock toasts fire exactly once. Cheap check: `npx vite build` → clean.
- **17B (2026-06-16):** wired AbortController into QuizMode + LabMode oracle submits (M2 component half). QuizMode (`App.jsx` ~5126): added `gradeAbortRef` + unmount cleanup `useEffect(() => () => abort, [])`; `submitFillBlankWithOracle` now aborts any prior grade, passes `controller.signal`, and on `aborted`/`AbortError` returns without `setGrading`/`handleAnswer` (records nothing). LabMode (~5691): same `gradeAbortRef` but cleanup keyed `[selectedLab]` (aborts when leaving the lab detail view too), declared above the early return next to `labKeyRef` so hook order stays stable; `submitTextWithOracle` rewritten identically (returns without `submitStep` on abort). ChatMode fetch left untouched (out of scope). Cheap check: `npx vite build` → built clean (pre-existing >500 kB chunk warning, owned by PHASE-39).
- **17A (2026-06-16):** `oracleGrader.js` balanced JSON extraction (M6) + abort semantics (M2 service half). Replaced the greedy `/\{[\s\S]*\}/` regex (was line 52) with `collectBalancedJsonBlocks` (string-aware depth scanner) + `asVerdict` normalizer; `extractJsonVerdict` now fast-paths `JSON.parse(text.trim())`, else scans every balanced `{...}` block and returns the LAST valid verdict (prose-example-before-real-answer case). `gradeAnswer`'s three catches (network, body-read `!response.ok`, `response.json()`) rethrow when `signal?.aborted || err?.name === 'AbortError'` (body-read catch gained an `err` binding); JSDoc documents "rejects with AbortError, else fallback". Stale header comment dropped "Dungeon fillblank challenges" (battles are MC/TF, never call gradeAnswer — F5). 6 new tests in `oracleGrader.test.js` (last-verdict multi-JSON, braces-in-feedback, fenced ```json block, unparseable→fallbackReason pin, AbortError rejection, non-abort fallback pin). Cheap check: `npx vitest run src/services/oracleGrader.test.js` → 17 passed.
