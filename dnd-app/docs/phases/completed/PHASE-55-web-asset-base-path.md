...
- 55A — DONE (`src/renderer/src/utils/asset-url.ts` new) — `resolveAssetUrl(path)`: passes self-describing URLs (`data:`/`blob:`/`file:`/`http(s):`) through unchanged; otherwise strips a leading `./` or `/` and prefixes `import.meta.env.BASE_URL` (no-op `/` on desktop, `/DungeonTableOnline/` on web). Routed every runtime `Assets.load` / bundled-asset chokepoint through it: `components/game/map/map-canvas/use-map-background.ts:49` (the confirmed map-bg 404), `components/game/map/token-sprite.ts:183` (token images), `components/game/map/occlusion-layer.ts:79` (occlusion tiles), `components/game/map/MapCanvas.tsx:332` (portal-map preload), and `services/library/remote-sounds.ts:204` (bundled-sound fallback). The map-bg debug-log `resolvedUrl` now reflects the actual loaded URL. Runtime `public/data` JSON (recipes, crafting) loads via the `window.api.game.loadJson` IPC bridge / remote-library, NOT an HTTP asset fetch, so it is unaffected by the base; @font-face url() in bundled CSS is base-rewritten by Vite. Unit test `utils/asset-url.test.ts` (8 cases: ./data + /data + ./sounds prefixing, data:/blob:/file:/http(s) passthrough, bare-"data/" non-match, empty input, desktop "/" no-op).
- 55B — DONE (`components/game/map/MapCanvas.tsx`, `components/game/map/map-canvas/use-map-background.ts`) — the `bgLoadError` toast now (a) auto-dismisses after 6s via a `useEffect` timeout, (b) carries a × close button (`aria-label` i18n-keyed `game.mapCanvas.dismissMapError`, added en+es, generated-keys regenerated → 6573 keys), and (c) clears on `map?.imagePath` change (a `setBgLoadError(null)` at the top of the loader effect) so a stale error no longer lingers after switching maps. The "map renders on the deployed web build" effect needs the deployed build — implementer-verify post-merge at `https://bmo.mybmoai.work/DungeonTableOnline/`.
