...
- **18G (2026-06-16):** README answer-key disclosure (L10). Added an "Answer keys are not secret (by design)" section after the Oracle/cloud-sync setup notes — tomes are plain JSON, `correctAnswer`/`acceptedAnswers` are client-readable in the sample files + localStorage/cloud save, so the app is unsuitable for proctored/graded exams; points at the sealed-tome future work (PHASE-41). Docs-only. Cheap check: grep confirms the section.
- **18F (2026-06-16):** CSP meta (build-only) + Referrer-Policy (M8). `index.html`: added `<meta name="referrer" content="strict-origin-when-cross-origin">` (harmless in dev). `vite.config.js`: build-only `cspPlugin` (`apply: 'build'` + `transformIndexHtml` head-prepend) injecting a `Content-Security-Policy` meta — `default-src 'self'`, `script-src 'self'`, `style-src 'self' 'unsafe-inline'` (Tailwind + React inline styles), `img-src 'self' data: avatars.githubusercontent.com`, `font-src 'self' data:` (KaTeX), `connect-src 'self' *.supabase.co wss://*.supabase.co *.workers.dev` + the exact `VITE_ORACLE_ENDPOINT` origin (parsed at build) when off `*.workers.dev`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`; frame-ancestors/report-uri/sandbox omitted (not enforceable via meta). Dev untouched (HMR ws stays unconstrained). Cheap check: `npx vite build` clean; `dist/index.html` carries both meta tags + all CSP hosts (supabase/wss/workers/avatars/object-src/base-uri/form-action verified); `vitest supabase.test.js` → 8 passed. Post-deploy runtime retest of auth/Realtime/Oracle is a noted follow-up (CSP violations surface as console errors), not a gate.
- **18E (2026-06-16):** hashed the Realtime channel topic (L9). `cloudSync.js`: added `hashChannelTopic` (FNV-1a 32-bit, base36, sync — `subscribeSaves` returns its unsubscribe synchronously so no async digest); `subscribeSaves` channel topic is now `saves:${hashChannelTopic(userId)}`; the `postgres_changes` filter keeps the raw UUID (server-required) with a comment explaining the split. `cloudSync.test.js`: `channel`/`removeChannel` added to the supabase mock; tests — hash deterministic/base36/per-UUID-distinct, and the subscribe topic starts with `saves:`, excludes the raw UUID, equals `saves:<hash>`. Cheap check: `vitest cloudSync.test.js` → 10 passed.
- **18D (2026-06-16):** startup base-path/redirect diagnostic (L13). `supabase.js`: pure `detectBaseMismatch({hostname,pathname,baseUrl})` (github.io → first-segment must equal base; else served path must start with base; root/`'/'` base ⇒ null) + `warnIfBaseMismatch()` runner that emits one `logWarn` (message-only) naming the `VITE_BASE`/`vite.config.js`/Supabase-redirect fix. `App.jsx`: `warnIfBaseMismatch()` called once at the top of the OAuth-callback mount effect. `supabase.test.js`: 6 `detectBaseMismatch` cases (github.io match/mismatch/deep-path, non-github match/mismatch, root base). Cheap check: `vitest supabase.test.js` → 8 passed.
- **18C (2026-06-16):** runtime RLS exposure probe + critical banner (M11). `cloudSync.js`: `checkRlsExposure(userId)` — cross-user `select user_id … neq(user_id).limit(1)`; any row ⇒ `{checked:true, exposed:true}`, empty ⇒ exposed:false, error/throw/no-userId ⇒ `{checked:false}`. New `RlsWarningBanner.jsx` (`role="alert"`, fixed top, dismissable) + colocated test. `App.jsx`: `rlsExposed` state + a read-only probe effect on `user?.id` (StrictMode-safe), banner rendered above the MergeChooser block; dismissal session-local (re-probes on reload). `cloudSync.test.js`: mock factory extended with the `select→neq→limit` chain + a resettable `limitResult`; 4 probe tests. `docs/supabase-setup.md` step 8: `pg_class.relrowsecurity` verification SQL + the runtime-banner note (+ the "needs a second user's row" limitation). Cheap check: `vitest cloudSync.test.js RlsWarningBanner.test.jsx` → 11 passed.
- **18B (2026-06-16):** Oracle endpoint → `VITE_ORACLE_ENDPOINT` (M9), both call sites, model bump, Worker stub doc. `oracleGrader.js`: replaced the hardcoded `const ORACLE_ENDPOINT` (line 5) with exported `getOracleEndpoint()` (trims + validates `^https?://`), `isOracleConfigured()`, and `ORACLE_MODEL = 'claude-sonnet-4-6'`; `gradeAnswer` early-returns the string-match fallback (`reason: 'Oracle not configured'`) when unset (after the empty-answer guard), fetches the resolved endpoint, and sends `ORACLE_MODEL`. `App.jsx` Oracle chat: imported the three helpers; chat mode default → `isOracleConfigured() ? 'oracle' : 'search'`; `send()` pushes a "not configured → Tome Search" system_notice + search result when Oracle requested but unconfigured; fetch uses `getOracleEndpoint()` + `ORACLE_MODEL`. `.env.example` gained `VITE_ORACLE_ENDPOINT=`; `.github/workflows/deploy.yml` build env gained `VITE_ORACLE_ENDPOINT: ${{ secrets.VITE_ORACLE_ENDPOINT }}` (forks self-disable). New `docs/oracle-setup.md` (proxy rationale + CORS-restricted Worker stub + rate-limit/model/CSP notes); README "Cloud sync setup" gained an Oracle pointer. `oracleGrader.test.js`: `vi.stubEnv('VITE_ORACLE_ENDPOINT', …)` added to both gradeAnswer describes; 4 new tests (unset→no-fetch fallback, endpoint+model on the request, `isOracleConfigured` matrix). `grep workers.dev|claude-sonnet-4-20250514 src/` → only the retirement comment. **⚠ USER ACTION (post-merge):** add repo Actions secret `VITE_ORACLE_ENDPOINT=https://dungeon-scholar-oracle.patrick-home-lab.workers.dev` so the owner deploy keeps its Oracle — until then (and note the old model retired 2026-06-15) the live site grades via string-match / Tome Search. Cheap check: `vitest oracleGrader.test.js` → 20 passed.
- **18A (2026-06-16):** logger module + console-site conversion (H6/L7). New `src/services/logger.js` (`errorMessageOf`, `logError` — PROD→message-only `console.warn`, dev→`console.error` with object; `logWarn` — always message-only; `import.meta.env.PROD` read at call time so `vi.stubEnv` works). Converted all 6 production console sites via `logError(context, err)` + imports: `AccountPanel.jsx:34/45` (delete cloud/account), `SignInButton.jsx:18` (sign-in), `usePlayerState.js:352` (cloud pull), `App.jsx` ErrorBoundary `componentDidCatch` (logError + dev-only `componentStack` behind `!import.meta.env.PROD`) + OAuth-callback catch. New `logger.test.js` (PROD message-only + no-payload-leak, dev object-ref, `logWarn`, `errorMessageOf` matrix). Acceptance grep clean except the excepted dev-only componentStack line. Cheap check: `vitest logger.test.js usePlayerState.test.jsx` → 27 passed.
