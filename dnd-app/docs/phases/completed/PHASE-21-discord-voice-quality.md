...
- **21A (2026-06-16):** sentence-chunked streaming TTS engine + queue (F1/F2/F3/F4). New `bmo/pi/services/discord_tts.py`: `split_sentences()` (stream2sentence primary, pure-regex fallback that never raises; merge sub-`min_chars` + hard-split over `max_chars`), `split_sentences_stream()` (generator-native, deferred), `@dataclass VoiceSpec`, `resolve_backend()` (kokoro `KOKORO_TTS_URL` → piper model-file → fish), `synthesize_chunk()` (kokoro REST / piper Python API into a `wave` WAV / fish), `apply_prosody()` (sox tempo+pitch, graceful no-op when sox/identity). `discord_dm_bot.py`: deleted `TTS_COOLDOWN`/`_last_tts_time`/`can_tts`/`mark_tts` + the `text[:500]` truncation (+ pruned now-unused `time`/`fish_audio_tts` imports); new `@dataclass NarrationJob`; queue now holds NarrationJob; `_narration_worker` → `_synthesize_and_play` pipelines synth(i+1)‖play(i) across `split_sentences` chunks; `_play_audio`→event-based `_play_chunk` (await `after` via `call_soon_threadsafe`, no busy-wait). **Plan deviation (rule 22):** PHASE-20 made `queue_narration` return a *string* status consumed by `dm_bot_control.py`; kept that contract (plan's "return dict" superseded). `requirements{,-ci}.in` += `stream2sentence`; both lockfiles pip-compiled (adds stanza/nltk/emoji/regex/udapi/udtools/termcolor — reuses the already-pinned `torch==2.12.0+cpu`, so no new heavy download; plan's "tiny" note amended). `setup-bmo.sh`: nltk-punkt predownload + libritts_r voice fetch (both `|| true`), `bmo-dm-bot.service` MemoryMax 512M→1G / CPUQuota 50%→150%. `SERVICES.md` discord_tts row. Tests: new `tests/test_discord_tts.py` (regex split/merge/hard-split, backend ladder mocked, prosody no-op/skip-speed/missing-sox); `test_dm_bot_voice.py` enqueue/no-vc/FIFO/no-truncation; removed the obsolete cooldown-wait test. Cheap check: `pytest test_discord_tts test_dm_bot_voice test_dm_bot_control` → green. **⚠ USER ACTION:** the live `bmo-dm-bot.service` MemoryMax/CPUQuota bump needs `sudo tee /etc/systemd/system/bmo-dm-bot.service … + daemon-reload + restart` (checked-in script only).
- **21B (2026-06-16):** barge-in cancellation, opt-in/off-by-default (F7). Bot: `DMSession` gained `narration_cancel: asyncio.Event` + `synth_task` (reset clears/cancels); `_synthesize_and_play` clears the event per job, tracks the in-flight synth task, and breaks between/after chunks on cancel (returns `"cancelled"`); `_play_chunk` bails on the cancel event; new `cancel_narration(flush)` (drain queue + `vc.stop()` + cancel synth, returns `{cancelled, flushed}`). `dm_bot_control.py`: `/control/narrate` honors `interrupt` (await cancel before enqueue) + passes `speaker`; new `/control/narrate/cancel`. `app.py`: proxy `/api/discord/dm/narrate/cancel` (+v1, rate-limited). dnd-app: `bmo-bridge.ts` `sendNarration(text, opts)` + `cancelNarration()` + `bargeInEnabled` gate (`set/isBargeInEnabled`); `ai-service.ts` sends `interrupt: isBargeInEnabled()` and fires `cancelNarration()` at `startChat` entry (both inert unless narration+bargeIn on); IPC `BMO_NARRATE_CANCEL` + `BMO_SET_BARGE_IN_ENABLED` + `BmoNarrateRequestSchema` (first zod for a BMO_* channel); `ai-handlers` payload-object narrate + cancel + bargeIn handlers; preload `bmoNarrate(payload)`/`bmoNarrateCancel`/`bmoSetBargeInEnabled` (+ d.ts); `bmo-narration.ts` payload shape + `cancelNarrationThroughBmo`; `use-narration-tts-store` `bargeIn` (default false, persisted `dnd-vtt-ai-narration-barge-in`, synced to main on set+init); `DMTabPanel` Barge-in toggle + Stop-voice button (DM-alert on failure) when narration on; i18n `bargeIn*`/`stopVoice*` en+es. Tests: pytest cancel flush/idempotent + interrupt route + cancel route + app-proxy; vitest bridge cancel + store bargeIn + narration-service payload. Cheap check: tsc web+node clean; targeted vitest+pytest green.
- **21C (2026-06-16):** per-NPC voice casting (deterministic, persisted, DM-overridable). New `bmo/pi/services/voice_casting.py`: `KOKORO_POOL`/`PIPER_SPEAKER_POOL` (grouped voice ids), `ARCHETYPE_GROUPS`, `@dataclass CastEntry`, `class VoiceCasting` (`BMO_VOICE_CAST_PATH`, atomic `os.replace` write + mtime reload + lock) with `get_voice` (persisted-or-sha256-seeded linear-probe past used voices, archetype-group filtered; `meta` accepted+ignored = PHASE-25 hook), `list_cast`/`set_voice`/`reset_voice`/`pool_for_backend`. `discord_dm_bot.py`: `DMBot._voice_casting`; `_synthesize_and_play(speaker=…)` resolves the cast voice into the `VoiceSpec` (kokoro_voice / piper_speaker), prosody still from 21D `get_prosody`. `app.py`: `/api/discord/dm/voices` GET/POST/DELETE (operate on the shared JSON directly, lazy import, rate-limited mutations). `SERVICES.md` voice_casting row. dnd-app: `voice-narration.ts` `[SPEAKER:Name]` line + 3-tag example; `ai-response-parser.ts` `VOICE_SPEAKER_RE` (display-case, ≤40) parse+strip; `ai-service.ts` forwards `speaker`; IPC `BMO_VOICE_CAST_GET/SET/RESET` + `VoiceCast{Get,Set,Reset}Schema` + bridge `getVoiceCast`/`setVoiceCast`/`resetVoiceCast` + handlers + preload (+d.ts); new `VoiceCastSection.tsx` (collapsible list/override/re-roll) in `DMTabPanel` when narration on; i18n `voiceCast*` en+es. Tests: new `test_voice_casting.py` (determinism/spread/archetype/probing/persistence/override/reset/mtime); `test_app_endpoints.py` voices GET/POST/DELETE+400; vitest parser SPEAKER + `VoiceCastSection.test.tsx`. Cheap check: tsc web+node clean; targeted vitest+pytest green.
- **21D (2026-06-16):** emotion-prosody map completion + normalization (F5). `voice_personality.py`: `PIPER_EMOTION_PROSODY` += `neutral`/`angry`/`menacing`; new `EMOTION_NORMALIZE` table + `normalize_emotion()` (canonical key or alias, incl. `fearful`→`scared`); `get_prosody` now COMBINES npc base × emotion overlay with clamped bounds (`speed∈[0.6,1.4]`, `pitch∈[-10,8]`) instead of dropping emotion when an NPC was set (npc-only/emotion-only numerically unchanged); `BMO_EMOTIONS` + `_EMOTION_ALIASES` gained `angry`/`menacing`/`neutral` (+uppercase) and the `fearful`→`scared` alias so `detect_emotion` recognizes the VTT vocabulary. Tests (`test_dm_bot_voice.py`): every VTT mood maps (only neutral flat), `fearful≡scared`, npc×emotion combine+clamp (`booming_dragon`+`angry` → speed 0.756 / pitch −10), bounds sweep, `normalize_emotion` unknown→None. Cheap check: `pytest test_dm_bot_voice.py` green.
