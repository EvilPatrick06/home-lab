...
- 63A Step 3 — DONE (restart rides the deploy; pre-state verified live 2026-07-15) — live `curl -sI` confirmed the pre-fix state exactly as the plan describes (hashed chunk → `no-cache`; VTT HTML → kiosk CSP). The live service runs from the deploy-owned checkout (`/home/patrick/home-lab-deploy`), which only ever resets to a validated ancestor of `origin/master` — so the restart + post-fix live-header check is performed automatically by the health-gated deploy (`.github/workflows/bmo-deploy.yml` → `bmo/pi/scripts/deploy.sh`, selective unit restart + /health gate) once the integrator merges `auto/dnd-phase-executer`. A manual restart pre-merge would be a no-op by design (deploy isolation, AUTOMATED-AGENT-GIT-WORKFLOW.md Rule 3). Post-fix header behavior is pinned by the 7 `TestWebappServingHeaders` tests against the real Flask app.
...
- 63B Step 3 — DONE (headers, test-pinned; live check rides the deploy per 63A Step 3 note) — VTT index + fallback carry exactly `_VTT_CSP`, kiosk HTML unchanged (both asserted in tests against the real app; live pre-state re-confirmed 2026-07-15). The attended browser-connected CSP-violation sweep (map upload, PDF viewer, 3D dice, multiplayer join) remains the gate for any further tightening and is carried to the next browser-connected QA run per the plan (explicitly \"an attended follow-up check, not a blocker\").
