...
Executed 2026-06-17 on top of PHASE-39's module split. Combined gate: `npm run build` exit 0 (no chunk warning; PWA `sw.js` + `manifest.webmanifest` emitted), `npx vitest run` **501 passing** (was 442 pre-39-tests baseline; +game/router/player/import/audio/notesCrypto/TomeNotes/usePlayerState/icons suites). No lint gate for ds; `deploy.yml` (test + build) is the authoritative gate.

- **40A — import size caps (L14).** NEW `src/services/importLimits.js` (`MAX_TOME_IMPORT_BYTES=2_000_000`, `checkImportSize`) + test (4 cases). Guards added to `handleImportFile` (file.size), `handlePasteImport` (text.length), `handleShareCodeImport` (code.length) in `App.jsx` — all reject before `JSON.parse`/`atob` with a user notif.
- **40B — defensive deck copies (L15).** `FlashcardsMode.jsx` + `QuizMode.jsx`: `baseDeck` now a `useMemo(() => (…prop || courseSet.x || []).slice(), [prop, courseSet])` — stable-identity defensive copy; the fallback branch no longer hands the raw `courseSet` array to component state.
- **40C — AudioContext auto-suspend (L8).** `audio/sound.js`: `armAutoSuspend`/`disarmAutoSuspend` (visibilitychange → suspend ctx + stop BGM timer when hidden, restart only if unmuted on return) + `closeAudio` (idempotent teardown). `App.jsx` mount effect now `armOnFirstGesture(); armAutoSuspend(); return () => disarmAutoSuspend();`. NEW `audio/sound.test.js` (MockAudioContext, 4 cases).
- **40D — cloudSync tests + listener fix (L18).** ONE-line production fix: `usePlayerState.js` `removeEventListener('blur-sm', …)` → `'blur'` (Tailwind-codemod corruption — leaked flush listener). Appended 4 describes / 13 cases to `usePlayerState.test.jsx`: Realtime echo dedup (exact-updatedAt + content-hash + genuine-apply + unsubscribe), BroadcastChannel dedup (stubbed), resolveMerge local/cloud/cancel, blur-cleanup regression.
- **40E — notesCrypto.js (F5 part 1).** NEW dependency-free `src/services/notesCrypto.js`: PBKDF2-HMAC-SHA-256 (`KDF_ITERATIONS=600_000`, OWASP) → AES-256-GCM; `deriveKey`/`encryptPayload`/`decryptPayload`/`clearKeyCache`; versioned payload `{v:1,kdf,iter,salt,iv,ct,updatedAt}` (b64); fresh 12-byte IV always, per-payload random salt, in-memory key cache. `decryptPayload` throws `decrypt-failed`/`unsupported-version`. Test (11 cases) incl. a committed wire-format fixture. Zero app imports (reusable by PHASE-41).
- **40F — per-tome notes UI (F5 part 2).** `updateTomeNotes` added to `usePlayerActions.js` hook (next to `updateTomeMetadata`, in the return + App destructure) — stores the `encryptPayload` blob on `tome.notes`, rides existing sync. NEW `components/TomeNotes.jsx` (create-with-passphrase / locked-unlock / unlocked-edit-save-lock-delete; `useDialogA11y`; plaintext+passphrase only in component state, wiped on unmount/lock/close). `LibraryScreen.jsx` per-tome `ScrollText` Notes button (`onNotes` prop). `App.jsx` `notesTome` state + modal render. Test (8 cases) incl. the privacy assertion that the saved blob never contains the plaintext.
- **40G — offline-first PWA (F6).** `npm i -D vite-plugin-pwa@^1.3.0` (no ERESOLVE under Vite 8). NEW `scripts/generate-pwa-icons.mjs` (zero-dep PNG via node:zlib + hand-rolled CRC32) → 4 committed `public/` PNGs (192/512/maskable-512/apple-touch-180 opaque) + `scripts/generate-pwa-icons.test.mjs` (signature + IHDR assert, 4 cases). `vite.config.js` VitePWA (autoUpdate, no runtimeCaching — Supabase/Oracle stay network-only; manifest derived from Vite `base`, verified tracks `VITE_BASE=/home-lab/`). `index.html` theme-color + iOS metas + apple-touch-icon. README: stale Vite `^7`-pin note replaced, "Install as an app (offline)" section + iOS isolated-cookie-jar caveat. Build emits `sw.js`/`manifest.webmanifest`/`workbox-*.js` precaching the lazy chunks.

Note: `npm audit` flags 2 pre-existing Vite-8/esbuild dev-server advisories (Windows-only) — not introduced here, and ds `deploy.yml` runs only test + build (no audit gate), so CI is unaffected.
