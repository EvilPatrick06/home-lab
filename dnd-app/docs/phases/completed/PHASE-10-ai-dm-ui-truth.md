...
- **10I (2026-06-11):** `NarrationOverlay.tsx` — wrapped the narration `<p>` in `<div className="max-h-[60vh] overflow-y-auto" tabIndex={0} aria-label>` so a long scene scrolls there and the countdown/hint/close stay on-screen. The parchment box gained `role="dialog"`, `aria-modal="true"`, `aria-label={t('...ariaLabel')}`. A `closeRef` on the close button + a mount effect (`closeRef.current?.focus()`) moves focus into the dialog (single tabbable control + a focusable scroll region — full focus trap intentionally skipped, documented; Escape-to-close already existed). New i18n key (en+es): `ariaLabel`. Tests (happy-dom): `role="dialog"` + `aria-modal` + label; close button focused on mount; the text wrapper has `max-h-[60vh]`/`overflow-y-auto`/`tabindex=0`; Escape calls `onDismiss` after the 300ms fade (fake timers). tsc web clean; 4 tests green.
- **10H (2026-06-11):** `AiDmCard.tsx` — `keyForProvider(dm, p)` helper; `openConfigure` prefills the key for the CONFIGURED provider (was a `claude ?? openai ?? gemini` fallback chain that prefilled the wrong key). Save preserves the other providers' stored keys (`provider==='X' ? apiKey || undefined : campaign.aiDm?.XApiKey`) instead of nulling them — switching provider no longer destroys saved keys. Honest gated Save: `saving` busy state (`<Button disabled={saving}>`, "Saving..." label); the `configure` result is checked (`if (!res.success) throw`); `saveCampaign` + configure wrapped in one try → on any failure `addToast(saveFailed, 'error')` and the modal STAYS open; closes only on full success. `providerReady` wired via `onProviderReady={setProviderReady}` + an inline amber `notReadyWarning` when `enabled && !providerReady` (informative, not hard-gating — detection probes the saved URL). New i18n keys (en+es): `saving`, `saveFailed`, `notReadyWarning`. Tests (happy-dom): gemini campaign with both keys prefills `gk`; Save preserves `geminiApiKey`; `configure {success:false}` → toast + modal open; pending `saveCampaign` → Save disabled. tsc web clean; 4 tests green.
- **10G (2026-06-11):** `AiProviderSetup.tsx` — every failure mode visible/recoverable; cloud gate means "key works". F7: `detectError` state; `detectOllamaStatus` catch sets it + a dedicated red retry block in the Ollama section (Retry detection → `detectOllamaStatus()`, NOT `handleAutoSetup` which would download off stale state); the message renders once (the catch no longer also sets the generic `errorMessage`). F8: `cloudModelsState` (`idle|loading|ready|error`); the cloud-models fetch extracted into a `loadCloudModels` callback (loading→ready→error); the model `<select>` shows a disabled "Loading models..." while pending, a `modelsError` line + retry on failure, a `modelsEmpty` hint when ready+empty, and option text `m.desc ? '${name} — ${desc}' : name` (kills the dangling "—"). F9: cloud readiness now means VALIDATED — the readiness effect's cloud branch sets `onProviderReady(keyValid === true)` (no key → false), and a debounced (600ms) auto-validate effect runs `handleValidateKey` (now `useCallback`) when `keyValid === null`; a garbage key keeps the wizard's Next disabled (CampaignWizard unchanged). The `onOllamaProgress` effect (PHASE-05 05D) is untouched. New i18n keys (en+es): `errorDetect`, `retryDetect`, `modelsLoading`, `modelsError`, `modelsEmpty`. Tests (extend PHASE-05's `AiProviderSetup.test.tsx`): detect-reject → retryable error (retry re-invokes detection); dropdown loading/error/empty + no-dangling-separator; fake-timer validation gating (invalid key → last `onProviderReady(false)`, valid key → eventual `true`). tsc web clean; 10 tests green.
- **10F (2026-06-11):** `AiContextPanel.tsx` — new `listError` state (`refresh` clears it at start, sets it in the catch alongside `setFiles([])`); render shows a red `loadFailed` line when `listError && !loading` instead of the `noFiles` empty state (the existing Refresh button is the retry). `handleClear` replaces the silent catch with `addToast(t('...clearFailed'), 'error')` and, on success, calls `refresh()` (truth from disk) instead of an optimistic `setFiles([])`. New i18n keys (en+es): `loadFailed`, `clearFailed`. Tests (happy-dom): list-reject → loadFailed (not noFiles); list-empty → noFiles; clear-reject → error toast; clear-success → `listMemoryFiles` re-invoked. tsc web clean; 4 tests green. (Note: augment `window.api` directly in happy-dom tests — `vi.stubGlobal('window', …)` nukes `document`.)
- **10E (2026-06-11):** Store gains `clearLastError: () => set({ lastError: null })` (type + impl, additive). `ChatPanel.tsx` records `lastAiRouteRef = { message, senderName }` when the AI-routing branch fires in `handleSend`; the red error line is now a flex row with a `×` dismiss button (`aria-label` `aiErrorDismiss` → `clearLastError`) and a `Retry` button (`aiErrorRetry`) gated on `lastAiRouteRef != null && campaign && (role==='none'||'host') && aiEnabled && !aiPaused` that clears the error then re-routes via `routePlayerMessageToAiDm` (fresh context rebuilt by the routing fn; known wart noted in a comment — a retry can duplicate the user line). New i18n keys (en+es): `aiErrorDismiss`, `aiErrorRetry`. Tests: store `clearLastError` nulls only `lastError`. tsc web clean; 47 tests green.
- **10D (2026-06-11):** New `utils/stream-preview.ts` `sanitizeStreamPreview(raw)` — cuts at the earliest block marker (`[STAT_CHANGES]`/`[DM_ACTIONS]`/`[RULE_CITATION`/`[RULING`), strips complete `[NPC:]`/`[EMOTION:]` voice tags, trims a trailing partial marker (`/\[[A-Za-z_]{0,16}$/`), collapses doubled spaces (newlines preserved). `ChatPanel.tsx` renders `sanitizeStreamPreview(aiStreamingText).slice(-200)`. Stick-to-bottom auto-scroll: a `stickToBottomRef` updated by an `onScroll` handler on the scroll container (`scrollHeight - scrollTop - clientHeight < 48`), plus an effect on `[aiIsTyping, aiStreamingText]` that sets `scrollTop = scrollHeight` only when still stuck (the indicator/preview live outside the virtualizer, so direct scroll is the only mechanism; the existing `chatMessages.length` effect is untouched). Tests: `stream-preview.test.ts` (passthrough, each marker cut, partial-trim, voice-tag strip + space collapse, earliest-of-many, and a property test asserting no machine substring leaks for ANY streaming split point). tsc web clean; 11 tests green.
- **10C (2026-06-11):** New `AI_GET_TOKEN_METER: 'ai:get-token-meter'` channel (`ipc-channels.ts`); main handler (`ai-handlers.ts`) returns `{ conversationBudget: getEffectiveBudgets().conversationHistory, contextWindow: getActiveContextWindow() }` (PHASE-01 exports) — honest "best current knowledge" (raw budget before the first Ollama stream sets the window). Preload `getTokenMeter()` (`index.ts` + typed in `index.d.ts`). `useAiReadiness` now fetches the meter inside the probe (one extra cheap IPC per cadence, `.catch(()=>null)` so a meter failure never fails readiness) and exposes `conversationBudget: number | null`; `ChatPanel` passes it to `AiDmStatusBar` as `maxTokens` (the `null` 10B stand-in is gone). `AiDmStatusBar` already renders the `~used / max` form, the `tokensUsedOnly` fallback when null, and the amber tint when `used > max`. `tokensTitle` (both locales) rewritten to explain the comparison. Tests: `use-ai-readiness.test.ts` gains budget-exposed + meter-fails-but-readiness-resolves cases; `ai-handlers.test.ts` gains an `AI_GET_TOKEN_METER` registration test (returns positive numbers). tsc web+node clean; 51 tests green. Acceptance: no `23,000`/`23.000` literal in any locale.
- **10B (2026-06-11):** New `useAiReadiness(active)` hook (`hooks/use-ai-readiness.ts`) — moves ChatPanel's one-shot probe into a reusable hook that distinguishes still-checking (`usable=null`, `probeFailed=false`) from check-failed (`probeFailed=true`) — the old code collapsed both to null and rendered green; re-probes every 30s while `document.visibilityState==='visible'`, skips in-flight ticks, exposes `recheck()`. New presentational `AiDmStatusBar` (`components/game/bottom/AiDmStatusBar.tsx`) with the render precedence typing→paused→checking→not-ready(provider-appropriate: ollama `aiNoModel` vs cloud `aiProviderUnavailable`)→ready; the dot/label row is a click-to-recheck button; kept presentational for PHASE-14. `ChatPanel.tsx` deletes the inline probe state/effect + status-bar JSX, renders `<AiDmStatusBar>` wired to the hook + `aiPaused`, and `recheck()`s when `aiLastError` goes non-null. New i18n keys (en+es): `aiPaused`, `aiChecking`, `aiProviderUnavailable`, `aiRecheckTitle`, `tokensUsedOnly`; `tokens` rewritten to `~{{used}} / {{max}} tokens` (the `23,000` literal removed early — ChatPanel passes `maxTokens=null` ⇒ used-only form until 10C wires the IPC). Tests: `AiDmStatusBar.test.tsx` (one per precedence rule + recheck click + token amber-over-budget), `use-ai-readiness.test.ts` (fake timers: initial probe, 30s re-probe, hidden-skip, probeFailed-on-error, inactive=no-probe). tsc web clean; 15 tests green; i18n gates + ChatPanel smoke green.
- **10A (2026-06-11):** New `src/shared/ai-defaults.ts` — `DEFAULT_PROVIDER_MODELS` (ollama/claude/openai/gemini) + `DEFAULT_AI_MODEL` (= ollama default), single source of truth. `ai-service.ts` now `import { DEFAULT_AI_MODEL } from '../../shared/ai-defaults'` + `export { DEFAULT_AI_MODEL }` (keeps `ai-vision.ts`'s `require(...).DEFAULT_AI_MODEL` stable); `app-constants.ts` re-exports it (`export { DEFAULT_AI_MODEL } from '../../../shared/ai-defaults'`, importers unchanged); `AiProviderSetup.tsx` provider-switch defaults read `DEFAULT_PROVIDER_MODELS[p]` (inline literal deleted). Truthful labels: `DMTabPanel.tsx` and `SettingsDropdown.tsx`'s `AiDmSettingsSection` (now takes an `aiDm` prop, dropped the `capitalize` class) compute `aiDm?.model ?? aiDm?.ollamaModel ?? AI_PROVIDER_LABELS[aiDm?.provider ?? 'ollama'] ?? 'AI'` instead of the hardcoded `'Ollama'`. Tests: `ai-defaults.test.ts` (4 keys, non-empty, DEFAULT_AI_MODEL=ollama); happy-dom render tests for both label surfaces (Gemini campaign shows `gemini-2.0-flash`, never `Ollama`; provider-label fallback). tsc web+node clean; 8 tests green. Acceptance greps: no `= 'Ollama'` in renderer; no default-model literal outside the shared module (the remaining `llama3.2:3b` hits are the curated installable-model catalog in `ollama-context.ts` + a `PlayerList` comment, not default definitions).
