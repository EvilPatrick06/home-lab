...
Executed 2026-06-18. PHASE-16 had already refactored `app.py` to 2695 lines (all line cites re-verified by symbol). Everything is dormant/opt-in — no live services restarted, no live `.git/hooks` modified. Gate: full bmo pytest **1177 passed, 6 skipped** (the 6 = pre-existing `bmo/docker/deploy.sh` shell tests, that legacy file stays absent); `compileall app.py` clean; both new workflows parse. Authoritative CI = `bmo / pi pytest`.

- **42A — canary boot mode.** `app.py`: `BMO_PORT`/`BMO_CANARY` module constants; `socketio.run(port=BMO_PORT)` + a CANARY log line in `__main__`; `init_services()` gates all 12 hardware/service blocks (LED/OLED/Voice/Camera/Smart-home/Calendar/Location/Weather/Audio/Music/Timers) + the Agent on `BMO_CANARY` with the import-only pattern (each `from X import Y` still runs as a deploy-break check; globals end `None`), then an early `return` before the unconditional tail side-effects (wpctl mic-gain, pollers, threads — also wrapped) so canary touches no hardware/audio/alarms/pollers. NEW `tests/test_canary_mode.py` (5 cases: skips-hardware, BMO_PORT env, /health route, no-shell-out). Default boot path behaviorally byte-identical. test_app_endpoints still 59 green.
- **42B — `deploy.sh`.** NEW `bmo/pi/scripts/deploy.sh` (`set -euo pipefail`; flags `[SHA] [--dry-run] [--services-only] [--no-canary]`; env knobs): 13 gates — flock lock → repo-root check → clean-tree → branch=master → resolve+ancestor-validate target (idempotent "already deployed") → ff-only merge → conditional pip → `compileall` → canary :5002 `/health` (trap teardown) → selective systemd restart (bots/fan/bmo by changed paths) → :5000 health gate → `rollback()` (git reset --hard OLD_SHA + reinstall + restart + re-poll). `run()` wrapper makes `--dry-run` fully side-effect-free. `setup-bmo.sh` §12 hook → non-restarting LFS-chaining hook (SOURCE only). NEW `tests/test_deploy_script.py` (12 hermetic dry-run cases) + `test_shell_scripts.py` extended (shellcheck/`bash -n`). shellcheck clean.
- **42C — `bmo-deploy.yml`.** NEW `.github/workflows/bmo-deploy.yml`: `workflow_run` after `bmo / pi pytest` green on a master push → gate job (skips green with a `::notice::` when TS secrets absent; auto-deploy needs `vars.BMO_AUTO_DEPLOY=='true'`) → deploy job (ephemeral Tailscale join → SSH → `deploy.sh <head_sha>`). `workflow_dispatch` for manual. NEW `tests/test_deploy_workflow.py` (6 drift guards: workflows-list matches `bmo / pi pytest`, permissions contents:read, gate `if` clauses, deploy refs tag:ci + deploy.sh path).
- **42D — Docker option (off by default).** NEW `bmo/docker/{Dockerfile,compose.yml,README.md}` + `bmo/.dockerignore` (python:3.11-slim, CPU torch, `/health` HEALTHCHECK, host networking, OLED/camera disabled, device-passthrough commented; README "optional path — host venv is supported"). NEW `.github/workflows/bmo-docker-build.yml` (arm64 `ubuntu-24.04-arm`, `push:false`, `REQUIREMENTS=requirements-ci.txt`). NEW `tests/test_docker_deploy_files.py` (5 cases). Never wired into setup-bmo.sh/systemd.
- **42E — docs.** `DEPLOY.md`: "Future improvements" dangling pointer → full "Automated deploy" section (deploy.sh flags/knobs/gate-order, canary semantics + why-not-traffic-swap, CI trigger chain, `BMO_AUTO_DEPLOY` opt-in, Docker link) + a console-only "CI deploy provisioning" checklist (Tailscale tagOwners/ACL/SSH rules, Pi tag, OAuth → `gh secret set`, the auto-deploy toggle). `SERVICES.md` :5002 row → "app.py (canary)". `SYSTEMD.md` → deploy.sh as preferred restart-after-update + post-merge-hook note.

**Deferred to the user (console/live-host, by design — warn-before-live-change):** (1) write the new non-restarting `post-merge` hook to the live `/home/patrick/home-lab/.git/hooks/` (re-run setup-bmo.sh §12 or copy it); (2) `BMO_AUTO_DEPLOY` repo variable — could not set from here (`gh variable` subcommand absent in the installed gh; unset already == false in the gate, so the workflow is correctly dormant — set to `"true"` via the GitHub web UI / newer gh only when opting into auto-deploy-on-merge); (3) Tailscale ACLs/tags + `TS_OAUTH_*` secrets per the DEPLOY.md checklist. Until (3), `bmo / deploy` skips green (dormant).
