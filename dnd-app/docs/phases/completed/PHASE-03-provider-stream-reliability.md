...
- **03A (2026-06-11):** `llm-provider.ts` — added `CLOUD_FIRST_TOKEN_TIMEOUT_MS`/`CLOUD_INACTIVITY_TIMEOUT_MS` (90s each) + `createStreamInactivityGuard()` (first-token + inter-token guard mirroring the Ollama pattern: `signal`/`bump`/`clear`/`timedOut`). Rewrote the stale `PROVIDER_REQUEST_TIMEOUT_MS` doc comment (now non-streaming-only; corrected the "120s" renderer reference → 330s inactivity backstop). New `llm-provider.test.ts` (5 cases, fake timers: first-token abort, bump-defer, clear, caller-signal-not-timedOut, defaults). tsc node clean.
- **03D (2026-06-11):** `gemini-client.ts streamChat` drops the model-level `timeout` and passes the guard signal per-request (`sendMessageStream(content, { signal })`, supported by @google/generative-ai 0.24.1); bump/clear/timed-out path identical. `chatOnce` keeps the model-level timeout. Stale "120s"/"no signal" comments corrected. New `gemini-client.test.ts` (3: no-model-timeout+per-request-signal, chatOnce-keeps-timeout, timed-out). tsc node clean.
- **03C (2026-06-11):** `openai-client.ts` — `max_tokens`→`max_completion_tokens` (both methods), `maxTokens?` param wired into both signatures, `rejectsSystemRole`+`buildMessages` fold the system prompt into the first user message for o1-mini/o1-preview (system role kept for everything else), streaming uses the inactivity guard with a "timed out" onError. New `openai-client.test.ts` (6: max_completion_tokens/maxTokens, o1-mini fold, gpt-4o system kept, signal+stream, timed-out, listModels regression). No `max_tokens` remains. tsc node clean.
- **03B (2026-06-11):** `claude-client.ts streamChat` uses the guard (`{ signal: guard.signal }` + `stream.on('streamEvent', bump)` + `guard.clear()` on done/catch); catch surfaces a "timed out" onError when `guard.timedOut()` (no-retry contract). `chatOnce` keeps `withRequestTimeout` (non-streaming ceiling). Test mock upgraded to capture options + listeners + a hang-until-abort mode; added signal-passed/streamEvent-registered/timed-out tests. 6 tests green; tsc node clean.
- **03E (2026-06-11):** `ollama-client.ts` — new `fetchOllamaModels()` (THROWS on network failure/timeout/non-OK, 5s `AbortSignal.timeout`, `OLLAMA_LIST_TIMEOUT_MS = 5000`); `listOllamaModels()` re-based on it (lenient, `[]`-on-failure for UI paths). `ai-service.ts resolveOllamaModel` now imports `fetchOllamaModels`, wraps it in try/catch — unreachable → throws "Cannot reach Ollama at <url> (<detail>)…"; empty list keeps "No Ollama models installed". `ollama-client.test.ts`/`ai-service.test.ts` mocks + assertions updated (resolveOllamaModel repointed to `fetchOllamaModels`, added unreachable-host case). 71 tests green; tsc node clean.
- **03F (2026-06-11):** `ollama-manager.ts` — `pullModel`/`deleteModel`/`listInstalledModelsDetailed` swapped `OLLAMA_BASE_URL`→`getOllamaUrl()` (registry ops honor the configured remote URL); imported `getOllamaUrl`/`listOllamaModels` from `./ollama-client`; deliberate-split comment above `detectOllama` (registry ops = configured URL; binary lifecycle detect/start/stop/version = localhost, 4 `OLLAMA_BASE_URL` sites kept). `ollama-manager.test.ts`: mocked `getOllamaUrl` → `http://remote-gpu:11434`, added "registry ops target the configured URL; binary-lifecycle stays localhost" test, updated the existing deleteModel test to the configured URL. 41 tests green; tsc node clean.
- **03G (2026-06-11):** `ai-service.ts` — extracted the disk read out of `getConfig()` into a new exported `loadConfigFromDisk()` (the single startup load point; `initFromSavedConfig` calls it first); `getConfig()` is now a pure in-memory snapshot (no `readFileSync`), so `resolveOllamaModel`'s in-memory model auto-switch (`currentConfig.model = picked`) no longer reverts on ChatPanel mount / AI-settings open / map analysis. `chatOnce` resolves the model first (`await resolveOllamaModel(currentConfig.model)`, no streamId → no notice). `ai-vision.ts getModelForProvider` is now async and resolves through `resolveOllamaModel`. `ai-service.test.ts`: four disk tests moved to a `loadConfigFromDisk` describe; added getConfig-no-disk + auto-switch-survives tests. 40 tests green.
