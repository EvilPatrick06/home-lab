...
> **33g DONE — 2026-05-29 (resumed "do it all").** Deleted the dead `use-network-store.ts` re-export barrel (zero importers); the 28e.5 forbidden-pattern rule guards against re-creating it. 33h: the content-vs-schema validator is stale against the post-Phase-15 data layout (per-class files, split feat dirs, conditions relocated) — needs a focused re-map of all 10 schema↔file pairs + wrapper schemas before it can gate check:full; 20 shape mismatches catalogued by `npx tsx scripts/audit/validate-content-vs-schemas.ts`.
