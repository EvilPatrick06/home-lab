...
> **PHASE 23 UPDATE — 2026-05-29 (resumed; "do them all" directive).** Additional items landed gate-green (vitest 6520): 23b (spell search + Ritual/Conc./Prepared filters), 23f (single attunement source — card toggle writes state.magicItemAttuned, AttunementTracker + MagicItemsPanel derive from getEffectiveMagicItems), 23j (HP damage/heal/temp helper), 23k (consumable Use button), 23l (sheet Initiative roll), 23c-core (updateCharacterInState + dm:character-update writes the canonical character store). STILL PARTIAL/DEFERRED (need two-window or large refactor, but no longer hard-blocked): 23a list virtualization, 23c full remoteCharacters removal (45 sites), 23d conflict banner, 23e remaining section memo, 23g optimistic-save rollback, 23i editor-hook standardization, 23n condition-sync message + QuickActions component.
