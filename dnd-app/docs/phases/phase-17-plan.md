...
> **PHASE 17 COMPLETE — 2026-05-29.** All critical/high live work across 17a–17f is done; 17g remains a catalogue for opportunistic cleanup. Final 4-gate green: biome lint (0 errors, pre-existing 17g warnings only), `tsc --noEmit` web + node (0 errors), vitest 6477/6477. Closing items this session: **17c** LOG-1 (Champion crit threshold in attack-resolver ×2 + combat-resolver), LOG-11 (creature cover filters dead/ally + clamp to half; Tiny exclusion deferred — no size category on `MapToken`, logged to ISSUES-LOG-DNDAPP). **17d** NET-5 (audited every broadcast `JSON.stringify` — all already try-catch-wrapped; added circular-ref test), NET-6/29/30 (`src/main/ipc/_safe.ts` `safeHandler`/`handle` wrapper migrated across all 107 handlers in ai/storage/plugin handler files — success returns pass through, throws normalize to `{success:false,error}`), NET-10 (`ai-service.configure` async via `atomicWriteFile`), NET-11 (memory-manager per-file mutex `mutate<T>`), NET-17 (dead AI_CONNECTION_STATUS handler removed), NET-19/20 (parsed.data plumbing), RUN-15 (uncaughtException/unhandledRejection → fatal dialog + `app.exit(1)`). **17e** GUI-4 (dispose throwaway cannon-es source geometries + `disposeDie` chokepoint). Prior sessions closed 17a (security), 17b (crash prevention), the 17c quick-wins, the 17d `.catch` batch, and the 17e subscription/timeout/escape items.
...
> **17d PARTIAL — DONE 2026-05-29 (silent-failure `.catch` batch):** RUN-2 (`selection-slice` 4 `load5e*().then` chains → `.catch` logger.warn), RUN-6 (`character-details-slice` `setClassEquipmentChoice` → `.catch`), RUN-3/4 (`use-ai-dm-store`: `game-action-executor` dynamic import → `.catch` + `pushDmAlert('error')`; `loadConversation` → `.catch`), RUN-5 (`use-game-effects`: `creature-mutations` + `game-action-executor` dynamic imports → `.catch` logger.error), RUN-7 (dialog null-window guard — landed in 17b). **REMAINING 17d:** NET-4 (client-manager post-connection peer errors), NET-6/29/30 (`safeHandler` wrapper across ~70 IPC handlers — large), NET-5 (broadcast `JSON.stringify` guards — coordinates with Phase 30), NET-10/11/17/19/20 (ai-service async configure, memory-manager mutex, dead handler, parsed.data plumbing), RUN-15 (uncaughtException policy).
...
> **17e PARTIAL — DONE 2026-05-29 (subscription + timeout leaks):** GUI-2 (`DmAlertTray` subscribes in `useEffect`, not a `useState` initializer — cleanup now runs, listeners no longer accumulate), GUI-3 (`DiceOverlay` tracks both nested roll-dismiss `setTimeout`s in a ref + clears on unmount), GUI-11 (`ShopView` tracks the 10s haggle auto-resolve timeout + clears on unmount/re-haggle). Also DONE: GUI-7 (RulingApprovalModal Escape/backdrop/Dismiss), GUI-9 (`Modal.tsx` header/scroll split), **GUI-8** (new `hooks/use-escape-key.ts` applied to every hand-rolled modal: Whisper, LightSource, TimeEdit, NarrowModalShell (covers its consumers), SentientItem, NetworkSettings, Handout, SharedJournal, DMNotes; ConfirmDialog already inherits Escape via the shared `Modal`). **REMAINING 17e:** GUI-4 (dice-textures CanvasTexture + cannon-es geometry dispose audit) — the one item left.
...
- **17c (game logic) — PARTIAL 2026-05-29 (quick wins done; deeper combat items remain).** DONE: LOG-2 (`doubleDiceInFormula` `g` flag — every dice group doubles on crit), LOG-8 (removed the 2014 "exhaustion 6 = death" trigger from `conditions-slice` add/update + dead helper; 2024 uses the cumulative −2 in `attack-condition-effects`), LOG-10 (`removeFromInitiative` tracks the active entry by id, not index — removing a pre-active entry no longer mis-advances the turn), LOG-12/TYP-4 (`action-validator` `remove_from_initiative` reads `entityName`, not the bogus `{label}` cast). 4-gate green (vitest 6471). Also DONE 2026-05-29: LOG-3 (`isInMeleeRange` iterates every occupied cell of attacker + target like `isAdjacent`, so a Large creature reaches from its nearest cell, not its origin), LOG-13 (`executeNextTurn` calls `nextTurn()` FIRST then reads the post-advance `currentIndex` from the store, so legendary-action resets + recharge rolls target the creature whose turn actually started — 2 specs updated to the corrected behavior). **REMAINING (deeper):** LOG-1 (Champion crit threshold — the resolver paths pass a sidebar stat block, not `Character5e`; needs a `Character5e` in scope at each crit site before wiring `getCritThreshold`), LOG-4 (AoE saves + target modifier — needs a `getCreatureSaveMod` helper), LOG-5 (cone geometry — `findTokensInArea` has no direction param; needs threading direction through callers), LOG-7 (thrown-at-range), LOG-14/15 done (combat data warmed at boot via preloadAllData). These are 17c's remaining live work.
