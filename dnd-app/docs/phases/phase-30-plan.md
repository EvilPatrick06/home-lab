...
- 30c — DONE (`stores/network-store/index.ts`) — host inbound dispatch now flows through `GameAuthority` over a `P2PTransport`; the whole `handleHostMessage` switch is the default handler (behavior-preserving — `host-connection` still validates upstream, broadcasts unchanged). Full vitest 6740 green.
- 30d — DONE — decoupled DM authority from the network host: added `PeerInfo.isDM` (falls back to `isHost`, so legacy host=DM is unchanged) and made `resolvePeerRoleId` honor `isDM ?? isHost`, so every `hasPermission` gate now reflects DM authority not network role. A 5-agent audit of all 84 raw `isHost`/`isCoDM`/`role==='host'` sites confirmed the DM-authority gates were already routed through `hasPermission`/`localHasPermission` (29e); the rest are genuine network-host transport gates (left) or boolean-only props with no peer/campaign in scope. Converted the one remaining DM-authority raw gate — the `game:state-update` per-peer view bucket in `network-store/index.ts` — to the legacy-preserving `isHost || isCoDM || hasPermission(...)` pattern. Full vitest 6740 green.
- 30e — DONE — `dm:transfer-dm` DM-authority transfer protocol: new message type + schema; `network-store.transferDm(peerId)` (host applies `isDM` to every peer + flips its own `localIsDM` + broadcasts); client-handler applies it; `buildLocalPeer`/`localHasPermission` honor `isDM` (incl. the legacy no-permissions fallback) so a host who transferred DM away loses DM tools and the new DM gains them; "Make DM" button in the lobby PlayerCard. +tests across client-handlers / local-permission / network-store. Full vitest 6749 green.
- 30f/30g/30i — satisfied by the back-compat design: clients re-point DM authority via the `dm:transfer-dm` handler (30f); the network host stays put so authoritative state persists + existing `game:state-resync` covers reconnect (30g); `isDM` defaults to `isHost` so existing sessions/saves need no migration (30i).
- **Network-host (PeerJS mesh owner) MIGRATION** — moving which peer owns the mesh mid-session is a reconnect-orchestration concern that Phase 32 (persistent Pi host over a WebSocket transport) solves at the architecture level; the P2P seam (TransportAdapter/GameAuthority) built here is exactly what Phase 32 swaps onto. Tracked under Phase 32.
