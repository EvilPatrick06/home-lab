...
> **31a/31b FOUNDATIONS LANDED — 2026-05-29 (resumed "do them all").** `network/sync/shard.ts` (Shard<T>+Delta), `network/sync/registry.ts` (registerShard/getShards/findShard), and `network/sync/diff.ts` (structuralDiff/applyDelta with order-exact array patch + fuzzed round-trip test) are in and gate-green. The broadcaster/applier (31c/d) mount inside Phase 30's GameAuthority and the per-feature shard migrations (31e–k) follow — both need the Phase-30 core + two-peer verification.
