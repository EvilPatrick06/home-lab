...
> _Not yet implemented — authored 2026-07-02 by phase-maker from the 2026-07-02 v2.7.1 WEB QA report. Both findings verified live + in source exactly as reported (no premise corrections needed this time); authoring added two precision guards — the immutable header must scope to the hashed `assets/` subtree only (stable-named `data/`/`sw.js`/manifest keep revalidating), and the SPA fallback is `no-cache`-safe by construction via `_cache_policy`'s HTML assign — plus the verified fact that the deployed shell references zero external script/style origins, which is what makes 63B's drops safe to propose. Pi-side Flask change only; no dnd-app rebuild or redeploy. To be implemented by the phase-executer per INSTRUCTIONS.md._
