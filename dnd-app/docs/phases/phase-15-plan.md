...
- 15a Step 10 — PARTIAL — merge.ts shipped; hydration hooks (`use-library-entry.ts`) deferred because they bind to `useLibraryStore.getEntry`/`getEntries` which land with Step 7 (truth-store rewrite).
...
Steps 7-9, 12-19 (excluding 19 board-test, which still needs consumer flip) deferred to next 15a iteration. Verified 2026-05-19: truth-store rewrite, library-service plug-in, data-store flip, hydration hooks, migration v4, report modal, and boundary test still pending — see the relevant Sub-Phase Details sections.
