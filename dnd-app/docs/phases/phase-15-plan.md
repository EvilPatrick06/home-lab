...
(none — no Phase 15 work has landed. Verified 2026-05-19: `src/renderer/src/types/library.ts` has no `EntryRef`/`DeepPartial`/`LibraryEntry`/`useHydratedRef`; `src/renderer/src/services/library/` contains only `content-index.ts` + `drag-data.ts` — no `schemas/`, `merge.ts`, `use-library-entry.ts`, `library-boundary.test.ts`, or `README.md`; `src/renderer/src/stores/use-library-store.ts:29-152` is still UI-only state (`selectedCategory`, `searchQuery`, `recentlyViewed`, `favorites`); `src/renderer/src/stores/use-library-ui-store.ts` does not exist; `src/main/storage/migrations.ts:1` still reads `CURRENT_SCHEMA_VERSION = 3` with no `MIGRATIONS[4]`; `src/shared/migrations/v4-character-refs.ts` and `src/main/storage/snapshot.ts` do not exist; `src/renderer/src/components/library/MigrationReportModal.tsx` does not exist; `src/renderer/src/components/bastion/` does not exist; `src/renderer/src/data/personality-tables.ts` + `src/renderer/src/stores/use-data-store.ts` still present.)
