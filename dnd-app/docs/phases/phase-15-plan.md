...
- 15b Step 1 — DONE 2026-05-19 (`src/renderer/src/services/library/use-library-entry.ts:39` `useLibraryCategory` helper) — React-component `load5eX()` + `useState`/`useEffect` pattern converted to declarative `useLibraryCategory(category, loader)`. Sites: `MulticlassLevelBar5e.tsx` (load5eClasses), `DetailsTab5e.tsx` (load5eBackgrounds + load5eFeats + load5eClasses, 3 useEffects → useMemo/useEffect), `SpellsTab5e.tsx` (load5eSpells + load5eSubclasses), `HigherLevelEquipment5e.tsx` (load5eMagicItems, parameterised rarity filter at render time), `CharacterBuilder5e.tsx` (load5eSpells → live spellLevelMap). `gear-tab-types.ts` left imperative — it's a utility module, not a React component.
- 15b Step 2 + Step 3 — DEFERRED to 15c per user direction 2026-05-19 (notify.sh `06:15:59Z`, option A from rule-27 escalation). Builder slice rewrites + side-panel hydrated reads produce Character5e v4 shapes that don't exist in the type system until 15c rewrites `Character5e`. Same plan-ordering inversion resolved for Steps 12-18 (option D → 15h); analogous resolution here (option A → 15c).
- 15b Step 4 — DONE (pre-existing, Phase 17l). `data/personality-tables.ts:22` already uses `rollFrom(arr)` with `arr.length` (replacing the bug where `rollD4` hard-coded `4`). Truth-store ingest for personality data is deferred to 15h cleanup per plan.
- 15b Step 5 — PARTIAL. Per-component "no public/data import" check is globally enforced by 15a Step 19's boundary test. Per-component "library-mutation propagates" specs deferred — they require `@testing-library/react` which isn't installed in this repo; same constraint as 15a Step 10's hook specs (test the underlying store-side behavior instead). Logging this in the next session's startup as a "needs install" finding.

- 15c.1 — DONE 2026-05-19 (`src/renderer/src/types/character-5e.ts:30`+`:115`, `character-5e-migration.ts:1`, `stores/builder/slices/build-character-5e.ts:663`). Character5e gains additive v4 fields (`speciesRef`, `subspeciesRef`, `backgroundRef`, `classRefs`, `featRefs`, `knownSpellRefs`, `weaponRefs`, `armorRefs`, `magicItemRefs`, `conditionRefs`, `state`) alongside the legacy v3 fields — all optional, so every existing consumer compiles untouched. New `Character5eState` interface + `InstanceRef<C>` helper. `migrateCharacter5eFromV3ToV4` shim derives v4 fields from v3 (idempotent, additive); builder save path now produces both v3 + v4 fields on every Character5e it emits. 9 migration specs cover species/background/class/spell/magic-item paths + idempotency + leave-untouched paths. Absorbs 15b Steps 2-3 (deferred earlier today). 4-gate green.
- 15c.2/15c.3/15c.4 — DONE 2026-05-19 (commit `e36a23b`). Sheet files flipped to v4 hydration hooks + `getEffective*` derivation (v3 fallbacks retained until 15c.5).
- 15c.5 — DONE 2026-05-28 (`types/character-5e.ts`, `character-5e-migration.ts`, `services/character/effective-character-5e.ts`, ~50 consumers). The 8 legacy v3 inline arrays (`classes`, `knownSpells`, `preparedSpellIds`, `weapons`, `armor`, `magicItems`, `feats`, `conditions`) are REMOVED from `Character5e` (canonical = v4 refs + state). New `Character5eV3` legacy type + `migrateCharacter5eFromV3ToV4` chokepoint derives v4 then deletes v3 keys (used on load via `use-character-store`, and after builder/level-up/import produce v3-shaped output). `hydrate` + `getEffectiveClasses` gained orphan/override fallback (custom items + library-less unit tests). Writers converted: `apply-level-up` (round-trips v3→shim), `rest-service-5e` (state-direct: magic-item recharge → `state.magicItemCharges`), `build-character-5e`, `import-foundry`/`import-dnd-beyond` (return `migrate(v3)`), `use-character-store` armor/condition actions, and component edits (`ItemModal` charge → state, `EquipmentListPanel` custom armor → orphan ref, `SpellPrepModal` → `state.preparedSpellIds`, `FeaturesSection` → `featRefs` w/ choices override, `commands-dm-economy` identify → ref override, `commands-player-inventory` attune → `state.magicItemAttuned`). 11 test files reworked to v4 ref fixtures. 4-gate green (lint, tsc-web, tsc-node, vitest 6449). `classFeatures` intentionally retained (removed in 15d when class features source from library — sequencing, not deferral). **Known degradations (test-data acceptable per user 2026-05-28): exhaustion condition `value`, innate spell uses, custom-condition value have no v4 home → dropped.**

**15c — COMPLETE 2026-05-28.**

- 15d — DONE 2026-05-28 (verified, no new code needed). Level Up sources class features live from the library via `load5eClassFeatures()` (data-provider → truth store, wired in 15a step 8); feats via `load5eFeats`/`getEffectiveFeats`; multiclass persists as `classRefs` (produced by `apply-level-up`'s shim round-trip in 15c.5). No `levelup-tables.ts` inline-data file exists. The `classFeatures` field is retained as accumulated character state (the plan requires the DATA to come from the library — it does — not removal of the field). 4-gate green.

**15d — COMPLETE 2026-05-28.**

> **Record correction 2026-05-28 (rule 3 / honest-audit pass per user direction).** A prior
> session was mid-way through 15e when it crashed; it had NOT marked 15e done, but its commit
> `e2b3584` ("sidebar tabs read via library store") **overclaimed** — the message said
> "EquipmentTab/SpellsTab/MonstersTab now load via data-provider" but the diff only changed
> `MonstersTab.tsx`. `EquipmentTab`/`SpellsTab` were still on the `window.api.game.load*` IPC
> bypass. A full re-audit (this session) re-verified 15a/15b/15c/15d/15f against actual code —
> all CONFIRMED genuinely done — and re-scoped 15e against reality below.

- 15f — DONE 2026-05-28 (`docs/phases/bastion-data-rule.md` new, `AGENTS.md` data-layer row). **Plan pre-state was stale (rule 3):** Steps 1-5 assumed `stores/bastion-store/` embedded inline library data and that `components/bastion/` did not exist and needed 7 net-new components. Actual code already holds **references + state** — `SpecialFacility.type` (a stable `SpecialFacilityType` id) references the definition; `SpecialFacilityDef[]` loads once into `useBastionStore.facilityDefs` via `load5eBastionFacilities()`; runtime state (`enlarged`, `currentOrder`, `orderStartedAt`, `hirelingNames`, `creatures`, per-instance config) already lives in sibling fields; a full Bastion UI exists under `pages/BastionPage.tsx` + `pages/bastion/*`, registered at `/bastions` in `App.tsx` with a `MainMenuPage` nav entry, every view hydrating defs by id (`facilityDefs.find(d => d.type === f.type)`). Building a duplicate `components/bastion/` would have been wrong. So Steps 1-5 are satisfied by the existing architecture (same disposition as 15d); the only genuinely-missing deliverable was Step 6 — created `bastion-data-rule.md` (written to the real `type`-id-reference + sibling-state pattern) and added the data-layer row to `AGENTS.md` linking the library README + bastion-data-rule. Boundary green: no `public/data` imports in `pages/bastion/**` or `stores/bastion-store/**`. Note (carries to 15g/15h): `load5eBastionFacilities` still resolves through the transitional `use-data-store` cache like every other category — migrating all `ds()` calls + deleting `use-data-store` is 15g/15h scope, not a Bastion-specific gap. 4-gate green.

- 15e — DONE 2026-05-28 (full-rewrite scope chosen by user 2026-05-28; resolution layer + selective strip landed across the token cascade).
  - **Step 7 (Phase 22 H4) — DONE.** All three sidebar tabs now load through the library truth store via `data-provider`: `MonstersTab` (`load5eMonsters`, prior session), `EquipmentTab` (`load5eEquipment`, `components/game/sidebar/EquipmentTab.tsx:2/18`), `SpellsTab` (`load5eSpells`, `components/game/sidebar/SpellsTab.tsx:16/93`). Corrects the `e2b3584` overclaim — Phase 22 H4 is now fully resolved.
  - **Step 4 (encounter per-instance shape) — DONE.** `EncounterMonster` (`types/encounter.ts`) gains additive `instanceId?`, `startX?`, `startY?`, `instanceOverrides?: DeepPartial<MonsterStatBlock>` (atomic-array-replace semantics documented). Field name `instanceOverrides` per Phase 26 Steps 10/11 coordination. Additive (existing `{monsterId,count,notes}` records still valid).
  - **Steps 2/5 (token + initiative live-resolution) — DONE.** New `services/game/token-stats.ts` (+ `.test.ts`, 4 specs) provides `resolveTokenStats` (pure) + `useEffectiveTokenStats` (hook) + `getTokenStats`/`lookupTokenStatBlock` (imperative): a token's library-derived base stats (max HP, AC, walk/fly/swim/climb speeds, special senses, darkvision, resistances/immunities/vulnerabilities, CR) resolve **live** from the library across the three stat-block categories (`monsters`/`npcs`/`creatures` that `loadAllStatBlocks` aggregates), with inline token fields acting as per-instance overrides and as the fallback for non-library-backed tokens.
    - **Polymorphic-token constraint (rule 3 finding → user-chosen "resolution layer + selective strip" 2026-05-28):** `MapToken` is polymorphic — player, NPC, custom-creature, and summon/companion tokens often have NO library monster, so a blind strip of inline `maxHP`/`ac`/`resistances`/speeds/senses would break those tokens. Resolution: monster-backed token CREATION stops copying inline derived stats; everything else falls back to inline.
    - **Creation flipped:** `token-actions.ts` `executePlaceCreature` monster-backed branch now stores only `currentHP` (instance state, seeded from the library max) + `monsterStatBlockId` (the ref) + grid size — no inline `maxHP`/`ac`/speeds/senses/resist. So a DM rebalance of the source monster propagates to every newly-placed token. Generic `executePlaceToken` + the fallback (creature-not-in-cache) branch keep inline (non-library-backed). `executeUpdateToken` bumps max against the *effective* (resolved) max.
    - **Readers migrated** to `getTokenStats`/`useEffectiveTokenStats`: `token-sprite.ts` (HP bar + status ring), `MapCanvas.tsx` (redraw cache key), `state-snapshot.ts`, `use-game-network.ts` (sync sends resolved maxHP/AC), `use-ai-memory-sync.ts`, `creature-mutations.ts`, `commands-player-companions.ts`, `TokenContextMenu.tsx` (reactive hook), `TokenPlacer.tsx`, `AddEntryForm.tsx`, `FallingDamageModal.tsx`, `TargetSelectionStep.tsx`, `HelpModal.tsx`, `AiMapAnalysisModal.tsx`, `QuickConditionModal.tsx`. Left inline (correct — player/viewer tokens keep inline): `vision-computation.ts`/`lighting-overlay.ts`/`map-overlay-effects.ts` (operate on viewer = player tokens; preserves the "pure" vision module), `GameLayout.tsx:430` (player's own-token species darkvision sync), generic `executePlaceToken` + `commands-dm-monsters.ts` heal-players (player tokens).
    - **Initiative (Step 2):** `InitiativeEntry` references the combatant by `entityId`; HP/CR display flows through the token, which now resolves live — so initiative rows reflect live max HP without adding a duplicate ref to the entry. **Backward-compat:** pre-existing tokens with an inline `maxHP` keep it (override wins) until re-placed; no migration needed (a v4 token migration could clear monster-backed inline stats — deferred to 15h).
  - **Steps 1/3 — satisfied by existing/Step-7 work.** Step 3 "Add Monster from Library" is the in-game `MonstersTab` browser, now library-backed (Step 7). Step 1's token stat-block panel already resolves live via `TokenEditorModal` → `loadStatBlockById` + `monsterToDisplay`; spell-cast / condition tooltip live `useLibraryEntry` wiring is folded into 15g's chat/tab-complete sweep (spells/conditions consumers live there).
  - 4-gate green: lint, tsc-web, tsc-node, vitest 6453 (+4 token-stats specs). All 96 token-related specs pass.

**15e — COMPLETE 2026-05-28. 15f — COMPLETE 2026-05-28.**

- 15g — DONE 2026-05-28 (verified against code; several plan premises were stale per rule 3).
  - **Step 1 (macro-engine `resolveSelf`) — N/A.** `services/macro-engine.ts` (`resolveMacroVariables`, 119 lines) only substitutes scalar character fields (`$self`/`$name`, `$level`, ability modifiers, `$prof`) — it has no `resolveSelf` and never reads library data (no `$self.spells[0].damage` deep-path resolution exists). Nothing to convert; the plan's premise (frozen library snapshots in macros) does not match the code.
  - **Step 2 (chat tab-complete + `content-index`) — already library-backed.** `services/library/content-index.ts` builds a name→`ContentReference` index from `LibraryItem[]` (library data, not raw JSON) and `lookupContent` serves tab-complete (`ChatPanel`, `chat-links`, `GameLayout`). No `public/data` import / `fetch` — no boundary violation. The "absorb into a store-derived selector" is a cosmetic refactor, not a correctness fix; left as-is (already inside the library boundary).
  - **Steps 3-6 (audio / weather / calendar / shop / UI-config) — already clean.** No file under those surfaces imports `public/data/**` or fetches `/data/5e/**` (boundary test green; grep confirms zero raw reads). Converted in earlier sub-phases or never violated.
  - **Step 7 (adventure-loader → façade; allowlist shrink) — DONE.** `services/adventure-loader.ts` `loadAdventures()` now routes through `data-provider`'s `loadJson` (the IPC library façade) instead of `fetch('./data/5e/adventures/...')`. Removed `adventure-loader.ts` from the boundary `ALLOWED_PREFIXES`. Test rewritten to mock `loadJson` (6 specs green). The allowlist is now `services/library/**`, `use-library-store.ts`, and `library-service.ts` (the official ingest loader — it legitimately reads `public/data`, validates, and writes the truth store; `data-provider` is the façade every other loader routes through). The plan's literal "shrink to `services/library/** + use-library-store.ts`" can't exclude the ingest loader itself; documented.
  - **Mis-tagged opt-outs corrected.** Four `// boundary-allow: Phase 15g sweep target` comments were false positives, not pending conversions: `downtime-service.ts` (`DowntimeActivity` interface → re-tagged "type definition"), `commands-dm-economy.ts` (`{...char5e, level}` character mutation → re-tagged "character record mutation, not a library entry"), `commands-dm-map.ts` + `map-environment-commands.ts` (`ChatCommand` definitions → re-tagged "ChatCommand definition, not library data"). The only remaining `15g sweep target` opt-outs are the two in `spell-data.ts`, explicitly scoped to 15h (`*-tables.ts` ported to library entries during cleanup).
  - 4-gate green: lint, tsc-web, tsc-node, vitest 6452.

**15e/15f/15g — COMPLETE 2026-05-28.**

- 15h — DONE 2026-05-28/29 (per user direction: migration framework built DORMANT; config store built + `use-data-store` deleted). Release skipped per user.
  - **Steps 11/12 (docs) — DONE.** CLAUDE.md "When adding new dnd-app files" gained a data-layer sub-bullet; `docs/SUGGESTIONS-LOG-DNDAPP.md` Info section gained a Phase 15 invariant summary. (Step 10 AGENTS.md landed in 15f.)
  - **Step 13 (release.yml tag filter) — verified.** `on.push.tags` is `'v*'` — safe (lightweight `phase-15*-done` tags don't start with `v`). Left as-is.
  - **Step 14 (cut v3.0.0) — SKIPPED per user (cloud session).**
  - **Steps 3-9 (migration framework v4) — BUILT DORMANT (user chose "build dormant now").** `CURRENT_SCHEMA_VERSION` stays **3** so the framework does not touch save files until the release flips it; the runtime shim remains the active conversion meanwhile.
    - `src/shared/migrations/v4-character-refs.ts` (new) — pure, main-process-safe `migrateCharacter5eToRefs` (record-based) = the single source of truth for the v3→v4 conversion. The renderer shim `migrateCharacter5eFromV3ToV4` now **delegates** to it (DRY; its 9 existing specs guard the shared logic). 3 new direct specs (derive/strip, idempotency, no-mutation).
    - `src/main/storage/migrations.ts` — `MIGRATIONS[4]` added (delegates to the shared core for `gameSystem === 'dnd5e'`); dormant (loop never reaches it at version 3). Fixed `migrateData` to capture migration return values (ref-shape migration returns a new object, unlike in-place 2/3).
    - `src/main/storage/snapshot.ts` (new) + 4 specs — `snapshotIfFirstMigration` writes `<save>.pre-phase-15.bak` once when crossing v3→v4; idempotent; never overwrites. Wiring into the file-load path (which has the on-disk path) is release-time.
    - `src/renderer/src/services/io/import-export.ts` — `BACKUP_MIGRATIONS[4]` added (maps each dnd5e character in a backup through the shared core); dormant until `BACKUP_VERSION` bumps to 4.
    - **Release-time remainder (not built):** `migration-report.json` + `getMigrationReport` IPC + `MigrationReportModal`. These surface *orphans* (entries whose ids aren't in the library) — but neither the established runtime shim nor the shared core does orphan detection (it maps ids to refs), and the modal is meaningless until the migration actually runs at release. The orphan-detection + report-UX is a coherent release-time addition, logged for when the v3.0.0 release is cut.
  - **`use-data-store` deleted; config store built (user chose "build a config store").** `git mv` → `src/renderer/src/stores/use-config-store.ts` (export `useConfigStore`); importers repointed (`data-provider`, `use-plugin-store`, `plugin-api`, tests). It remains the imperative-façade loader cache (TTL + waiter coalescing + the homebrew/plugin **merge**) holding the ~20 non-library config categories + caching content for `load5e*`. **Why content still routes through it (logged debt):** the homebrew/plugin merge lives ONLY here — the truth store's `upsertHomebrew`/`loadPluginContent` are not wired into the data-provider load path (`ingestIntoLibraryStore` ingests only the 'official' source), so moving content reads to the truth store would silently drop homebrew/plugin entries from `load5e*` consumers. Fully decoupling content caching requires wiring the truth store's homebrew/plugin merge first — a separate, runtime-risky task documented in the new store's header JSDoc. React components already read content via the truth store (side-write); the dual `clearAll` keeps the two in sync.
  - **Legacy deletions NOT done — plan premise false (documented, not blocking).** `character-common.ts` interfaces (`SpellEntry`/`WeaponEntry`/`ArmorEntry`/`MagicItemEntry5e`) are still referenced (15/11/12/5 sites) — not "unreferenced"; deleting needs consumer conversion. `data/personality-tables.ts` is still consumed and `'personality-tables'` is not a `LibraryCategory` — no port target. Both retained; left as future cleanup.
  - 4-gate green: lint, tsc-web, tsc-node, vitest 6459.

**15a–15h — code-complete 2026-05-28/29.** Phase 15's invariant (library = single source of truth for content; boundary test green) holds across all sub-phases; the v4 migration framework is built dormant; `use-data-store` is gone in favor of `use-config-store`. **Plan file retained (NOT deleted per rule 8):** rule 8 deletes a plan only after its release ships, and the v3.0.0 release is skipped per user this session. Remaining to fully close Phase 15 at release time: (1) cut v3.0.0 (flip `CURRENT_SCHEMA_VERSION`→4, bump `BACKUP_VERSION`→4, wire `snapshotIfFirstMigration` into the load path), (2) add orphan-detection + `MigrationReportModal`/IPC, (3) optional legacy-interface/table cleanup + truth-store homebrew/plugin-merge wiring to fully retire content caching from `use-config-store`.
