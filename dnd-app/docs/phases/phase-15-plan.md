...
- 15e — PARTIAL 2026-05-28 (full-rewrite scope chosen by user 2026-05-28; foundation landed, reader cascade staged).
  ...
  - **Steps 2/5 (token + initiative live-resolution) — FOUNDATION LANDED, reader cascade staged.** New `services/game/token-stats.ts` (+ `.test.ts`, 4 specs) provides `resolveTokenStats` (pure) + `useEffectiveTokenStats` (hook) + `lookupTokenStatBlock`: a token's library-derived base stats (max HP, AC, speeds, senses, resistances/immunities/vulnerabilities, CR) resolve **live** from the library across the three stat-block categories (`monsters`/`npcs`/`creatures` that `loadAllStatBlocks` aggregates), with inline token fields acting as per-instance overrides and as the fallback for non-library-backed tokens. **Key constraint discovered (rule 3):** `MapToken` is polymorphic — player, NPC, custom-creature, and summon/companion tokens often have NO library monster to resolve from, so a destructive strip of inline `maxHP`/`ac`/`resistances`/speeds/senses (as the plan's literal "token holds only `monsterRef`" implies) would break those tokens. The faithful realization is the resolution layer above. **Remaining (the bulk of the 63-file cascade, for a follow-up — analogous to 15c.1 additive → 15c.5 strip across sessions):** (a) migrate the ~15 reader files that read `token.maxHP`/`token.ac`/`token.resistances`/speeds/senses directly (`token-actions.ts`, `creature-mutations.ts`, `vision-computation.ts`, `TokenContextMenu.tsx`, `token-sprite.ts`, `MapCanvas.tsx`, `lighting-overlay.ts`, `TokenPlacer.tsx`, `commands-player-companions.ts`, combat target/falling-damage modals, …) to `useEffectiveTokenStats`/`resolveTokenStats`; (b) once all readers route through the resolver, stop the creation paths (`token-actions.ts` `executePlaceCreature`/`executePlaceToken`, `map-token-slice.ts`) from copying inline derived stats for monster-backed tokens, so library rebalances actually propagate (until creation stops storing inline, the inline override masks the live value); (c) `InitiativeEntry` (`types/game-state.ts:67`) holds `entityId` only — add live max-HP read once the token resolver is the single read path; (d) Step 1 spell-cast modal / condition tooltip live `useLibraryEntry('spells'|'conditions')` reads + Step 3 "Add Monster from Library" drawer `useLibraryEntries` — not yet located/wired. **Note:** the headline acceptance "library edit to a monster's stat block reflects in the open token panel within one frame" is ALREADY met today — `TokenEditorModal` resolves the linked block live via `loadStatBlockById(token.monsterStatBlockId)` + `monsterToDisplay`. The remaining drift is only in the denormalized combat fields (HP bars, AC display), which the resolver layer above closes once the reader cascade lands. 4-gate green (lint, tsc-web, tsc-node, vitest 6453).

**15e — PARTIAL (Step 7 + Step 4 + resolution foundation done; token/initiative reader cascade + creation strip staged for follow-up). 15f — COMPLETE 2026-05-28.** Remaining for Phase 15: 15e token reader cascade (above), 15g (macro/chat/audio/weather/calendar/shop/UI-config sweep + boundary allowlist shrink + `content-index` absorb), 15h (migration framework v4 + cleanup / delete `use-data-store` + legacy `*-tables`; release skipped per user — cloud session).
