...
| Phase 36 | Pi-hosted library + offline cache | **Coordinate.** Phase 36 swaps `loadCategory`'s source from bundled `public/data/5e/**` to Pi-fetch + manifest-keyed cache + seed fallback. Phase 15's architecture (truth store, hydration hooks, schemas, override discipline, boundary test, migration) is unchanged — only the loader's source moves. Phase 36 adds `remote-loader.ts`, `library-cache.ts`, `seed-bundle.ts` to the boundary-test ALLOWLIST. Phase 15 Sub-Phase H deletion list retains the seed slice (`public/data/5e/5e-seed/`). |
