...
- 15h — PARTIAL 2026-05-28 (safe docs done; substantive items blocked by the skipped release + stale plan premises, awaiting user direction).
  - **Steps 11/12 (docs) — DONE.** CLAUDE.md "When adding new dnd-app files" gains a data-layer sub-bullet (EntryRef + hooks + boundary test); `docs/SUGGESTIONS-LOG-DNDAPP.md` Info section gains a Phase 15 invariant summary for grep workflows. (Step 10 AGENTS.md landed in 15f.)
  - **Step 13 (release.yml tag filter) — verified.** `.github/workflows/release.yml` `on.push.tags` is `'v*'` — safe (lightweight `phase-15*-done` tags don't start with `v`, so they can't trigger the release workflow). Plan's preferred `'v*.*.*'` is a stricter nicety; left as-is (no release this session).
  - **Step 14 (cut v3.0.0) — SKIPPED per user (cloud session, no releases).**
  - **Steps 3-9 (migration framework v4) — BLOCKED, awaiting user call (data safety).** `CURRENT_SCHEMA_VERSION` is still 3; `MIGRATIONS[4]`, `src/shared/migrations/v4-character-refs.ts`, `src/main/storage/snapshot.ts`, the `migration-report.json` + `getMigrationReport` IPC, and `MigrationReportModal` do not exist. The v3→v4 *character* conversion already happens at runtime via the existing shim `migrateCharacter5eFromV3ToV4` (`types/character-5e-migration.ts`, run on load in `use-character-store`). Bumping `CURRENT_SCHEMA_VERSION` to 4 now would mark the user's on-disk saves as v4 with **no released build carrying the migration** (the v3.0.0 release is explicitly skipped this session) — a dev build would then write v4 saves that a released v2.x build rejects. That is a save-file-affecting change that should ride with the release, not land blind/untestable in a cloud session. **The migration framework is the v3.0.0 release payload; deferred until the release is cut.**
  - **Cleanup deletions — BLOCKED / stale premise.**
    - `stores/use-data-store.ts` canNOT be cleanly deleted: it is a generic TTL cache keyed by ~80 `DataCategory` values, **~20 of which are NOT library content** and have no `LibraryCategory` home (`themes`, `diceColors`, `keyboardShortcuts`, `dmTabs`, `notificationTemplates`, `xpThresholds`, `abilityScoreConfig`, `currencyConfig`, `builtInMaps`, `sessionZeroConfig`, `moderation`, `presetIcons`, `lightingTravel`, `rarityOptions`, etc.). Library *content* already side-writes to the truth store (15a Step 8); the boundary invariant holds regardless. Deleting use-data-store needs an architectural call: keep it as a config-only cache, or build a separate config store. Library content categories' read-through could move off it, but the config categories must land somewhere first.
    - `character-common.ts` legacy interfaces (`SpellEntry`/`WeaponEntry`/`ArmorEntry`/`MagicItemEntry5e`) are **still referenced** (15/11/12/5 non-def sites) — the plan's "unreferenced" premise is false; cannot delete without converting those consumers.
    - `data/personality-tables.ts` is consumed by `npc-templates.ts`, `PersonalityEditor5e.tsx`, etc., and `'personality-tables'` is **not** a `LibraryCategory` — "port to the library" has no target category; needs a category decision.
  - **Net:** Phase 15's core invariant (library = single source of truth for content; boundary test green) is fully achieved across 15a-15g. 15h's remaining items are (a) the release-gated migration framework (defer to release) and (b) cleanup that the plan mis-scoped (use-data-store config split, still-referenced interfaces, non-category tables). Awaiting user direction on (a) defer-vs-build-dormant and (b) the use-data-store config-cache decision before Phase 15 can be marked shippable + its plan deleted.
