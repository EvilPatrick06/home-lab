...
---
**Implementation 2026-05-29 (cloud session — releases skipped; packaged-app size/feature verification + 14i benchmark deferred to a build-capable session):**
- **14a — DONE** (`package.json`, `ollama-manager.ts`). Removed `build.win.extraResources` Ollama entry (the ~2 GB bundle). Removed `getBundledOllamaPath()` + its `detectOllama` branch; detection now falls back to system paths + PATH + running server (comments updated). `app`/`existsSync` imports still used (temp dir / path checks). Release.yml Ollama-step removal is folded into 14h.
- **14b — DONE** (`ollama-manager.ts`). `downloadOllama`/`installOllama` are cross-platform: Linux returns `LINUX_INSTALL_MARKER` + emits indeterminate progress (`onProgress(-1)`), `installOllama` runs the official `curl -fsSL https://ollama.com/install.sh | sh` via a fixed-command `runOfficialInstall` (no arbitrary exec); macOS best-effort `brew install ollama` (`MACOS_BREW_MARKER`) or an actionable throw if no brew. Windows unchanged (temp-dir + `.exe` security guard preserved). `updateOllama` inherits the path (download→install). `ai-handlers` already forwards `{type,percent}` (incl. `-1`). Test updated (Windows-only-throw spec → Linux-marker-guard spec); 37 specs green.
- **14e — DONE** (`updater.ts`, `package.json`). Removed both `disableDifferentialDownload = true` (manual handler + auto-flow); differential is now the electron-updater default. Lowered `build.compression` `maximum`→`normal` (§C1 — enables blockmap block reuse; `store`-vs-`normal` benchmark is 14i, release-gated → deferred). Comments rewritten (the gone 1.7 GB-bundle rationale); noted Ollama is fully decoupled from the update path.
- **14f — DONE** (`updater.ts`). Removed `autoInstallOnAppQuit = true` from the auto-flow (stays `false`, set at init) so installs go ONLY through the controlled `performInstall` path — never silently on quit. `oneClick`/`perMachine` unchanged (§C3). Silent ON → `performInstall(prefs.autoInstallSilent)`; OFF → visible. (SettingsPage copy tweak is folded into 14d/14f UI follow-up.)
- Added `build.publish.timeout: 300000` (§B7 / 14h Step 6).
- 4-gate green: lint, tsc-web, tsc-node, vitest 6467.
- **14d — DONE** (`OllamaManagement.tsx`). Not-installed branch gains a working **Install Ollama** button (`handleInstall`: `downloadOllama()` → `installOllama(path)` → `refreshAll()`, with toasts), self-contained via a local `installing` state (indeterminate — Linux `install.sh` has no parseable progress; matches the 14b `-1` signal). Cross-platform via the 14b manager path. Check/Update controls already present (verified). Re-check preserved.
- **14c — DONE** (`OllamaFirstRunPrompt.tsx` new, mounted in `App.tsx` next to `UpdatePrompt`). One-time modal: on launch, if `localStorage['dnd-vtt-ollama-first-run-prompted'] !== 'true'` AND `detectOllama()` reports `installed: false`, show "Install Ollama for local AI?" with **Install** / **Not now**. Both set the flag (never reappears). Install uses the 14b path (`downloadOllama`→`installOllama`, toasts, indeterminate "Installing…"); a profile with Ollama already present never sees it; detection failure leaves the prompt for a later launch (no nag). No new IPC channel.
- **14h — DONE** (`.github/workflows/release.yml`). Rewrote the `preflight→build→verify-assets` graph into the prescribed **4-job graph**: `checks-fast` (version-match + lint + tsc-web + tsc-node, no vitest — the gate `build` waits on), `test` (vitest sharded ×3, parallel with build), `build` (matrix Win+Linux, `needs: checks-fast`, `electron-builder … --publish never` + dist artifact, Electron/builder cache keyed per-OS), `publish` (`needs: [test, build]`, downloads artifacts `merge-multiple`, `gh release upload … --clobber`, folded 6-asset verify). Deleted the 3 Windows Ollama steps (14a); debug upload now `if: failure()` only (§B4); top-level `concurrency` group `cancel-in-progress: false` (§B5); `npm ci --prefer-offline --no-audit --no-fund` everywhere (§B5). YAML validated; only a header comment mentions Ollama (no steps). Cannot be runtime-verified without an actual tagged release (release-gated) — does not affect the local 4-gate.
- **14g — PARTIAL (safe parts done; deps→devDeps move DEFERRED).** Done: `electron.vite.config.ts` renderer build gains `minify: 'esbuild'` + `sourcemap: false` + `reportCompressedSize: false` (§A4); removed the dead `vendor-anthropic` `manualChunks` rule (§6 — `@anthropic-ai/sdk` is main-only/externalized, never in the renderer bundle). **Deferred:** the `dependencies`→`devDependencies` move (§A2, the big size lever) — the plan itself flags it as "crashes only in the *packaged* app," and the 4-gate (lint/tsc/vitest, all run with devDeps present) provably CANNOT validate it; it needs a packaged-build feature-by-feature smoke. Logged for a build-capable session. `removePackageScripts`/`removePackageKeywords` already default-true; chunk-index gzip (§A3) is an optional micro-win, skipped.

---
**Phase 14 — code-complete 2026-05-29 (cloud session).** 14a/14b/14c/14d/14e/14f/14h DONE; 14g safe parts done. 4-gate green throughout (vitest 6467). **Deferred to a build-capable / release session (cannot be validated in a cloud session):** (1) 14g dependencies→devDependencies move (needs packaged feature-by-feature smoke — the headline size lever); (2) 14i — cut a test release, benchmark the differential delta at `normal` vs `store` compression, verify install/update/first-run flows on packaged Win+Linux, decide+document the Linux update channel (§C4), update `CLAUDE.md`/`docs/RELEASE.md` sizes+asset list. **Plan file retained** per rule 8 (no release shipped this session). Net effect already in code: Ollama unbundled (config), cross-platform Ollama install + first-run prompt + Settings install button, differential re-enabled + compression lowered, silent-on-quit bug removed, Vite build-speed flags, and the parallel 4-job release pipeline.
