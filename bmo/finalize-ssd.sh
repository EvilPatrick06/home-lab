...
cp /home/patrick/DnD/bmo/setup-bmo.sh /mnt/ssd-root/tmp/setup-bmo.sh
...
mkdir -p ~/DnD/bmo/pi/config
mkdir -p ~/.config/rclone
cp /tmp/.env ~/DnD/bmo/pi/.env 2>/dev/null
cp -r /tmp/config/* ~/DnD/bmo/pi/config/ 2>/dev/null  
