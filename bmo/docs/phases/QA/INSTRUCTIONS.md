...
2. **Open bmo's dashboard.** The bmo admin dashboard is **`/bmo`** (the bare root `/` redirects to the VTT — out of scope):
   ...
   - **Web IDE:** `/ide` (standalone full-page) and the **IDE tab** inside the dashboard. (`bmo/README.md` also mentions an embedded editor at `http://bmo.local:5001` — verify which is live; note any mismatch as a finding.)
