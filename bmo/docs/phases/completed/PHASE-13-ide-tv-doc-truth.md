...
Implemented 2026-06-29 against `origin/master@d9dccc65` (re-anchored from the authoring base per INSTRUCTIONS.md rule 3). Server-side Python + docs. Targeted checks green: `python -m pytest tests/test_tv_api.py tests/test_terminal_service.py` → 13 passed; `ruff check` clean; `py_compile` OK on all touched modules. No live-Pi mutation (rule 6).

- **13A** — `routes/tv_api.py`: added `import select` and `_TV_READ_TIMEOUT_S = 8`; `_tv_cmd` gained a `timeout` param that gates `stdout.readline()` with `select.select(...)` and, on expiry, resets the worker and returns `{"error": "TV unreachable", "timeout": True}` (factored the terminate logic into `_reset_tv_worker_locked()`, now shared with `/pair/cancel`). The status `connect_test` and both `pair_start`/`pair_finish` callers pass the short timeout; a pairing timeout returns a fast **503** `unreachable` (which PHASE-12 12D / PHASE-11 11D consume) instead of a ~30s hang. Non-interactive callers keep the unbounded read. Tests: `test_tv_cmd_times_out_and_resets_worker`, `test_pair_start_timeout_returns_503`, `test_pair_finish_timeout_returns_503`.
- **13B** — `dev/terminal_service.py`: `start_pty`/`open_terminal` gained a `cwd` param; the forked child `os.chdir(cwd)` (try/except → falls back to inherited cwd) before `execvp`. `routes/ide.py` `on_terminal_open` now defaults the cwd to `_IDE_ALLOWED_ROOTS[0]` (`~/home-lab`, the explorer/file-API root) and jails any caller-supplied `cwd` through `_ide_safe_path` (out-of-jail → fall back). Test: `test_open_terminal_threads_cwd_and_paint`, `test_open_terminal_defaults_cwd_none`.
- **13C** — `dev/terminal_service.py`: `start_pty` gained `paint_prompt=True`; after the reader thread starts, a `threading.Timer(0.25, …)` sends Ctrl-L (`\x0c`) so the first prompt repaints without a keypress (a redraw only — no blank command injected). Kept minimal per the report's info/unverified status; verified by diff + the param-plumbing test.
- **13D** — `bmo/docs/SERVICES.md`: the `:5001` ports-table row and the `/api/ide/*` row now state the production IDE is `/ide` served in-process on `:5000`, with a Note cross-referencing `DESIGN-CONSTRAINTS.md` "Two IDE implementations coexist". `bmo/pi/systemd/bmo-ide.service` Description marks it the experimental loopback `ide_app` (not the production `/ide`). Text-only; unit not enabled/disabled (rule 6).
