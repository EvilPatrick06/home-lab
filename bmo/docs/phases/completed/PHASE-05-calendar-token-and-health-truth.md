...
- **05A** (2026-06-28) — Persist the access token on every refresh, not just cold build. `CalendarService` now tracks `self._creds` (`bmo/pi/services/calendar_service.py`), `_with_service_retry` captures the token before each call and `_persist_token_if_changed` writes `token.json` when it changed, and `_get_service`'s cached path opportunistically refreshes+persists an expired-but-refreshable cached client. The cold-build refresh and `invalid_grant`/`invalid_scope` → reauth `RuntimeError` mapping are unchanged. Added a module logger (no new `print`). Tests: `TestTokenPersistenceOnRefresh` (persist-on-change, no-write-when-unchanged, cached-expired refresh+persist, cached-valid no-op).
- **05B** (2026-06-28) — Guard Google read-only/auto-generated event types. Added `CalendarReadOnlyEventError` + `NON_EDITABLE_EVENT_TYPES` allowlist + `_is_event_type_restriction()`; `update_event` raises the typed error for birthday/etc. before calling `update()` and also catches a late `eventTypeRestriction` 400; `delete_event` catches the same. `routes/calendar_api.py` `/update` and `/delete` now map `CalendarReadOnlyEventError`→422, `RuntimeError`→503, else→500 (were unguarded 500s). Tests: `TestReadOnlyEventGuard` (birthday→typed error & no update(); default updates; restriction-400 caught).
- **05C** (2026-06-28) — Reconcile the calendar health signal with the live read path (`bmo/pi/services/monitoring.py`). Added `_resolve_calendar_service()` + `_calendar_live_probe()`; before escalating a stale-on-disk `expiry` to the reauth CRITICAL (past the 10-min grace), the monitor now probes a live read — a success demotes to `degraded` ("token persistence lagging") and clears the circuit, so the card no longer says "down" while reads succeed; a genuinely dead token (probe fails) still escalates to the actionable reauth CRITICAL. Tests: `TestCalendarHealthReconcile` (stale+live-ok→degraded+circuit cleared; stale+dead→reauth CRITICAL; probe outcome classification).

_Cheap checks: `pytest tests/test_calendar_service.py tests/test_monitoring.py` 81 passed; `ruff check` clean on all touched modules._
