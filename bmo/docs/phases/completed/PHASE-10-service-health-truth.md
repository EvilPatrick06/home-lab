...
Implemented 2026-06-28 on `auto/bmo-phase-executer` (base `origin/master@a28aa11e`; lines re-anchored from the plan's `a2d87c53` per INSTRUCTIONS.md rule 3). Cheap checks green: `ruff check` clean; `pytest tests/test_monitoring.py tests/test_monitoring_health.py tests/test_config_preflight.py tests/test_notification_service.py` = 64 passed (+ `test_app_endpoints.py` 68 passed, since app.py was touched).

- **10A — honest, single-basis reporting.** The monitor's calendar-expiry messages now say "Access token expired …" (access-token / last-refresh delta) so they don't read as the same figure as config-preflight's on-disk credential-file expiry (`services/monitoring.py` `_check_calendar_token`). Config-preflight gained an additive `calendar_token_ttl_basis` label naming its clock ("on-disk credential file (config/token.json) expiry field") — JSON keys unchanged (`services/config_preflight.py` `run_preflight` return). The mdns probe now reports `status: "skipped"` with a clear "avahi-utils not installed" message instead of a bare `unknown` (`services/monitoring.py` `_check_remote_access`).
- **10B — re-tier expired third-party OAuth.** `google_calendar` removed from `_REQUIRED_FOR_OVERALL` and placed in a new `_DEGRADED_NOT_CRITICAL` set; the `get_status` rollup gained a `degraded` tier (precedence critical > degraded > warning) and a `down_degraded_tier_services` field, so a calendar-only OAuth outage yields `overall: "degraded"` (still flagged with its intact reauth message) while on-device failures (`svc_bmo`/`svc_docker`/network/pihole/cloudflared) still drive `overall: "critical"` (`services/monitoring.py` `_REQUIRED_FOR_OVERALL`, `get_status`). The frontend already maps `degraded` to the amber pill (`web/static/js/bmo.js:1129`).
- **10C — mirror critical/persistent alerts into the notification center.** Added `NotificationService.add_system_notification(key, title, body, severity)` — a key-de-duped feed insert the dashboard bell (`/api/notifications`) reads (`services/notification_service.py`). The monitor gained `set_notification_sink` + `_notify_feed_if_allowed` (state-change de-dupe reusing `_alert_fingerprint`); `_emit_alert` now mirrors CRITICAL/WARNING into the feed and `_process_state_transitions` posts a "recovered" notice + clears the de-dupe (`services/monitoring.py`). `init_services()` wires `health_checker.set_notification_sink(notifier.add_system_notification)` once both exist (`app.py`). A lapsed/revoked calendar token (its existing CRITICAL emit) therefore now raises a single, de-duped in-dashboard re-auth notification.
- **Tests:** `tests/test_monitoring.py` — 10B tiers (calendar→degraded, device→critical, both→critical), 10A mdns skip + access-token relabel, 10C feed mirror de-dupe + no-sink no-op + recovery notice. `tests/test_monitoring_health.py` — existing required-down-critical test re-pointed to `svc_bmo` (its old `google_calendar` example now correctly degrades) + a new calendar→degraded case. `tests/test_notification_service.py` (new) — add_system_notification insert / key de-dupe / distinct-key stacking. `tests/test_config_preflight.py` — the new basis label.
- **Not done (owner action, rule 6 / Out of scope):** running `reauth_calendar.py` / editing live `token.json` / moving the Google OAuth app to "Production"; installing `avahi-utils`. This phase makes the down/recovered signal honest, correctly-tiered, and visible — the genuinely revoked refresh token still needs the owner's one-click reauth.
