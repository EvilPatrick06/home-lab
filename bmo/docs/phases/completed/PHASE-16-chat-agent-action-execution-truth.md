...
- **16A — LED service-key aligned end-to-end.** `bmo/pi/app.py` now registers the live controller under the agent-declared key **and** the legacy alias at both init sites: `service_map["led_controller"] = led_controller` added alongside `service_map["leds"]` in the SIMULATED branch (`app.py:602`) and the real-`LedController` `try` branch (`app.py:617`). The four chat LED handlers (`agent.py` `_handle_led_*`) read `services.get("led_controller")`, which now resolves; `/api/led/*` is unaffected (it uses the global object). Regression test asserts the handler resolves a registered controller and raises when it's absent.
- **16B — command results made honest.** New `CommandError` exception (`agent.py`, defined above `class BmoAgent`) is raised by control handlers on an expected failure instead of *returning* an error string. `_execute_command` gains an `except CommandError` arm returning `{"success": False, "error": <msg>}` — so an error string can no longer be recorded as `success: True` with the error in `result`. Converted every masquerade-as-success error return: the 4 LED handlers (controller-missing + unknown color/mode), all TV control handlers (`tv_pause/play/stop/volume/mute/power/key/off/launch` + "No app specified"), and an explicit `alarm_set` "service not available" guard. The chat reply-assembly gained a `CONTROL_ACTIONS` pass that appends an honest "(Heads up: I couldn't actually do that — …)" line for any failed real-world control command, so the LLM's optimistic confirmation is corrected; informational appends and successful-control replies are unchanged. No new bare `print()` (no-new-prints guard honored).
- **16C — `create_alarm` tz fallback de-crashed.** `bmo/pi/services/timer_service.py:283` replaced `… or datetime.now().astimezone().tzinfo.key` (which raised `AttributeError` on a fixed-offset `datetime.timezone`) with the safe constant fallback the `Timer` ctor already uses: `_normalize_timezone(timezone_name) or DEFAULT_EXISTING_ALARMS_TZ`; the now-unreachable `if not tz_name:` guard was deleted. Any caller with an empty/unresolvable tz now anchors to the default instead of crashing.
- **Tests:** `tests/agents/test_base_agent.py` — handler resolution, `CommandError` on missing controller, `_execute_command` success-truth (failure not false-success; unknown color is a failure; success path intact), and end-to-end reply surfacing (a failed LED command via a `command` block yields a "couldn't actually do that" line; a successful one does not). `tests/test_timer_service.py` — `create_alarm` with `None`/`""`/`"Not/AZone"` creates the alarm at `DEFAULT_EXISTING_ALARMS_TZ` with no `AttributeError`; a valid tz still anchors there. 233 passed across the affected suites; `ruff` clean on the parseable modules; `app.py`/`agent.py` `py_compile` clean.
- **Live-Pi boundary respected (rule 6):** no `systemctl`/deploy/live LED toggle. Physical actuation is confirmed by the owner-run deploy; the code paths are covered by pytest.
