...
BMO's runtime is managed by 9 systemd services plus 3 timers (12 unit files total; sources in `bmo/pi/systemd/`, some also generated by `bmo/setup-bmo.sh`). Each installed unit has a `.service`/`.timer` file in `/etc/systemd/system/`.
...
| `bmo-kiosk.service` | `bmo/pi/systemd/bmo-kiosk.service` | Chromium fullscreen kiosk on HDMI |
| `bmo-dm-bot.service` | `bmo/pi/systemd/bmo-dm-bot.service` | `python -m bots.discord_dm_bot` |
| `bmo-social-bot.service` | `bmo/pi/systemd/bmo-social-bot.service` | `python -m bots.discord_social_bot` |
...
sudo cp ~/home-lab/bmo/pi/systemd/bmo-kiosk.service /etc/systemd/system/
...
Source in repo: `bmo/pi/systemd/bmo-*.service` + generated by `bmo/setup-bmo.sh`
...
1. Write `.service` file in `bmo/pi/systemd/` (or wherever makes sense)
2. Install:
   ```bash
