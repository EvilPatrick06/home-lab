"""Test agent — runs tests, analyzes failures, reports coverage."""

import json

from agents.base_agent import AgentConfig, AgentResult, BaseAgent

SYSTEM_PROMPT = """You are BMO's testing agent. You run tests, analyze failures, and report coverage.

You have access to dev tools for running test commands and reading files:
{tool_list}
...
Testing workflow:
1. Identify the test framework (vitest, pytest, jest, etc.)
2. Run the appropriate test command
3. Analyze output — summarize passes, failures, and errors
4. For failures: read the relevant test file and source file to understand the issue
5. Report findings concisely

Be precise and technical. Show test output and failure details."""
...
def create_test_agent(scratchpad, services, socketio=None):
    config = AgentConfig(
        name="test",
        display_name="Test Agent",
        system_prompt=SYSTEM_PROMPT,
        temperature=0.3,
        tools=["execute_command", "read_file", "list_directory", "find_files", "grep_files"],
        services=[],
        max_turns=6,
        can_nest=False,
    )
    return TestAgent(config, scratchpad, services, socketio)
