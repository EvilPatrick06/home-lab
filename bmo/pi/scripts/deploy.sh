...
# Standalone board-pending-decisions delegator scheduled tasks call to fetch the
# NEW board decisions for their namespace (event-era replacement for the polling
# board-decision-relay). deploy.sh OWNS it too, refreshed every deploy so it can
# never drift. Overridable for tests/alternate hosts.
PENDING_CLI_DEST="${BMO_PENDING_CLI_DEST:-/home/patrick/bmo-board/board-pending-decisions}"
...
# install_pending_decisions_cli — mirror of install_board_cli for the
# board-pending-decisions helper. Generates a THIN DELEGATOR at
# $PENDING_CLI_DEST that execs the deployed canonical
# ($REPO_ROOT/bmo/pi/scripts/board-pending-decisions.sh) IN PLACE, so its data
# dir resolves to the deploy checkout data/ (where the board cog writes the
# decisions outbox). SHA-independent; all writes dry-run-guarded.
install_pending_decisions_cli() {
  local canonical="$REPO_ROOT/bmo/pi/scripts/board-pending-decisions.sh"
  if [ "$DRY_RUN" -eq 1 ]; then
    run "install standalone board-pending-decisions -> $PENDING_CLI_DEST (delegates to $canonical)"
    return 0
  fi
  if [ ! -f "$canonical" ]; then
    log "pending-decisions CLI: canonical $canonical missing — skipping standalone refresh"
    return 0
  fi
  mkdir -p "$(dirname "$PENDING_CLI_DEST")"
  local tmp="$PENDING_CLI_DEST.deploy-tmp.$$"
  cat > "$tmp" <<EOF_PENDING_CLI
#!/usr/bin/env bash
# AUTO-GENERATED by bmo/pi/scripts/deploy.sh on every deploy — DO NOT EDIT.
# Thin delegator: always runs the DEPLOYED canonical board-pending-decisions.sh
# so this standalone path stays in lockstep with master and its data dir
# resolves to the deploy checkout data/ (the dir the board cog writes the
# decisions outbox into).
exec "$canonical" "\$@"
EOF_PENDING_CLI
  chmod +x "$tmp"
  mv -f "$tmp" "$PENDING_CLI_DEST"
  log "refreshed standalone board-pending-decisions -> $PENDING_CLI_DEST (delegates to deployed canonical)"
}
