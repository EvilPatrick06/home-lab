...
# existing backlog is retired opportunistically. CLI/dev/ide/tools/scripts/
# tests/mcp_servers/wake + the *_calendar.py auth CLIs are exempt (stdout is
# their interface). ide/ and tools/ are the IDE-service and dev-tooling split
# of the former dev/ dir (agents-subpackage reorg 2026-07) — same exemption. Lower .print-baseline whenever you convert prints to logging.
# (BMO-SUGGESTIONS 2026-06-22.)
set -euo pipefail
cd "$(dirname "$0")/.."   # bmo/pi
EXCL='^\./(venv|\.venv|cli|dev|ide|tools|scripts|tests|mcp_servers|wake)/|^\./cli\.py:|^\./services/(authorize|reauth)_calendar\.py:|^\./services/calendar/(authorize|reauth)\.py:'
