...
# Preserve CLI/environment overrides even if bmo/.env sets blanks.
CLI_PI_USER="${PI_USER:-}"
CLI_PI_HOST="${PI_HOST:-}"
...
# Load optional environment overrides from bmo/.env
if [ -f "$ENV_FILE" ]; then
...
if [ "$PI_HOST_VERIFIED" = false ]; then
    ...
    log_warn "If this is wrong, set PI_HOST / PI_TAILSCALE_HOST / PI_IP in bmo/.env."
fi
